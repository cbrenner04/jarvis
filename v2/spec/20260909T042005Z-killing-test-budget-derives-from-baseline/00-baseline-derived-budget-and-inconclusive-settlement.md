...
- Before the first candidate of a resolved killing set is tested, the verifier runs that set once on the unmutated tree with the ceiling bound and records its wall time; the measurement is cached per killing-set key for the verification and counts against `MAX_VERIFICATION_MS`; rules out inferring mutant behaviour from a bound the baseline also exceeds.
...
- [ ] `diff-derived-mutation-verifier.test.ts` test `a genuinely non-terminating mutant still settles non-terminating-mutation` proves a baseline well inside the bound and a mutated run exceeding it still settles `non-terminating-mutation`.
