...
- [ ] A ready-gate test whose finalizer input carries `readyCommand: "npm run verify"` asserts the gate spawns `npm` with `["run", "verify"]` and never `bun run ready`; it fails against the pre-fix hardcode.
- [ ] A sibling ready-gate test with no `readyCommand` on the input asserts the gate still spawns `bun` with `["run", "ready"]`.
- [ ] A classification test asserts a failure whose `ReadyGateError.command` is the run's configured ready command, with a scope carrying that same `readyCommand`, still reaches `ready_gate_out_of_scope` on out-of-allowset failing paths; it fails against the pre-fix literal compare, which returns `ready_gate_failed`.
- [ ] A classification test asserts a required-integration failure (`ReadyGateError` with command `bun run test:integration:v2`) against a scope whose `readyCommand` is a configured command still classifies `ready_gate_failed` — the negative case proving the command guard suppresses non-gate errors rather than the guard never firing.
- [ ] A `run workflow implement` admission test with `projects.demo.readyCommand` configured asserts the sent write step carries that `readyCommand`; it fails against the pre-fix admission, which stamps only `fixCommand`.
- [ ] An admission test asserts a project with no `readyCommand`, and one whose `readyCommand` is whitespace-only, both send a write step with no `readyCommand` property.
- [ ] `v2/src/execution/ready-finalize.test.ts` — `runs the configured ready command as the ready gate`; Keystone checkpoint: its test body carries `// @mutate v2/src/execution/ready-finalize.ts "resolveReadyGateCommand(gateOptions?.readyCommand)" -> "resolveReadyGateCommand(undefined)"`, reverting the gate to baseline `bun run ready` semantics, and the mutation turns that regression RED while the fallback test stays green.
- [ ] `v2/src/execution/ready-finalize.test.ts` — `classifies a configured-command failure as out of scope`; Mutation checkpoint: its test body carries `// @mutate v2/src/execution/ready-finalize.ts "error.command !== resolveReadyGateCommand(scope?.readyCommand).display" -> "error.command !== \"bun run ready\""`, restoring the literal classification key, and the mutation turns that test RED.
- [ ] `v2/src/execution/ready-finalize.test.ts` — `keeps a required-integration failure unclassified`; Mutation checkpoint: its test body carries `// @mutate v2/src/execution/ready-finalize.ts "if (error.timedOut || error.command !== resolveReadyGateCommand(scope?.readyCommand).display) {" -> "if (error.timedOut) {"`, dropping the command guard so a non-gate error classifies as out of scope, and the mutation turns that test RED.
- [ ] `v2/src/config/machine-config-loader.test.ts` — `readProjectReadyCommand ignores a blank or non-string readyCommand`; Mutation checkpoint: its test body carries `// @mutate v2/src/config/machine-config-loader.ts "typeof readyCommand === \"string\" && readyCommand.trim() !== \"\"" -> "typeof readyCommand === \"string\""`, letting a whitespace-only value through as a gate command, and the mutation turns that test RED.
- [ ] `v2/src/commands/workflow.test.ts` — `stamps the configured readyCommand onto write steps`; Mutation checkpoint: its test body carries `// @mutate v2/src/commands/workflow.ts "...(readyCommand !== undefined ? { readyCommand } : {})," -> "...({}),"`, dropping the admission carry, and the mutation turns that test RED.
- [ ] `v2/docs/install-and-config.md` — the project registry section documents `projects.<key>.readyCommand` and `projects.<key>.fixCommand` as non-empty-string overrides read at write-step admission, with the `bun run ready` / `bun run fix` defaults, that a blank or non-string value reads as absent, that the configured command is what appears in gate-failure output and the repair prompt, and that terminal-publication settlement does not consume them.
- [ ] `v2/docs/v1-behaviors.md` — an entry records that v2 now honors the per-project `readyCommand` v1 already honored (v1 source: `v1/src/modes/patch/completion-pipeline.ts`), naming the v2 seams (`v2/src/commands/workflow.ts` admission, `v2/src/execution/ready-finalize.ts` gate) and the remaining delta: v2 resolves it at CLI admission and carries it on the write step, and v2's terminal-publication gate does not consume it.
