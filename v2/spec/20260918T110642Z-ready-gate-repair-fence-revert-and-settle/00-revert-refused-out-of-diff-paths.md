...
- [ ] A new `write-loop` test drives a repair pass that modifies a tracked path and creates an untracked path outside run diff and spec tree; afterward `git status --porcelain` is empty for both (tracked restored to pre-repair content, new file deleted) and the failure detail names both paths and states they were reverted; it fails against the pre-fix leave-dirty behavior.
