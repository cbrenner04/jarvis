...
- Anchor functions to relocate: `tryFinishSpecIfDone`, `runCompletionReadyGate`, `generatePrBody`, plus supporting helpers (`mapExitCodeToReason`, `normalizeReadyFailureText`, `isReadyFailureUnchanged`, `diffAcceptanceCriteria`, `getSpecDisplayName`, `getIndexTitle`, `getCurrentBranch`, `lookupPrUrl`). Rules out splitting a helper from its sole caller.
...
- [ ] `v1/src/modes/patch/completion-pipeline.ts` exists and defines `tryFinishSpecIfDone` and `runCompletionReadyGate`.
