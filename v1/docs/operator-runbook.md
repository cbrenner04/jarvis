...
**Which file is authoritative (2026-07-13).** `~/.jarvis/runs.jsonl` is the source of record and is **v1-only** — a v2-driven run writes **nothing** to it. v2 writes `~/.jarvis/telemetry.jsonl` instead, and every v2 invocation currently records `cost_usd: null` / `usage_source: unavailable`, so **cost for v2 work is presently unattributable** (seed `shared-invocation-loses-cost-and-claude-output`). Do not silently fill v2 cost cells: leave them blank with a note, per the missing-binding rule below. Until that seed ships, a v2-heavy session's spend cannot be reported, and that is a finding to state, not a gap to estimate around.
