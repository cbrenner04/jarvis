...
| ex-wlsl-fixture-golden | v2/src/execution/write-loop-staged-markdown-lint.test.ts | plan/intent write step staged Markdown lint cases | staged markdown lint uses committed golden/violation fixture bodies | committed `REVIEW_MD_LINT_FIXTURE_IDS` registry with `readReviewMdLintFixture` / `locateDiscoveredFile` loud-failure reads for plan/intent reprompt and finalize golden bodies | behavioral | n/a | | |
