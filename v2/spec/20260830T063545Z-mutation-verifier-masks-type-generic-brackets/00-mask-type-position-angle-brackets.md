...
- [x] `diff-derived-mutation-verifier.test.ts` proves a real value comparison (`a < b`) on a line that also contains a generic still yields the comparison as a candidate — constructible on main today; fails only if over-masking removes it.
...
- [x] Existing `diff-derived-mutation-verifier.test.ts` `describe` blocks stay green: `diff-derived-mutation-verifier`, `verification bounds`, `masking non-code spans`, `maskNonCodeSpans`, `dual-constraint detection`.
- [x] `bun run typecheck`, `bun run test:v2`, and `bun run test:integration:v2` pass.
- [x] `v2/docs/workflow-runner.md` completion-verification paragraph records that pinned type-position `<`/`>` forms (generics, type casts, type params, `satisfies`, arrow generic params) are excluded from operator-flip candidates alongside string/template/comment masking, and names residual risks (unpinned nested-generic depth, old-style assertions, TSX/JSX) consistent with existing masking caveats.
- [x] `v2/docs/v1-behaviors.md` diff-derived mutation evidence entry records the same fixture-bounded type-position bracket masking for comparison-operator candidates and the same residual risks — not complete TypeScript type-syntax exclusion.
