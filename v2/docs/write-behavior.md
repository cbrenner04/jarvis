...
When completion publication's ready gate fails with a `ReadyGateError`, the write loop invokes the agent again with the gate command, exit code, and the last 16 KiB of gate output. It commits and republishes after each repair, for at most three repair attempts. Repair iterations consume the normal iteration budget; a blocked repair, exhausted budget, or still-red gate returns retryable `ready_gate_failed`. When the repair budget is exhausted with a still-red gate, the row settles `failed` (not `completed`), logs `readyGateFailureOrigin: repair_budget_exhausted`, `readyGateRepairCount`, same-run `finalizationLineageRunId`, and `retainedFinalizationCheckpoint` when draft-PR evidence exists, and skips draft→ready flip. Gate-only `jarvis run resume` replays the retained finalization checkpoint (operator changes committed under original attribution, existing draft PR refreshed), then runs the ready gate, required integration (when applicable), mutation verification, runtime smoke, and a single draft→ready flip on success; a green gate completes the same row; a red gate returns `failed` / `ready_gate_failed` / resumable without write-agent dispatch and admits another gate-only resume. Flip failures return `ready_flip_failed` and do not trigger repair.
