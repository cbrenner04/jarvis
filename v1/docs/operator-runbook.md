...
- **One-run actuator probes:** use `jarvis1 run --agent`, `jarvis1 plan --agent`, or `jarvis1 prompt --agent` instead of editing config `agentOrder` (see [agents.md](./agents.md#per-run---agent-override)). Seed `per-run-agent-override-flag` tracks future cross-mode override work beyond these surfaces.
