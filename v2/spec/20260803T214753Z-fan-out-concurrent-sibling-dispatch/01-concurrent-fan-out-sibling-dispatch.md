...
- [x] `pipeline-execution.test.ts` proves a cross-branch wait yields to the macrotask queue — a `setTimeout(…, 0)` armed before the wait fires while a peer is still pending — and that the wait is bounded, settling a named failure rather than hanging when the peer never advances.
