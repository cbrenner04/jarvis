...
- Shared invocation preserves v1 patch-mode fallback semantics: rotate only on quota-classified outcomes in the effective order; hard `error`, `model_config`, `timeout`, and abort stop immediately.
- Phase 1 runs one agent turn total; `progress` returns a typed non-success result with no contract check and no second invocation, because looping starts in Phase 2.
- The first `write` contract should use the smallest deterministic primitive set that proves the produced artifact in the worktree; Deferred to first consumer: any broader reusable contract DSL — pin when a second behavior or step needs it.
- The durable operator doc owns how to run `write` and how to verify success; `v2/docs/v2-build-order.md` and `v2/spec/v2-meta-index.md` remain sequencing only, and `v2/docs/v2-architecture.md` remains boundary-only.
- External worktree helpers must take explicit project/worktree naming inputs from the runner call site rather than deriving names from spec-path conventions, because Phase 1 worktrees are behavior-run outputs, not spec-owned patch branches.
- `write.execute` is the prompt identity boundary for this slice; do not introduce `write.step`, `phase-1-*`, or other orchestration-labeled prompt/runtime IDs before workflows exist.
- Draft split rule: keep shared refactors shippable on their own, but merge adjacent seams rather than force four tiny PRs when a smaller isolated slice would leave dead plumbing with no caller.
- Phase 1 must not assume a preexisting `src/shared/` surface: if prompt/agent code moves out of `v1/**`, land the smallest root-shared extraction in the same subspec as its first live caller rather than as dead preparatory plumbing.
- `v2/docs/prompts.md` contains a historical `write.patch.execute` example; treat it as design-era illustration, not Phase 1 naming authority, and update that doc only if the prompt contract itself changes.
- Contract-miss results must stay distinct from agent-declared `blocked` results, because a false terminal claim and an external dependency blocker need different operator diagnosis even if both stop the run.
