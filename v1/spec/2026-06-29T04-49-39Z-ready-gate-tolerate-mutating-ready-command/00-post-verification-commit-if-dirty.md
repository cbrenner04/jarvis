...
- Pre-verify porcelain must already be clean (existing pre-ready fix-commit idempotency contract); post-verify dirt is treated as verification-produced churn on the common path — rules out path-scoped diffing or allowlists.
...
- No second verification pass after post-verification commit — rules out verify-after-commit loops; verification already returned green.
...
- Mirror pre-ready commit semantics (trailer threading, push, post-commit porcelain re-check) for the post-verification step; introduce post-verification commit/push error types.
...
- Add coverage for mutating `readyCommand` that leaves committable churn and for residual still-dirty after post-verify commit.
- Update durable docs for the new gate step and error semantics.
...
- [ ] Residual still-dirty porcelain after post-verification commit still throws `ReadyVerificationDirtyError` (or successor) with guidance to inspect unexpected changes, not to fold autofix into `readyCommand`.
...
- [ ] `v2/docs/v1-behaviors.md` records post-verification commit-if-dirty, updated gate ordering, error classification, recorded-green timing, and retry semantics.
- [ ] `v1/docs/operator-runbook.md` **The gate** section describes when the harness commits after verification and cross-links `v2/docs/v1-behaviors.md`.
- [ ] Stale gate-order recitations in `v1/docs/run-loop.md`, `v1/docs/config.md`, `v1/docs/worktrees-and-commits.md`, and `v1/docs/workflows.md` are updated or deduped per `v2/docs/documentation-standard.md`.
- [ ] `bun run typecheck` passes.
- [ ] `bun run test` passes.

## Documentation updates

- `v2/docs/v1-behaviors.md` — post-verification commit-if-dirty, gate ordering, error/retry classification, recorded-green timing (primary durable home).
- `v1/docs/operator-runbook.md` — **The gate** section: harness commits after verification when applicable.
- `v1/docs/run-loop.md` — completion-transition gate step order and exit-6 table rows for post-verification failures.
