...
- **Tests beside v2 source.** Co-located under `v2/src/<domain>/`; no `v2/test/` mirror of `v2/src/`. Grandfathered `v2/test/fixtures/` (Biome demos) stays; other fixtures stay near owning code unless shared. [`Source layout`](v2-architecture.md#source-layout).
...
The same idea also describes implementation. Patch mode today is roughly write loops (**`implement`**) plus human checkpoints:
