...
- **Per-turn commits on the published branch**: git-enabled v2 intent, plan, and implement workflows preserve each qualifying per-turn commit on the published branch. Terminal boundaries (`write` completion, optional shrink, review/review-debate publication) append a commit only when that boundary introduces file changes not already on `HEAD`. Each commit carries its own `Jarvis-Agent` trailer (the agent that produced that turn's edits) and `Jarvis-Step` trailer classifying the workflow purpose; branch commit count is monotonic across those boundaries — prior per-iteration checkpoint commits remain ancestors of the published tip. v2 `## Commits` (plan/implement template) and the attribution footer list every qualifying `Spec:` commit ahead of base individually, with each commit's `Jarvis-Agent` label; the `Written by` summary dedupes contributing agents in first-appearance order. Only operator squash-merge to `main` may squash; the harness does not collapse per-turn history before merge.
