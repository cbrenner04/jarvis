...
- Run `bun run typecheck` and `bun test` before ticking the acceptance criteria they cover. Do not run `bun run ready` — Jarvis runs that harness gate automatically when the spec completes and flips the draft PR to ready.
