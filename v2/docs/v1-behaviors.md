...
- Seeded plan flows run intent draft, then refine (controlled by `--refine-turns`, default `3`), draft phase, then review cycles (controlled by `--review-passes`, default `1`, where each cycle = debate role sequence) with explicit per-cycle stderr milestones. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/review.ts`
- The pipeline is not always end-to-end in one invocation: for fresh `commit: true` seeded plans (file or inline), the run stops after the refine phase by appending a review-gate `## Blocker` to `intent.md` and exits `1`; drafting and review only run after the operator clears the blocker and reruns with `--resume-draft`. `commit: false` flows instead continue through draft and review in a single invocation. Sources: `v1/src/commands/plan.ts` (`shouldStopAfterPhase0Refine`, `appendPhase0ReviewGateBlocker`)
- When `--refine-turns 0` is set on a fresh seeded run, jarvis still runs the intent-draft step, then skips refinement entirely. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`
...
| --- | --- | --- | --- |
| inline (text arg) | `true` | seed `intent.md` -> intent draft -> refine -> review-gate `## Blocker` | exits `1`; operator clears blocker and reruns `--resume-draft` for draft+review |
| inline (text arg) | `false` | seed `intent.md` -> intent draft -> refine -> draft -> review | exits `0`; prints `jarvis1 run <indexPath>` next step |
| file (path arg) | `true` | seed `intent.md` -> intent draft -> refine -> review-gate `## Blocker` | exits `1`; operator clears blocker and reruns `--resume-draft` for draft+review |
...
- `--refine-turns 0` keeps the intent-draft step and skips refinement; there is no fresh interactive rejection path because there is no no-arg fresh mode. Sources: `v1/src/commands/plan.ts`
