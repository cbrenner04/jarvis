...
**Non-terminating mutants.** When a scoped killing test exceeds the 30-second per-candidate wall clock, verification settles `non_terminating_mutation_failed` with `nextAction: "resume"` — distinct from `surviving_mutation_failed` (tests pass under the mutation). In-loop and publication-time discovery share this settlement: no live-agent reprompt and no review `write.mutation-repair`. Recovery is agent-free `jarvis run resume` on the owning write row after fixing the killing test or mutation site; the resume tail replays mutation re-verification, the ready gate, and publication only.
