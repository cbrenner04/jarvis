...
| Command | Use |
| --- | --- |
| `jarvis tui` | Split-pane run monitor (stacked below 120 cols): left pane is one unified work tree — pipelines (pipeline → stage → run) and ad-hoc `run workflow` invocations as top-level rows, ordered together — plus a queue block; right pane shows pipeline context and stage roll-up for a pipeline, then the selected durable-stage record for a stage, then selected durable-run workflow/outcome/error/PR/worktree detail for a run selectable in the left pane (`monitorSelectableNodeIds`); an ad-hoc row shows only its selected durable-run detail, with no pipeline context. Retained steering feedback follows selected-run detail. Rows losslessly hard-wrap by display columns—split right-pane width or stacked terminal width, floored at one—with no ellipsis and preserved tones; extended grapheme clusters stay atomic, so a wider grapheme can overflow a narrower row. The 4-line dock supports `[`/`]` divider nudges; **`j`** or ↓/↑ walk every row in the work tree (queue display-only); **`e`** expands/collapses a selected pipeline or stage and is a no-op on an ad-hoc row; kill (`k`) on live runs. Typed steering (`pause`/`kill`/`resume`/`approve`/`reject`) refuses an ad-hoc row selection with the `unattributed` code. `:` or `/` focuses command input. **`elapsed`** shows pipeline age from `createdAt` through `finishedAtMs` (or live clock), stage age from `startedAt` through `endedAt` (or live clock), and run age from `createdAt` through `finishedAtMs` (collapsed workflow rows use the representative run's timestamps); elapsed freezes only when the recorded end timestamp is present and keeps advancing on the local display tick when status is terminal but the end timestamp is absent |
