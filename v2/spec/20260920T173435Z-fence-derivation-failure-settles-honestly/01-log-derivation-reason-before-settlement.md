...
- The record is appended before settlement at both sites, so it survives a later change to the settlement shape, which is deferred (see `## Deferred` in `intent.md`).
