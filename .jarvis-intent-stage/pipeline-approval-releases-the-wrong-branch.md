...
Splitting does not apply: every acceptance criterion lands on the daemon pipeline continuation path (`continuePipeline` / `runPipeline` / approval continuation); persistence reopen already admits one failed branch row and CLI approve wiring is branch-scoped.
...
- `pipeline resume` admits a fan-out with exactly one failed branch stage; `multiple_failed_stages`
  remains refusal when multiple independent branch failures exist — rules out leaving resume blocked
  after a single recoverable branch failure.
...
- Daemon pipeline stage-advance resolves successors keyed by `branchKey`.
