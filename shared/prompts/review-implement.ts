...
function passContext(context: ReviewDebateRenderContext): string {
  const base =
    context.totalPasses === 1
      ...
      : `This is review pass ${context.passNumber} of ${context.totalPasses}.`;
  return context.priorCycleVerdict?.trim()
...
export async function renderPatchReviewCriticPrompt(
  ...
  return renderStep(PATCH_REVIEW_CRITIC_PROMPT_ID, context, {}, runner);
}
export async function renderReviewDebateRolePrompt(
  ...
  return renderStep(
    PATCH_REVIEW_DEBATE_ROLE_PROMPT_IDS[role],
    context,
    role === "advocate"
      ? { ADVERSARY_FINDINGS: priorRoleOutput ?? "(no prior findings)" }
      : role === "adjudicator"
        ? { ADVOCATE_RESPONSE: priorRoleOutput ?? "(no advocate response)" }
        : {},
    runner,
  );
}
export const implementReviewPromptProfile = bindReviewPromptProfile<ReviewDebateRenderContext, ReviewDebateRenderRole>(
  implementReviewProfile,
  {
    critic: (context) => renderPatchReviewCriticPrompt(context),
    actuator: (context, verdict) => renderReviewDebateActuatorPrompt(verdict, context.specPath),
    debateRole: (role, context, prior) => renderReviewDebateRolePrompt(role, context, prior),
  },
);
...
export async function renderReviewDebateCyclePrompts(
  context: ReviewDebateRenderContext,
  prior: Partial<Record<ReviewDebateRenderRole, string>> = {},
  runner = realAsyncSubprocessRunner,
): Promise<ReviewDebateCyclePrompts> {
  return {
    adversary: await renderReviewDebateRolePrompt("adversary", context, undefined, runner),
    advocate: await renderReviewDebateRolePrompt("advocate", context, prior.adversary, runner),
    adjudicator: await renderReviewDebateRolePrompt("adjudicator", context, prior.advocate, runner),
  };
}
export function nextReviewDebateCycleContext(
  ...
  return { ...context, passNumber: context.passNumber + 1, priorCycleVerdict };
}
