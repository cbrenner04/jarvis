...
- [ ] Selecting a group representative, and selecting a member of an already-expanded stage, each materialize exactly one run node per member with unchanged node ids — `v2/src/tui/tui-monitor-pipeline-tree.test.ts` — `selecting a representative or an already-visible member materializes each member row once`.
