...
- [ ] A test proves a bullet reading "builds a.ts — rules out b.ts; adds c.ts." is still refused for naming two built artifacts (a.ts and c.ts), since b.ts sits only inside the rules-out clause.
