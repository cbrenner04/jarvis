...
- [x] Pass the resolved project (`ProjectMatch`) and effective `commit` flag into
  `cleanupCommand` from the CLI cleanup case.
- [x] When `commit === false`, resolve source/destination/ready-intent under the
  external home via `computeProjectSafeId`/`computeNoCommitSpecRoot` and the config dir;
  skip the in-repo home search.
- [x] Perform external archiving as a filesystem move with no git operations; prune
  the consumed ready-intent only after a successful move (no-op if absent).
- [x] Refuse a reserved-name (`completed`/`ready-intents`) spec-dir basename before
  any move or prune.
- [x] Tests cover: commit:false external archive move + ready-intent prune, no git
