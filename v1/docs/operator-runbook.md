...
- **`bun run ready`** — the full completion gate. Built-in `ready` is strict verification-only; built-in autofix moved to `bun run fix`. The authoritative built-in ready/fix split and exact step order live in [`v2/docs/v1-behaviors.md`](../../v2/docs/v1-behaviors.md). Jarvis runs `ready` on spec completion; the operator runs it before any hand/admin-merge. `jarvis1 triage <spec-path> --merge` (or PR ref / worktree name) runs this gate as the first step before waiting for CI, so both gates are enforced together.
