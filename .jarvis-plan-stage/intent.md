...
- On a missing `machineProfile`, require `--profile` and accept only a committed `config/machines/<name>.json` resolved from the Jarvis checkout, listing available profiles on omission or mismatch; a supplied profile that differs from an existing `machineProfile` exits `1` without mutation; rules out guessing, overwriting, or resolving profiles from the target repo.
- Preserve existing `agents` and `machineProfile`; rules out using init as an overwrite command.
- Register `projects.<key>` with resolved cwd as `root` and `git remote get-url origin` as `origin` when present, where key is `--name` or the repo basename; a missing origin still completes setup but the required-origin preflight exits `1`, and an existing registered origin is preserved on rerun; rules out relocating the repo under `~/Work`, requiring an origin for setup, or silently refreshing a stored origin.
...
- Make `--check` run the same report without config, scaffold, or repo writes; reject `--scaffold`, resolve the project key from `--name` or cwd basename, the profile from `--profile` or stored `machineProfile`, and the spec directory from `--target-dir`, stored `plan.targetDir`, or the `spec` fallback, all read-only. With missing config, supplied selectors only identify checks: they do not establish setup state, so missing profile or registration remains required and exits `1`; rules out a diagnostic command that repairs state implicitly.
...
- [ ] Missing `--profile` when `machineProfile` is absent, a supplied profile that conflicts with the configured profile, an unknown profile, none of the default candidate agent CLIs when `agents` is absent, and a project key bound to another root each exit `1` without mutation and print actionable diagnostics; profile diagnostics list committed choices and key-conflict diagnostics name both roots.
...
- [ ] `jarvis init --check` writes nothing, rejects `--scaffold`, resolves supplied profile, name, and target-directory selectors without persisting them, reports every check once, exits `1` when config registration or another required check such as GitHub authentication is missing, and exits `0` when all required checks pass; tests inject check runners and fail against the baseline.
