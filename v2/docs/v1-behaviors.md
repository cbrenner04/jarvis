...
- Inline-draft, refine, and draft prompts all forbid self-referential deliverables: acceptance criteria must verify target state outside the active spec directory (code/tests/docs/operator behavior/generated evidence), so criteria that only grade active-spec prose are invalid even without saying "the spec." Sources: `prompts/plan/inline-draft.md`, `prompts/plan/refine.md`, `prompts/plan/draft.md`
...
- File/interactive plan flows seed an intent file and then run an agent-driven phase pipeline: intent seeding, refine phase (controlled by `--refine-turns`, default `3`), draft phase, then review cycles (controlled by `--review-passes`, default `1`, where each cycle = debate role sequence) with explicit per-cycle stderr milestones. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/review.ts`
...
- Plan prompts are intentionally data-delimited with sentinel delimiters (e.g., `<<<INTENT_BEGIN>>>`, `<<<INTENT_END>>>`) so intent/spec text is treated as content, not executable instructions. Template rendering is non-recursive: placeholders in the template source are substituted exactly once, and literal placeholder-looking text in injected values (e.g., `<SPEC_GUIDANCE>` appearing in intent) is preserved as data without recursive substitution. This allows spec-governance and prompt-documentation content to reference exact placeholder names. Sources: `v1/src/modes/plan/template-renderer.ts`, `v1/src/modes/plan/inline-draft.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/review.ts`, `v1/src/commands/plan.ts`
