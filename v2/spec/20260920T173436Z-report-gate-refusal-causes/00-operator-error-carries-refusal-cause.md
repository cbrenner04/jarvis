...
- [x] A test drives a genuinely corrupt stored `gate_refusal_recovery_state` column through `StateStore.loadRun`'s decode into `composeRunOperatorError` and proves it composes as `legacy_unknown` with no slot count or bound.
