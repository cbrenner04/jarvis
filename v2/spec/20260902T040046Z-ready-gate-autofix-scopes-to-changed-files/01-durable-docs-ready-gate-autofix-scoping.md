...
- [ ] `v2/docs/operator-runbook.md` § Autofix, bounded repair, and settlement documents scoped built-in autofix on changed paths, raised `--max-diagnostics` on autofix biome, that out-of-diff pre-existing findings do not settle `completion_commit_failed` for built-in autofix, and that successful built-in autofix does not surface or name untouched out-of-diff pre-existing findings.
- [ ] `v2/docs/write-behavior.md` ready-gate repair autofix paragraph documents built-in scoping to changed paths and distinguishes it from completion-commit scoped format and configured `fixCommand`.
- [ ] `v2/docs/v1-behaviors.md` records v2 built-in ready-gate repair autofix scoping to changed paths with raised diagnostic limit as the current parity baseline.
