...
- Prompt telemetry is written with namespace `<project-key>:prompt` (matching patch mode's `<project-key>:run` and plan mode's plan naming), mode field `"prompt"`, and one terminal row per command (intermediate fallback attempts are stderr-only). Exit reasons include `success`, `all-agents-quota`, `agent-failure`, `model_config`, `watchdog-iteration-timeout`, and others matching patch-mode taxonomy. Sources: `v1/src/modes/prompt/run.ts` (`appendTelemetryLine`)
