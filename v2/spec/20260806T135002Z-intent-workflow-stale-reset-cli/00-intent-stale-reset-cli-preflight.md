...
ownership-stamped `.jarvis-intent-review-verdict.md` plus `.owner` sidecar. `STALE_RESET_WORKFLOWS`
excludes `intent`, so the next CLI re-run reuses that tree; review then refuses non-retryably with
