...
- **`daemon-wait-run-completion.test.ts` should not need a named timeout.** Its intermittent `Test (v2)` staller was root-caused to leaked daemon wait handles and fixed at teardown/source.
