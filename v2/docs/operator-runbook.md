...
A resumed failed `plan` redraft automatically retires ordinary uncommitted `.jarvis-plan-stage/` draft dirt and rematerializes the lane from base before dispatch; whole-pipeline and branch-scoped resume need no preceding `cleanup --abandon` or override flags for that draft dirt. Branch-scoped resume resets and dispatches only the named lane — sibling approval gates and stages stay untouched. Reset policy survives detached continuation, claim loss, and daemon restart via a durable row marker until dispatch completes. Optional `--reset-despite-dirty` and `--reset-despite-landed-criteria` mirror standalone `plan`/`implement` re-run overrides — see [Incomplete re-run preflight gates](#incomplete-re-run-preflight-gates): each flag skips only its matching stale-reset gate when auto-clear or ordinary preparation would refuse (`--reset-despite-dirty` gate (3) only; `--reset-despite-landed-criteria` gate (2) only). Neither flag overrides the descendant check, a live-held worktree claim, or an operator `## Blocker` on staged `intent.md` (reserved harness `Artifact contract check failed:` sections are cleared with draft dirt). The worktree is preserved and the stage fails without dispatch for those preserved refusals or when stale-reset preparation refuses without a matching override. Resolve the named state first; reserve `jarvis cleanup --abandon <branch>` for preserved refusals or partial teardown, and use `pipeline recover` instead when the corrected staged tree should be landed rather than discarded.
