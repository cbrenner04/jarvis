...
- `recoverReconciledRuns` in `daemon.ts` calls `admitRunForResume` before its failed settlement (reconciliation leaves the dead prior owner stamped) and logs a rejected outcome as `run_settlement_rejected`. The other unguarded resume paths (write-loop resume, workflow publication repair, `recoverPlanStage`) are routed through admission by `run-admission-stamps-owner-daemon`.
