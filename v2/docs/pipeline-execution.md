...
Failed-plan `pipeline resume` is the current disposable-lane caller. Before `runPlanStageStaleResetPreflight` threads `disposableLane: true` through `maybeResetStaleWorkspace`, `classifyNeverLandedLane` structurally requires no open PR on the lane branch and no commits ahead of base whose changed paths leave harness workflow staging. An open PR or an ahead-of-base commit touching any other path classifies the lane `landed` (non-disposable). A failed `gh pr list` probe or git error classifies it `inconclusive`, carrying the probe reason: resume then refuses before any reset, preserves the worktree, does not pass `disposableLane`, and does not bypass a staged operator blocker — the refusal (`never-landed classification is inconclusive (…)`) names `gh` reachability and the sandbox-safe recovery (re-run `pipeline resume` where `gh` is reachable, or `cleanup --abandon` after confirming no open PR). A failed probe is never read as a confirmed absence of PRs. A structurally never-landed lane may therefore be retired and rematerialized from base through non-descendant `HEAD` and landed-criteria drift without reset overrides, while path-scoped unlanded commits, live run/worktree ownership, claims, ready or multiple PRs, and operator dirt outside harness draft staging still refuse. Intent/implement redispatch and ordinary stage dispatch do not mark lanes disposable.
