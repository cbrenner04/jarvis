...
| `run_leaf` | The selected row is a nested run leaf |
| `unattributed` | The selected row is an unattributed run |
| `stale_non_targetable` | The selected pipeline or stage has no live owning daemon in the current refresh — retained rows only; also fires when a registry-named pipeline's selected gate stage has been elided from the tree (decided or never reached) |
| `not_awaiting_stage` | `approve` / `reject` require an awaiting stage selection — reachable only via a still-rendered gate row, so this never fires for an elided registry-pipeline gate (that case reports `stale_non_targetable` instead) |
