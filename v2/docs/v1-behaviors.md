...
- Review phase flow is: baseline `bun run ready` (install → check:fix → typecheck → test → check) → N debate cycles, each cycle = three read-only reviewer attempts (adversary → defender → judge, using registered role prompts) + injected executor writing code from the verdict (using patch agent order). Reviewer attempts are read-only on code; code edits are reverted after each reviewer role. Executor is the only writer of code. Spec-tree edits are reverted for all roles including executor. Non-empty changes are committed per role (`review: adversary`, `review: defender`, `review: judge`, `review: executor`) and per executor invocation; empty verdict produces no executor commit. Prior cycle verdicts are passed as context to the next cycle's adversary. Sources: `v1/src/modes/review/run.ts`, `v1/src/modes/patch/review.ts`, `v1/src/modes/patch/prompt.ts`, `v1/docs/run-loop.md`
- Verdict is a durable doc (`verdict-patch.md`) next to the executed spec, overwritten each cycle with the judge's outcome-focused requirements (what must be true and why, not implementation details). Verdict is self-contained (executor reads only the verdict, not prior role artifacts). Empty verdict (no content or whitespace-only) skips executor invocation and produces no executor commit via existing no-change path. Sources: `v1/src/modes/patch/review.ts`, `v1/src/modes/review/run.ts`
- Role-specific prompts are registered under `prompts/patch/review-*.md` (adversary, defender, judge) with self-contained verdict instructions; executor uses the standard patch `patch.prompt.body` template adapted to the verdict as the task. Sources: `prompts/patch/review-adversary.md`, `prompts/patch/review-defender.md`, `prompts/patch/review-judge.md`, `v1/src/modes/patch/prompt.ts`
...
- Quota exhaustion during any review role attempt triggers fallback to the next configured review agent; if all agents are exhausted, the cycle exits `2`. `model_config` exits `3`. Executor runs with patch agent order and also respects quota fallback. Sources: `v1/src/modes/review/run.ts`, `v1/src/modes/patch/review.ts`
- Telemetry records review pass invocations with `patch_phase: "review"` and role-specific iteration counts so they are distinguishable from implementation iterations and role progression is traceable in `~/.jarvis/runs.jsonl`. Executor invocations are recorded with agent name from the patch agent order. The run summary shows review attempts separately from implementation attempts while preserving correct totals. Sources: `v1/src/telemetry.ts`, `v1/src/modes/patch/review.ts`, `v1/src/run-summary.ts`
...
- Review behavior snapshots current spec markdown into prompt context, forbids `intent.md` rewrites except blocker append, validates `index.md` still exists, and can skip committing when no changes were produced. Plan review routes through the shared review runner in `v1/src/modes/review/` with a plan adapter; review agents resolve from `modes.review.agentOrder ?? modes.plan.agentOrder` and cycle counts from `resolveReviewPasses(cfg, --review-passes)` (`modes.review.passes`, default `1`, where each cycle runs adversary → defender → judge → executor). Plan reviewers (adversary/defender/judge) are read-only on the spec; any spec edits during reviewer roles are reverted by the harness. Per-role artifacts are stored as temporary files and injected into successor roles (e.g., adversary findings → defender, defender response → judge). The judge's verdict drives one refine-loop invocation (the verdict executor), which applies the verdict as a spec refinement task under the existing plan write boundary. Verdict is persisted as `verdict-plan.md` in the spec folder, overwritten each cycle. Per-role commits are emitted with messages `review: <role>` for reviewer roles and `review: executor` for the executor. Empty verdicts skip executor invocation and do not create an executor commit. Sources: `v1/src/modes/plan/review.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/review/run.ts`, `prompts/plan/review-*.md`, `v1/src/commands/plan.ts`, `v1/src/config.ts`
