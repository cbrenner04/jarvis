...
1. **Deterministic header** generated by jarvis from `index.md`: the H1, a
   `## Progress` line of the form `N of M subspecs complete` (counting only
   `.md` linked subspecs), and a `## Subspecs` checklist that mirrors the
   index's linked-subspec lines verbatim including their `[ ]` / `[x]`
   state.
...
- When that rename succeeds, cleanup creates a commit in the project root that stages only the moved spec paths, so unrelated modified or untracked files stay out of the cleanup commit.
