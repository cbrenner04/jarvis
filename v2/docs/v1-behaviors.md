...
- Review phase flow is: baseline `bun run ready` (install → check:fix → typecheck → test → check) → N shared-runner review passes (agent critique/rewrite, spec-tree edits reverted including untracked additions, non-empty changes committed per pass) → final `bun run ready` → `gh pr ready`. Sources: `v1/src/modes/patch/review.ts`, `v1/docs/run-loop.md`
...
- Quota exhaustion during any review pass triggers fallback to the next configured review agent; if all are exhausted, exit `2`. `model_config` exits `3`. Sources: `v1/src/modes/review/run.ts`, `v1/src/modes/patch/review.ts`
...
- Review behavior snapshots current spec markdown into prompt context, forbids `intent.md` rewrites except blocker append, validates `index.md` still exists, and can skip committing a pass when no changes were produced. Plan review routes through the shared review runner in `v1/src/modes/review/` with a plan adapter; review agents resolve from `modes.review.agentOrder ?? modes.plan.agentOrder` and pass counts from `resolveReviewPasses(cfg, --review-passes)` (`modes.review.passes`, default `2`). Sources: `v1/src/modes/plan/review.ts`, `v1/src/modes/review/run.ts`, `v1/src/modes/plan/prompts/review.md`, `v1/src/commands/plan.ts`, `v1/src/config.ts`
