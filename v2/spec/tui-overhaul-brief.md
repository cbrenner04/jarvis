...
Replaces the 2026-07-27 brief. **Status 2026-08-07: slices 1-5 shipped; slice 6 (steering + log) is seeded and 3/5 implemented — waitState removal + right-pane windowing (#2691), unattributed segment FIFO + label (#2693), and pipeline steering approve/reject/resume as dock verbs [4 subspecs] (#2698). Remaining: run-steering (kill/pause/resume-run — blocked: its single subspec timed out and needs splitting) and log-follow; slice 6 is marked shipped when log-follow merges.** Slice-6 mutation checkpoints no longer need single-line ACs (multiline `test.each` titles resolve since `mutation-checkpoint-pin-resolution` #2696); claude-first review works (`--include-partial-messages`, #2609). TUI test strategy is settled for the whole phase — rendered-ink assertions are unsupported because CI cannot observe them; prove layout with pure functions, keybindings through the injected input hook, and behavior through production monitor state ([test-writing.md § TUI test strategy](../docs/test-writing.md#tui-test-strategy)). Goal is a **Jarvis command center**: one terminal app to start pipelines in any registered project, monitor them, steer gates and runs, and read enough state to decide without cross-checking CLI output.
...
Slice 5 landed seven pieces: a pure typed command parser (`tui-command-parser.ts`, #2529), a reusable `pipeline_start` admission API the CLI now goes through too (`pipeline-start-admission.ts`, #2530), the four dock rows as a pure function over monitor state (`monitorDockLines`, #2531), the painted dock plus its session state (#2533), command focus and grapheme-cursor editing driven through the injected input hook (#2545), and dispatch — a submitted `start` reaches `admitPipelineStart` on the same seams as `jarvis pipeline start`, detached, plus `expand`/`collapse` and parse-error feedback (#2554), and the status row projecting retained RPC and command feedback together (#2575).

Subspec 02 also carried the operator-runbook and parity documentation, folded in by #2573 after the two documentation-only subspecs were retired as over-split. Slice-6 progress (2026-08-07): `waitState` removed + right-pane detail windowed to selectable runs (#2691); unattributed segment FIFO + `─ Unattributed (N) ─` label (#2693); pipeline `approve`/`reject`/`resume` are live dock verbs (#2698). Still missing: run `kill`/`pause`/`resume-run` (run-steering — blocked, subspec needs splitting) and `log` follow (still `recognized_unavailable`); both are the remaining slice-6 specs.
...
| 6 | **Steering + log**  | Approve/reject/resume; run pause/kill; log follow; unattributed segment; waitState removal + detail windowing | **partial 3/5** — waitState removal + detail windowing (#2691), unattributed FIFO+label (#2693), pipeline steering (#2698) shipped; run-steering (blocked, split needed) + log-follow remain |
