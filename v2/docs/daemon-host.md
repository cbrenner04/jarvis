...
`budget-soft-stopped`). For `failed` / `blocked`, a terminal `loop_finished` reporting a resumable
loop-level finalization outcome (`ready_gate_failed`, `surviving_mutation_failed`,
`completion_commit_failed`, `iteration_commit_failed`) outranks last-attempt store detail — e.g.
`runStatus: "failed"` + last attempt `outcome_kind: "blocked"` + `loopOutcomeKind: "ready_gate_failed"`
composes `ready_gate_failed` / `resume`, not `agent_blocked`. For any other `loopOutcomeKind`,
last-attempt store detail still wins over conflicting `loop_finished` (e.g. `runStatus: "failed"` +
`loopOutcomeKind: "complete"` resolves from attempt detail). The stale-log guard is scoped to the
durable-status resumable kinds (`paused`, `budget-exhausted`, `killed`): when `runStatus` is `failed`
or `blocked` with no mappable attempt detail, those `loopOutcomeKind` values from stale logs do not
win — operators see a non-resumable stop (typically `harness_failure`). Spawn-boundary failure on resume can demote
...
operator-action signal on the error contract. `wait.resumable` (and `list`, plus
the workflow entry projection) is `true` only when resume is not terminal-blocked
and shared snapshot reconstruction succeeds (`resumeContext.ok`), not echoed from
the persisted `loop_finished.resumable` self-report — advertised `resumable`
and a successful `resume` RPC agree by construction. Rows that compose
`nextAction: "resume"` but fail reconstruction surface
`unsupported_resume_context` with `resumable: false` and `resume_unsupported` on
the RPC. The raw log field survives only in `jarvis run log`'s replay of the
settle-time self-report. `resumable` may be absent on store-only quiescent
resolves (e.g. `killed` without persisted loop fields).
