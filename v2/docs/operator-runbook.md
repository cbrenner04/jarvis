...
**A second daemon on the same key is refused, not swapped in.** `startIpcServer` probes the socket path before binding; when a live daemon answers (or is too busy to answer within 250ms) the new process exits non-zero with `A daemon is already listening on <path>`. That refusal is the guard working — the running daemon keeps serving. Only a genuinely stale entry (`ECONNREFUSED`/`ENOENT`) is removed and rebound. Before this check, every `daemon start` — including the auto-start any `jarvis` command performs when it cannot reach a daemon — unlinked the incumbent's socket first, which stranded every run on the machine behind a `connect ENOENT` while the daemon itself stayed alive and healthy.
