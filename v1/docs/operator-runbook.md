...
- **Merging anything to `main` during a long in-flight run leaves that run behind base.** When the run completes, criteria are done but the ready-flip blocks (`branch <b> does not contain base main; PR stays draft`) — you then hand-run [Integration-merge-then-retest](#integration-merge-then-retest-pattern). (Observed 2026-06-28: both Tier A impl runs blocked this way after sibling PRs merged mid-run; clean conflict-free integrations.) Either batch `main` merges for when no run is in flight, or expect the behind-base finalize. Structural fix (auto-integrate base at final ready) seeded as `run-auto-integrate-base-on-behind` — delete this caveat once it ships.
