...
- [ ] `v2/src/execution/write-loop.test.ts` — `settles ready_gate_command_missing without autofix or repair when the gate command is absent`; Keystone checkpoint: the test body carries a `// @mutate` directive reverting the missing-command short-circuit in `publishWithReadyRepair` so pre-fix autofix/repair dispatch returns, and the test turns RED when applied.
- [ ] `v2/src/execution/write-loop.test.ts` — `settles ready_gate_command_missing without autofix or repair when the gate command is absent`; Mutation checkpoint: the test body carries a `// @mutate` directive on the missing-command classification guard and turns RED when the guard is inverted without a production inversion hook.
...
- [ ] `v2/src/execution/write-loop.test.ts` test `returns retryable ready_gate_failed when the gate fails and does not call the flip` stays green (ordinary red gates still enter bounded repair).
