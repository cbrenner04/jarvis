# Plan and intent write steps lint staged Markdown before finalization

- [ ] [00 - Staged-path markdownlint runner](./00-staged-markdown-lint-runner.md)
- [ ] [01 - Plan write step lints staged Markdown before finalize](./01-plan-write-step-lint.md)
- [ ] [02 - Intent write step lints staged Markdown before finalize](./02-intent-write-step-lint.md)
