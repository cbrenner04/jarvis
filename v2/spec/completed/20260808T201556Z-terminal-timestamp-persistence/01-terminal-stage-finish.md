...
- [x] `v2/src/persistence/state-store.test.ts` — `updateStage leaves endedAt null on a non-terminal status write` asserts `awaiting` and `running` writes persist `endedAt` as null, proving the stamp is suppressed off the terminal set.
- [x] `v2/src/persistence/state-store.test.ts` — `updateStage preserves a caller-supplied endedAt on a terminal status write` asserts the caller's millisecond value survives the write unchanged.
