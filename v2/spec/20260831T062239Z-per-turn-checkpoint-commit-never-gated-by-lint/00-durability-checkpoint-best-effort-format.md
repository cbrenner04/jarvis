...
- [ ] `v2/src/execution/write-loop.test.ts` test `checkpoint durability uses best-effort biome format not completion check` proves the checkpoint uses best-effort formatting, not the terminal completion's fail-closed check: swapping the checkpoint's format call to the strict `runCompletionFormat` path turns this test red (the lint-dirty checkpoint would then fail to commit).
