...
- [ ] A changed line in a `*.test.ts` file produces no mutation candidate; `diff-derived-mutation-verifier.test.ts` fails against pre-fix code.
...
- [ ] Inverting `isProductionFile` in `diff-scan.ts` fails a test in `diff-derived-mutation-verifier.test.ts`.
