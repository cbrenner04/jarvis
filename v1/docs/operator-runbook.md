...
1. Implementation PRs: spec complete (all criteria ticked) → Jarvis flips the PR to `ready` (or run `gh pr ready`). Plan PRs (`plan/*` head branch): subspec acceptance criteria may remain unchecked — `jarvis1 triage` is the merge path.
2. `jarvis1 triage <spec-path> --merge` (or `jarvis1 triage <pr-ref> --merge`, or `jarvis1 triage <worktree-name> --merge`) — atomically runs the local `bun run ready` gate, marks the PR ready if draft, waits for CI to be green, then admin-squash-merges. Supported for implementation worktrees, plan worktrees (`.worktree/plan-<name>`), and plan PR references when the resolved head branch is `plan/<name>`. Refuses to merge on gate-red or any CI-check red; reports the specific failing gate/check name and exits non-zero, leaving the PR unmerged. `--mark-ready` still requires spec completeness for all branch kinds; only `--merge` skips AC completion for `plan/*`. Works for both marker-bearing and markerless worktrees: when `.active-spec-path` is absent the spec is derived from the branch name. Merge lands the spec PR only — no `jarvis1 run` or implementation worktree creation.
