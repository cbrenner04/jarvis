...
Counts after the 2026-09-20 closeout: 1 open spec dir and 6 queued ready-intents; of those 6, 1 is evidence-gated, 1 needs a rewrite before planning, and 4 are live. 30 seeds. Landed: ledger § Landed.
...
| **P1** | Chain D tail: [[mutation-repair-commits-pushed-before-settlement]] (dispatchable, unblocked by #4102) → [[repair-exhausted-error-names-site-and-killing-set]]; then [[non-terminating-mutation-settlement-names-its-site]] (one-line spread, same sites) | `mutation_repair_exhausted` is terminal and unpushed repair commits left green CI describing a tree that no longer existed |
...
| **P1** | [[spawned-cli-tests-inherit-a-five-second-connect-bound]] | `CONNECT_TIMEOUT_MS = 5_000` in `v2/src/ipc/client.ts:11` bounds spawned-CLI tests under load; misread as a test-timeout problem |
| **P1** | Resume honesty residue: [[pipeline-resume-resumes-a-resumable-implement-row]]; [[resume-surfaces-admission-gate-refusal]]; [[interrupted-pipeline-stage-cannot-be-resumed]] (#2996) | A stage never reflects a lane recovered by `run resume`; admission refusals reach only the daemon log; a killed stage is a dead end |
| **P1** | Retirement safety: [[stale-reset-destroys-commits-for-external-specs]] (gate 2 blind to external trees); [[implement-retirement-destroys-artifacts-before-materialization]]; [[abandon-refuses-unlanded-work-with-no-pr]] | The blind gate, the `--base <own-branch>` collision, and `--abandon` on unpublished work |
| **P1** | Cleanup and home: [[retention-tiers-for-session-logs-and-telemetry]] (sequence after #4086); [[worktree-materialization-fails-on-committed-node-modules-symlink]] (#4003, half landed); [[daemon-status-reports-stopped-on-a-busy-daemon]] | Orphan telemetry lives forever; a `stopped` probe on a busy machine is a destroying guard acting on an inconclusive answer |
...
- **A seed PR is not a fix.** The brief once marked #3833 and #3832 landed by citing the seed PRs themselves; #3833 has since landed for real (#4065), and [[daemon-status-reports-stopped-on-a-busy-daemon]] is still open with its classifier renamed (`classifyLegacyDaemonUnit` / `probeSocketLiveness`).
- **A ready-intent premise that the source refutes:** `workflow-terminal-waits-await-durable-boundary` blamed a 5 s test deadline; `bunfig.toml` already sets `[test] timeout = 30000` and `workflow.test.ts` contains no `5000`. Plan PR #4105 was rejected on the premise. The real bound is `CONNECT_TIMEOUT_MS = 5_000` in `v2/src/ipc/client.ts:11`, now carried by [[spawned-cli-tests-inherit-a-five-second-connect-bound]] The ready-intent is retired.
...
- **Issues closed:** #3949, #3974, #3372 (2026-09-18); #3040 (2026-09-20, #4085). #3423 (#4090), #3417 (#4076).
