...
- Patch/run already honor `git: false` (loop-only, no worktree/branch); plan does not.
- The no-commit external-spec boundary check and post-failure git cleanup calls live in `v1/src/commands/plan.ts`.
- This is an unvalidated bug report — the first implementation step is reproduce + confirm direction (loop-only vs reject-fast).
