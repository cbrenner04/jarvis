...
The split has enough information to catch itself: an intent whose prerequisite no sibling intent in the same split delivers is a detectable inconsistency at split time. (v2 has no `siblings` project key; that was v1-only. A cross-repo registry concept is a separate decision, so this seed is scoped to the split-internal consistency check.)

## Decisions

- The intent split checks each emitted prerequisite against the split's own output: a prerequisite delivered by no sibling intent and not declared already true either produces an intent covering it or fails the split loudly; rules out the silent-drop `completed`.
...
- [ ] A test proves a split whose emitted intents name a prerequisite that no sibling intent delivers fails (or emits the covering intent) rather than settling `completed`; fails against the current silent drop.
