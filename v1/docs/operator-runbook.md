...
- **False multi-subspec completion.** Run exits `0` (`criteria-complete`) with `iterations: 0` and no `spec complete` on stdout after uncommitted ticks on a completed subspec while the index still has unchecked linked subspecs — re-run `jarvis1 run <spec-path>`; the harness should commit subspec 00 and continue to subspec 01.
