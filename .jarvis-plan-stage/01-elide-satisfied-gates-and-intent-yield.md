...
- Any stage whose artifact carries a non-empty `downstreamInputs` array appends ` → N intents` to its row label; the intent stage is the one that has it. Rules out keying the suffix on `stageId === "intent"`, which breaks the moment a definition renames or moves the split stage.
...
- `v2/src/tui/tui-monitor-pipeline-tree.ts`:
  ...
  - Add `function intentYieldSuffix(artifact: unknown): string` that resolves `downstreamInputs` to an array (empty when absent or not an array), then `if (inputs.length === 0) return "";` (guard-mutation anchor) followed by the ` → ${inputs.length} intents` return.
...
- [ ] `v2/src/tui/tui-monitor-pipeline-tree.test.ts` — `a post-split workflow stage settled skipped still renders under its branch` asserts a `skipped` workflow stage under a fan-out branch keeps its row; it fails against the pre-fix code, which has no stage-kind resolution at all.
...
- [ ] `v2/src/tui/tui-monitor-pipeline-tree.test.ts` — `a skipped gate row is absent from a fanned-out branch subtree` asserts a branch's `skipped` gate node id is absent from the flattened tree; it fails against the pre-fix code, which renders it.
...
- [ ] `v2/src/tui/tui-monitor-pipeline-tree.test.ts` — `the intent stage row appends the split yield only when the artifact lists downstream inputs`; Mutation checkpoint: its pinning test carries `// @mutate v2/src/tui/tui-monitor-pipeline-tree.ts "if (inputs.length === 0) return \"\";" -> "if (inputs.length < 0) return \"\";"` inside the test body — suffixing every stage, so an artifact-less stage paints ` → 0 intents` — and the mutation turns that regression RED.
- [ ] A snapshot whose `name` matches no registry definition renders every gate-shaped row unchanged: the existing `v2/src/tui/tui-monitor-pipeline-tree.test.ts`, `v2/src/tui/tui-monitor-lines.test.ts`, and `v2/src/tui/tui-entry.test.tsx` suites — whose pipeline fixtures all carry unregistered names — stay green, including the `not_awaiting_stage` approve/reject assertions.
- [ ] An elided gate's record still reaches the detail pane: the right-pane `Stages` roll-up for a `full-review` pipeline lists the `approved` gate that the tree no longer paints (asserted in `v2/src/tui/tui-monitor-lines.test.ts`).
- [ ] `v2/docs/operator-runbook.md` § Observe records that gate rows appear only while `awaiting` or `rejected`, that decided and unreached gates leave the tree with their records still in the detail pane, and that the intent row shows `→ N intents` when the stage split.
- [ ] `v2/docs/v1-behaviors.md` § TUI / observability pipeline-tree bullet records stage kind resolved from the registry definition by `snapshot.name` (unregistered names elide nothing), the gate render rule, the `→ N intents` label suffix, and that `approve` / `reject` on a non-`awaiting` gate of a registry-named pipeline now reports `stale_non_targetable`.
- [ ] `v2/spec/tui-command-center-brief.md` seed table row 3 records `tui-intent-branch-subtree` as shipped by this spec directory rather than `seeded`.
