...
Every work-tree row (pipeline, branch, stage, run, ad-hoc) composes as `indent · marker · fill label · right-aligned cluster`: two columns of indent per depth, a one-glyph marker, a label padded or ellipsized to fill the remaining width, and a right-aligned cluster of status/liveness/elapsed atoms. The marker is `▼` when a structural node (pipeline, branch, stage with children) is expanded, `▶` when collapsed, and blank for leaves (runs, ad-hoc rows) — `e` flips `▼`/`▶` on the selected structural node and is a no-op on a leaf's blank marker. A pipeline row's cluster can carry `✋<n>` (awaiting-gate count) and `✗<n>` (failed count) from that pipeline's own records. The selected row paints inverse across its complete padded width — indent, marker, label fill, and every cluster atom — never a `>` caret or a label-only highlight; status and liveness tones survive under inverse. Below a row's supported-width floor, or when the cluster does not fit, cluster atoms drop right-to-left (most-droppable first) down to the never-dropped compact status; if even that does not fit, the row falls back to one clipped, unpadded line.
