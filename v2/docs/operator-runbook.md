...
| Command | Use |
| --- | --- |
| `jarvis tui` | Split-pane run monitor (stacked below 120 cols): left pane is one unified work tree — pipelines (pipeline → stage → run) and ad-hoc `run workflow` invocations as top-level rows, ordered together — plus a queue block; a fanned-out pipeline's subtree groups by branch: pre-split stages render inline under the pipeline, then one branch node per fan-out `branchKey` followed by that branch's post-split stages; branch labels strip the `-`-segment prefix shared by every sibling branch key (capped so each keeps at least one segment) — a lone branch or one whose siblings share no leading segment keeps its full key, always shown unstripped in the detail pane; a branch node summarizes as its first stage not yet `succeeded`/`approved`/`skipped`, or its last stage once every stage settles; post-split `default` placeholder rows (superseded once branches admit) never render as tree rows, though their records stay visible in the pipeline's Stages roll-up; for a pipeline whose `name` matches a registry pipeline definition, an approval-gate stage row renders only while its record is `awaiting` or `rejected` — `pending`, `approved`, and `skipped` gates leave the tree while their records stay in the Stages roll-up; a pipeline whose `name` matches no registry definition elides no gate rows; the intent stage's row appends a `→ N intents` suffix when its record's artifact lists downstream inputs; **`e`**/↓/`expand`/`collapse` reach a branch node like a stage, and reveal-on-select stops at the branch without expanding sibling branches; right pane shows pipeline context and stage roll-up for a pipeline, then the selected durable-stage record for a stage (keyed by stage id and branch key, so two branches sharing a `stageId` show distinct records), then pipeline context plus the full branch key for a branch node, then selected durable-run workflow/outcome/error/PR/worktree detail for a run selectable in the left pane (`monitorSelectableNodeIds`); an ad-hoc row shows only its selected durable-run detail, with no pipeline context. The right pane's detail rows are grouped into blank-line-separated sections — `Pipeline`, `Stages`, `Stage`/`Branch`/`Run`, `Workflow`, and, when retained, the steering-feedback section — each preceded by a single blank row except the first; a section with no rows to show (a stage-less pipeline's `Stages`, a workflow-less run's `Workflow`) paints neither its heading nor a blank row. Within a section, a field whose value is `null`, `undefined`, or `""` paints no row at all; `false`, `0`, `[]`, and `{}` still paint their row. Retained steering feedback follows selected-run detail. Rows losslessly hard-wrap by display columns—split right-pane width or stacked terminal width, floored at one—with no ellipsis and preserved tones; extended grapheme clusters stay atomic, so a wider grapheme can overflow a narrower row. The 4-line dock supports `[`/`]` divider nudges; **`j`** or ↓/↑ walk every row in the work tree (queue display-only); **`e`** expands/collapses a selected pipeline, stage, or branch node and is a no-op on an ad-hoc row; kill (`k`) on live runs. Typed steering (`pause`/`kill`/`resume`/`approve`/`reject`) refuses an ad-hoc row selection with the `unattributed` code, a branch node selection with `not_awaiting_stage` (approve/reject) or `not_pipeline` (resume). `:` or `/` focuses command input. **`elapsed`** shows pipeline age from `createdAt` through `finishedAtMs` (or live clock), stage age from `startedAt` through `endedAt` (or live clock), and run age from `createdAt` through `finishedAtMs` (collapsed workflow rows use the representative run's timestamps); elapsed freezes only when the recorded end timestamp is present and keeps advancing on the local display tick when status is terminal but the end timestamp is absent |
