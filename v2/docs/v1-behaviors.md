...
- Role-specific prompts are registered under `prompts/patch/review-*.md` (adversary, advocate, adjudicator) with self-contained verdict instructions; actuator uses the standard patch `patch.prompt.body` template adapted to the verdict as the task. Sources: `prompts/patch/review-adversary.md`, `prompts/patch/review-advocate.md`, `prompts/patch/review-adjudicator.md`, `v1/src/modes/patch/prompt.ts`
...
- Shrink flow: pre-shrink `bun run ready` gate (failure logs warning, skips shrink) → one agent invocation with `patch.prompt.shrink` + `global.terse` (not `patch.rules`) → post-invocation spec-tree revert, out-of-scope revert, contract validation (`bun test`, no deleted scoped `*.test.ts`, no AC checkbox regression) → single `shrink:` commit with attribution and PR footer refresh, or discard-all on miss/failure without elevating run exit code. Sources: `v1/src/modes/patch/shrink.ts`, `prompts/patch/shrink.md`
- Shrink scope is the accumulated set of non-spec paths touched during implementation iterations; edits outside that set are reverted post-invocation. Prompt carries run-scoped diff (allowlisted files only) and explicit allowlist. Sources: `v1/src/modes/patch/shrink.ts`, `v1/src/modes/patch/prompt.ts`
...
- Fresh plan input is copied byte-for-byte to the generated spec directory as `intent.md`; the source ready-intent is left untouched. Plan derives `<plan-name>` from the ready-intent frontmatter `name:`. Invalid `name:` or missing `## Prerequisites` fail before creating any branch/worktree/spec output. Sources: `v1/src/commands/plan.ts`, `v1/docs/plan-mode.md`
...
- Draft validation order: blocker (if any) → index.md existence → numbered subspecs presence. Blocker check runs first so prerequisite gate failures short-circuit the subsequent index-existence and subspec-presence checks within `validateDraftOutput`. Blocker indicates failed prerequisites, missing intent details, or other unrecoverable plan conditions; when present, plan exits `1` without proceeding to index/subspec validation. Sources: `v1/src/modes/plan/draft.ts`
...
- Resume invocations track prior plan commit history to continue numbering (`plan: review <N>`) and append `r<n>` resume suffixes to resumed review/blocker commit subjects for later attribution and chronology. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/commits.ts`
...
- Patch-mode draft PR creation invokes the agent to generate the PR narrative with a short description followed by a `Decisions:` section, placed inside `<!-- jarvis:narrative:start/end -->` markers; generation failures fall back to `Auto-generated by jarvis`. Sources: `v1/src/modes/patch/pr.ts`, `v1/src/modes/patch/run.ts`
- Patch mode rewrites existing PR bodies via `gh pr edit <branch> --body-file -` preserving any human-edited narrative inside the markers and regenerating the narrative when empty and an agent is available; header and attribution footer are regenerated each rewrite. Sources: `v1/src/pr.ts`, `v1/src/modes/patch/pr.ts`, `v1/src/modes/patch/run.ts`
...
- Root package.json exposes runnable test commands: `test` (aggregate, bare `bun test`), `test:v1` (`bun test ./v1/`), `test:v2` (`bun test ./v2/`), and `test:shared` (`bun test ./shared/`). Each scoped slice targets its exact owner root with a trailing slash to prevent fuzzy substring filtering. Sources: `package.json`, `bunfig.toml`, `test/test-slices.test.ts`
