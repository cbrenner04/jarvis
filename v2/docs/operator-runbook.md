...
**First hollow from linker miss: criterion omits the enclosing `test()` title (2026-08-06).** Blocker: `no @mutate directive linked to this criterion; add // @mutate … on the named pin` — even when a directive exists but the criterion omits the enclosing `test()` title (`criterionText.includes(directive.pinTitle)`, case-sensitive, no fallback). Fix the criterion (include the title verbatim or as a linker-matching substring), not the directive; not proof-form or directive-syntax. Plan-draft now hard-rejects the same omission when the pinning file resolves on disk: `artifact.exists` `contract_miss` with `criterion:` / `reference:` / `reason: missing_enclosing_test_title` or `reason: unresolvable_enclosing_test_title` before implement runs. A **second** hollow on a different guard is the opposite case — see the premise-smell bullet below: do **not** amend the criterion.
