...
- Before returning `resumed`/`admitted`, `pipeline resume` / `pipeline recover` evaluate the same pre-dispatch refusals the continuation would hit: dirty worktree, lane not descended from / behind base, landed-criteria drift, unresolved `## Blocker`, worktree claim, never-landed classification.
...
- [ ] `pipeline recover` refused by an analogous gate returns the reason to the CLI with non-zero exit.
