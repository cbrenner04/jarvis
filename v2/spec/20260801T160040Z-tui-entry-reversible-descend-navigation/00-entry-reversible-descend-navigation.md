...
- [x] `tui-entry.test.tsx` — overflow fixture: forward `j` walk from first selectable through list boundary until `selectedNodeId` unchanged (step cap), then `k` back — backward visit order is the exact reverse of forward; fails pre-fix against slice 01 merged. **Amended at merge (operator):** this pin is a true regression guard but it already passed against slice 01 (`8a9ecc78`); the "fails pre-fix" claim is false.
- [x] `tui-entry.test.tsx` — first pipeline row in the initial painted tree slice: `j` selects its first child, not `ids[0]` via fallthrough; fails pre-fix against slice 01 merged. **Amended at merge (operator):** this pin is a true regression guard but it already passed against slice 01 (`8a9ecc78`); the "fails pre-fix" claim is false.
- [x] `tui-entry.test.tsx` — after each `selectNextRun` or `selectPreviousRun`, `selectedNodeId` is in `monitorSelectableNodeIds`; fails pre-fix against slice 01 merged; membership invert covered by fallthrough mutation checkpoints. **Amended at merge (operator):** this pin is a true regression guard but it already passed against slice 01 (`8a9ecc78`); the "fails pre-fix" claim is false.
...
- [ ] `tui-entry.test.tsx` — reintroducing `ids[0]` fallthrough when `indexOf` is `-1` in `selectNextRun`/`selectPreviousRun` turns the reversible-walk pin RED; `Mutation checkpoint:` names that fallthrough. (Manual) **Un-ticked at merge (operator):** reintroducing both fallthroughs leaves the tui suite at 226 pass / 0 fail. After slices 00 and 01, `indexOf` is never `-1` in any reachable state — descend-expand keeps the node in the list — so the guard is unreachable code and the checkpoint cannot be observed. Follow-up: `seeds/tui-selection-fallthrough-guard-is-unreachable`.
- [ ] `tui-entry.test.tsx` — reintroducing `ids[0]` (and backward fallthrough) in `selectNextRun`/`selectPreviousRun` turns the first-painted-pipeline descend pin RED; `Mutation checkpoint:` names that fallthrough. (Manual) **Un-ticked at merge (operator):** same unreachable guard as the criterion above; the mutation turns nothing red.
