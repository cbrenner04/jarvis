...
- [ ] A resolved `codex` binding invokes `codex exec --color never --sandbox workspace-write -c approval_policy="on-request" --model <adapterModel>` when a model is present, pipes the augmented prompt on stdin, uses caller `cwd`, and honors abort signals.
