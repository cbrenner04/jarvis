...
- Gate runs only for a claim blocker (`detectBlockerClaim`) that base-ref validation did **not** already reject, i.e. immediately before the "blocker stands" commit + exit 7. Rules out: a second gate on non-claim blockers (most blockers are real); re-running snapshot churn for a blocker base-ref already cleared (wasted full re-test).
...
- [ ] In `runIteration`, after base-ref validation declines to reject a claim blocker (and before the blocker-stands commit), invoke the seam when present, git is enabled, and the shared rejection bound is not hit.
