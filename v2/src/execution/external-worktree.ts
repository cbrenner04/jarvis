...
/** Raised when a fresh managed worktree cannot be created and validated. */
...
async function ensureExternalWorktree(
  ...
  throwIfAborted(signal);
  try {
    if (existsSync(worktreePath)) throw new Error(`existing path is not a git worktree: ${worktreePath}`);
