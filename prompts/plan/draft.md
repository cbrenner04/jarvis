...
- **One artifact per bullet.** A bullet under `## Acceptance criteria`, `## Decisions`, or `## Documentation updates` may name at most one backticked repo-relative artifact path claimed as built or changed. Split work on multiple files into one bullet per file; prose bullets naming no artifact are allowed. Two exemptions read from wording alone, and both fail closed: a bullet naming several artifacts that stay unchanged (preservation wording like "stays green" or "remains"), and a bullet stating one outcome holds identically across several artifacts (marker: "identical" or "the same"). Neither applies to a bullet that also makes a build claim — any build verb anywhere in it ("adds", "creates", "gains", "emits", …) means the bullet is counted, so write preserved and built artifacts as separate bullets. Four further mentions never count as a second artifact: a path named inside a `rules out` clause; in `## Acceptance criteria`, a test path paired with the production path it covers (same directory, `.test` removed); in `## Documentation updates`, any non-markdown path after the bullet's leading path; and a bare extension or naming-convention suffix (`.test-support.ts`, starting with `.` and a second `.` later). A draft with offending bullets across more than one subspec file is refused once, naming every offender in the tree in a single error.
- **Gate acceptance criteria name the scoped test script, never the bare aggregate.** When a subspec's closing criteria cover the mechanical gate, name the script(s) matching the surfaces that subspec touches — for this repo `bun run test:v2` / `bun run test:integration:v2` for `v2/**`, and those plus the `test:shared` pair for `shared/**` — following target-repo `AGENTS.md`. Write `- [ ] bun run test` only when the scope rule genuinely resolves to the full aggregate. The aggregate runs every slice and takes minutes; a subspec whose criteria demand it per iteration exhausts the implement budget and times out with the work already correct.
