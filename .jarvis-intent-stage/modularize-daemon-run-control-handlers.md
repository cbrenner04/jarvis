...
- Delete `activeRunsByHandler`; tests construct context directly.
...
- [ ] `activeRunsByHandler` is absent from `v2/src/daemon/`; a structural guard or export test fails if reintroduced.
- [ ] Each handler-group module has co-located direct tests; existing daemon integration tests stay green when re-pointed.
- [ ] Recorded daemon test inventory (count + titles) matches the pre-split baseline.
