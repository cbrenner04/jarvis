...
`check:fix`, which leaves `noImplicitAny` / `noExplicitAny` / unused-var / non-null-assertion
fixes that only `check:fix:unsafe` (or a hand edit) resolves. The later `check` step then fails
red on those residuals, the gate blocks a green spec, and the operator hand-finalizes. This bit E,
#9, #10's seed — and recurred this session on the snapshot-gate and patch-rules runs.
...
- A standalone `finalize` command or any new human step.
...
- The ready gate runs safe `check:fix` then `check`; `check:fix:unsafe` exists in `package.json`.
- `v1/src/ready-gate.ts` already commits `check:fix` output and re-checks the worktree.
