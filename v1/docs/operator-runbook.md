...
- **`bun run ready`** — the full completion gate (typecheck + lint + tests, with a serial retry on parallel-test failure). Jarvis runs it on spec completion; the operator runs it before any hand/admin-merge. `jarvis1 triage <spec-path> --merge` (or PR ref / worktree name) runs this gate as the first step before waiting for CI, so both gates are enforced together.
