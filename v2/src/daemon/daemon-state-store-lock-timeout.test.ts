...
test(
  "list and wait report state_store_lock_timeout after busy_timeout past a committed write boundary",
  async () => {
    ...
    const boundaryLog = logReader
      .tail(runId)
      .find((record) => record.event.kind === "boundary_committed");
