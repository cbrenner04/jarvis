...
- [ ] Pressing `e` twice on a selected stage through the injected input hook returns left-pane tree row ids to their starting value with a distinct intermediate list; `tui-entry.test.tsx` `e on a selected stage returns left-pane tree row ids to their starting value after two presses` fails pre-fix when reveal-on-select still self-expands the selection.
...
- [ ] With more pipelines than fit a small terminal, every id from `monitorSelectableNodeIds(currentState)` appears in left-pane tree row ids for the same terminal size; `tui-entry.test.tsx` `aligns selectable node ids with left-pane tree rows for the measured terminal size` fails pre-fix.
- [ ] `bun run typecheck` and `bun run test:v2` pass.

## Documentation updates

- `v2/docs/operator-runbook.md` — `jarvis tui` row: replace the self-expand caveat with real `e` and `j`/↓/↑ row-navigation behavior.
- `v2/docs/v1-behaviors.md` — TUI pipeline-tree `e` and row-navigation parity entry.
