...
describe("plan ready-intent deletion on commit", () => {
  test("commit: true run deletes source ready-intent from worktree", async () => {
    const dir = mkdtempSync(join(tmpdir(), "jarvis-delete-ready-intent-"));
    const cfgDir = join(dir, "cfg");
    ...
    try {
      ...
      // Set up fake remote
      const remoteDir = mkdtempSync(join(tmpdir(), "jarvis-remote-delete-"));
      try {
        ...
        // Create ready-intent
        const readyIntentPath = writeReadyIntent(projectRoot, "my-feature");

        // Mock agent that generates a spec
        class FakeSpecAgent implements Agent {
          readonly name: AgentName = "claude";
          async run(_prompt: string, opts: AgentRunOptions): Promise<AgentResult> {
            const specDirPath = join(opts.cwd, "spec", "my-feature");
            mkdirSync(specDirPath, { recursive: true });
            writeFileSync(join(specDirPath, "index.md"), "# Spec\n\n- [ ] [00](./00-task.md)\n");
            writeFileSync(join(specDirPath, "00-task.md"), "# Task\n\n## Acceptance criteria\n\n- [ ] done\n");
            return { kind: "ok", stdout: "", stderr: "" };
          }
          attributionLabel(): string {
            return "fake-agent";
        ...
        const cap = captureIo();
        const { client: logClient } = capturingLogClient();
        const code = await planCommand({
          io: cap.io,
          ...
          config: { dir: cfgDir },
          logClient,
          createAgent: () => new FakeSpecAgent(),
          // Use skipGhCheck to avoid worktree creation in test context
        });

        // Note: Full plan command with real worktree creation would be a full
        // integration test. This test structure is preserved for a future full
        // integration test when the test harness supports it.
      } finally {
