...
- **A `Test (v2)` (or other scoped `Test (…)`) CI step can hang with zero step progress** for 15-20+ minutes on GitHub's own runners, unrelated to local machine load — every other run of the same step elsewhere completes in under a minute. Not yet root-caused (see seed `ci-test-v2-stage-hangs`); recovers reliably via `gh run cancel <run-id>` then `gh run rerun <run-id> --failed`. Observed 6 times across 4 PRs in one session (2026-07-05) — don't assume a single stuck run is a fluke; budget for a second or third cancel+rerun before escalating.
