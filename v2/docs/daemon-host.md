...
| `resume` | `{ runId: string }` | `{ ok: true }` | Resumes workflow write runs when shared snapshot reconstruction succeeds and the same admission predicate that projects `list`/`wait` `resumable` (`nextAction: "resume"` on the composed operator error; see [Operator error on list and wait](#operator-error-on-list-and-wait)). Rejected `daemon_superseded` if the daemon is retiring (see [Daemon retirement on supersession](#daemon-retirement-on-supersession)). The matching persisted step must retain non-empty rules, artifact path, agents, model config, and resolvable bindings; the reconstructed input preserves step identity, workflow snapshot, and timeout. Missing or invalid context returns `resume_unsupported` before claim/spawn. Accepted reasons include every composition that yields `nextAction: "resume"` (e.g. `resumable_pause`, `resumable_budget`, `resumable_kill`, `completion_commit_failed`, `ready_gate_failed`, `surviving_mutation_failed`, `landing_failed`, `invalid_token`, `missing_blocker`); compositions yielding `nextAction: "stop"` / `"inspect_spec"` / `"fix_config"` / `"retry_later"` are rejected with `terminal_run` whose message names the owning recovery from `RUN_OPERATOR_ERROR_RECOVERY` (see `run-operator-error.ts`). Ad-hoc stopped runs remain unsupported. When the admitted row is durable and its snapshot step behavior is `review` or `review-debate` with terminal `loopOutcomeKind: "surviving_mutation_failed"`, reconstruction instead branches to a publication-tail respawn: it looks up the workflow's completion (write) step in the shared snapshot, resolves bindings from that step's `stepRules`/`expectedArtifactPath`/`agents`/`agentModelConfig` (preferring its completed hidden `~shrink` row for the recorded completion agent), and re-runs the publication tail — commit any pending worktree changes, then `publishWithReadyRepair` — against the review row's own id, carrying forward the row's recorded `iterationsConsumed` and the daemon's `operatorSessionId`/log sink (so ready-gate-repair telemetry is visible); the write step's agent binding is not re-invoked. PR-body summary derivation mirrors the inline publication tail for an `implement`-role completion step (`specTemplate` + `deriveSpecRunBodySummary`); `creationTitle` falls back to the workflow snapshot's title when the row has none. This reconstruction reads only `role` from the persisted snapshot step, not `landing`/`promptId`/`publishCompletion` (absent from `WorkflowSnapshotStep`), so a non-`implement` completion step or one authored with `publishCompletion: false` is not distinguished from an ordinary implement completion by this path. |
| `wait` | `{ runId: string }` | `{ runStatus, loopOutcomeKind?, iterationsConsumed?, resumable?, error? }` | Long-running one-shot wait for the next invocation boundary. On a workflow entry, whichever durable sibling row owns the rollup `surviving_mutation_failed` — a hidden `~shrink` row or a durable review row alike — supplies outcome fields and error detail (chronologically last terminal record wins among multiple candidates); entry resumability remains tied to the entry row. Unsupported stopped write context returns `error: { reason: "unsupported_resume_context", retryable: false, nextAction: "stop" }` and forces `resumable: false`, even when the historical loop record was resumable — this does **not** apply to a durable review-step row with `loopOutcomeKind: "surviving_mutation_failed"`, whose `resumable` reflects the publication-tail admission described in the `resume` row above, not the write-context fallback. Otherwise behavior is unchanged; optional `error` matches `list` for the same run (see [Operator error on list and wait](#operator-error-on-list-and-wait)). |
...
(`{ input }`) runs, `workflow` for runs started via `{ steps }`, `publication-tail`
for a `resume` on a durable review-step row that settled `surviving_mutation_failed`
(see the `resume` row above). Only a `write-loop` entry carries the
`abortController`/`pauseController` `pause`/`kill` act on. Every step's run — not
only step 0's, as the run row for each step is durably created — gets a
`workflow`-kind entry keyed by that step's `runId`. A `pause`/`kill` naming any of
those run IDs, or a `publication-tail` run id, is rejected `run_not_active`, same
code as an absent/unknown run; real kill/pause plumbing for a running workflow
step or publication-tail resume is deferred to a future consumer.
