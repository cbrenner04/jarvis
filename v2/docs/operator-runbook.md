...
4. Sweep for leaked test orphans before launching heavy work — **by CPU and parentage, not age** (see [Known gotchas](#known-gotchas) — leaked `bun test` children).
...
**A socket that `lsof` shows bound but `stat` says is missing = bound-then-unlinked (2026-09-05).** The signature of a daemon whose socket entry was deleted out from under it: `lsof -p <pid> | grep unix` names the path, while `stat`/`ls`/`os.path.exists` report it absent. The daemon is healthy and serving established connections; every new `connect()` fails `ENOENT`, so `daemon status` reads `stopped` and its runs look orphaned. **This mimics every documented dead-daemon shape whose recovery is `kill -9`, and applying that here destroys live work across every registered project.** Check `lsof` before concluding a daemon is dead. Fixed by [#3468]/[#3469]/[#3471]; the diagnostic stays useful because the symptom is indistinguishable from the deadlock shape by CLI alone.

**To rule out a socket reaper, bind a live canary (2026-09-05).** A listening socket named `daemon-<16hex>.sock` under `~/.jarvis`, left in place for ~90s: if it survives, nothing is periodically reaping that glob and the deletion is tied to daemon startup instead. Cheap, decisive, and it redirected a search that had already wrongly cleared the real culprit twice by code reading alone.

**A second daemon on the same key is refused, not swapped in.** `startIpcServer` distinguishes liveness from occupancy before binding (full bind-path order: [daemon-host.md § Socket path](./daemon-host.md#socket-path)). When a live daemon answers, or a probe timeout classifies the path live after 250ms — with a longer reprobe before bind when only the initial probe timed out — the new process exits non-zero with `A daemon is already listening on <path>`. That refusal is the guard working — the running daemon keeps serving. Pre-bind removal is stale-only: `ECONNREFUSED` unlinks, `ENOENT` removes nothing. That asymmetry matters because a sandboxed caller — any `jarvis` command run from an agent session — gets `ENOENT` for a live socket it cannot resolve, and unlinking on it deletes a healthy daemon's entry. When a socket file still occupies the path but nothing accepts, occupancy-aware reclaim after `listen` `EADDRINUSE` may unlink once after a post-bind reprobe returns stale. Before the live-peer guard, every `daemon start` — including the auto-start any `jarvis` command performs when it cannot reach a daemon — unlinked the incumbent's socket first, which stranded every run on the machine behind a `connect ENOENT` while the daemon itself stayed alive and healthy.

**Wedged `daemon start`.** When socket bind fails after bounded reclaim, `jarvis daemon start` exits with an error naming the socket path, `errno`, and `jarvis cleanup` recovery — run `jarvis cleanup` to reap dead socket files under `~/.jarvis/daemon-*.sock`. Other startup deaths still surface only `Daemon process N died during startup`; actionable diagnosis for those shapes lives in the keyed process log `~/.jarvis/daemon-<digest>.log` (no `jarvis daemon log` subcommand yet — ready intent `daemon-process-log-read`). The CLI does not replace that log for every startup failure.
...
**Daemon start reaps orphaned ready-gate test groups.** Before IPC opens, startup sweeps every durable `ready_gate_pgid` whose owning run is not live (dead or absent `owner_identity`), signaling the recorded process group (SIGTERM→SIGKILL) and clearing the column. This complements the live run-termination reap path for ready-gate and required-integration spawns. Three publication-time verifier spawns (base-ref probe, diff-derived mutation verifier, runtime smoke verifier) are still unbound and can leak across sessions — see [Known gotchas](#known-gotchas).

**Notification-sweep event-loop starvation (2026-09-03).** Pre-fix daemons could stall IPC for tens of seconds per five-second tick by re-deriving full incident history on the event loop; overlapping timer ticks compounded it. Symptom: keyed socket binds but reads time out — `jarvis daemon status` reads `stopped` while `lsof` shows a listener and agents still write. Retrying `jarvis daemon start` stacks CPU spinners without unblocking IPC. **Before `kill -9`:** rule out [superseded same-key daemon](#a-superseded-daemons-live-runs-are-invisible-to-the-cli-and-look-exactly-like-the-deadlock-shape-2026-09-02) (multiple `daemon-entrypoint.ts` PIDs) and [`daemon stop` / `run kill` deadlock](#known-gotchas) (`in-progress` + `not-live` with no live agent). When those checks fail and one daemon PID runs hot CPU with dead IPC, `kill -9 <daemon-pid>` then `jarvis daemon start` — same recovery shape as deadlock, but the reachable daemon owns the runs. Bounded derivation and non-overlapping sweeps ([daemon-host.md § Operator notifications](./daemon-host.md#operator-notifications)) prevent recurrence on current builds; restart after merging those fixes.
...
`intent-reviewed`, `plan-reviewed`, and `plan-reviewed-light` were legacy aliases and are **now retired** — they resolve to nothing and are rejected as unknown segments (`v2/src/cli.test.ts`). Use `intent` / `plan`, and do not build new `*-reviewed` preset code paths.
...
**Chained intent→plan for external ready-intents.** On opted-in projects (inverted `effectivePublishGit`: project `plan.commit`, then machine `modes.plan.commit`, then `true`), a pipeline's plan stage resolves `ready-intents/<name>.md` from the intent artifact even when that file exists only under `~/.jarvis/specs/<projectSafeId>/ready-intents/` and not on the prior intent worktree or git branch — the same external-home routing standalone plan uses. Chained plan dispatch does not require committing the ready-intent to the intent branch first. Resolution fallbacks and containment rules: [`daemon-host.md` § Pipeline stage resolution](./daemon-host.md#pipeline-stage-resolution).
...
jarvis pipeline list                              # merged human snapshot; does not follow live work
jarvis pipeline list --json                       # merged JSON snapshot
```

`jarvis pipeline list` queries every discovered keyed daemon socket plus the invoking digest's socket, skips sockets that fail, and merges answering snapshots. This keeps pipelines visible across executable-digest rotation. Duplicate `pipelineId` rows prefer a finished snapshot, then more ended stages, then the earlier socket path. Human and JSON rows sort by `createdAt` descending and `pipelineId` ascending. If no daemon returns a valid snapshot, the command exits non-zero with `No live pipeline daemon responded; run jarvis daemon start, then retry.`; if any answering daemon instead returned a malformed payload, it reports `invalid daemon response` without suggesting a restart. Listing never auto-starts a daemon.

JSON pipeline rows include identity, derived state, optional admitted `terminalAction`, optional unchanged durable admission `seedPath`, and nullable terminal-publication success/failure. A relative `seedPath` remains relative to admission `cwd`; the snapshot does not expose `cwd`. Ordered stages include durable `id`/authored `position`, lifecycle fields, `artifact`, and `failureDetail`, preserving nullable and falsy JSON diagnostics. A terminal `failed` workflow stage never names a still-live entry run in `workflowInvocationId`; terminal rows may retain the settled entry-run id. Each socket receives one non-blocking `pipeline_list` request with no client-side polling — use the merged result for point-in-time inspection, not completion tracking.

The human listing's first column is the shortest prefix (at least eight characters) unique among the listed pipelines, and every pipeline verb (`wait`, `approve`, `reject`, `resume`, `recover`, `dismiss`, `undismiss`) accepts it verbatim or the full `pipelineId` from `--json`: the daemon resolves an argument of eight or more characters that strictly prefixes exactly one pipeline (dismissed included). A prefix matching several pipelines refuses with `pipeline_id_ambiguous` and lists the candidate ids on stderr; an argument matching nothing keeps the verb's ordinary not-found reason (`pipeline_not_found`, or `unknown_pipeline` for `wait`). Arguments shorter than eight characters never prefix-resolve.

Dismissed pipelines are hidden by default. Add `--all` to see them too — it sends `{ includeDismissed: true }` to every socket and appends a trailing `dismissed`/`-` column to the human listing (present only under `--all`). `--all` composes with `--since`/`--state` (filtered after merge) and with `--json` (which preserves `dismissedAt` without a synthetic marker field) — it does not lift the existing `--json`+`--since`/`--state` incompatibility. A default listing where every pipeline is dismissed still prints `No pipelines.`, with no hidden-count hint; reach for `--all` to check.
...
**`stageId` is the authored stage name, not the row's `id` (2026-09-04).** A `pipeline list --json` stage row carries both `id` (a row uuid) and `stageId` (the authored name, e.g. `approve-intent`); `approve` / `reject` want the latter. Passing the uuid fails with a bare `stage_not_found` that names neither field, which reads like the gate is gone. Read `stageId`, not `id`.
...
```sh
jarvis pipeline resume <pipeline-id> [<branch-key>] [--reset-despite-dirty] [--reset-despite-landed-criteria]
```

Use **`pipeline resume`** (not `pipeline start` or `jarvis run resume`) when a pipeline stalled at a failed stage or an approval gate and you want the daemon to reopen or claim continuation from persisted admission context. Recovery eligibility (admitted vs refused vs no-effect, branch scope, deferred settlement, missing context, terminal states): [`pipeline-execution.md` § Operator recovery](./pipeline-execution.md#operator-recovery). Exit **`0`** on `kind: "resumed"` means the daemon admitted detached continuation, not that the pipeline finished — stdout prints the admitted pipeline ID (same as `pipeline start`); pair with `pipeline wait` or `pipeline list` for progress. Terminal pipelines (`pipeline_terminal_succeeded`, `pipeline_terminal_rejected`) and other refusals print the daemon `reason` verbatim on stderr and exit non-zero. Failed resume replays from the failed stage (preserving predecessor invocation IDs); awaiting resume claims the pipeline without dispatching past the gate — approve the gate separately, then `pipeline wait`. When approval continuation was lost after the gate already reads `approved`, `pipeline resume` dispatches the pending successor via `continuePipeline` without reopening a failed stage (whole-pipeline and explicit `default` only when aggregate derived state is not `awaiting-approval` or `running`; mixed fan-out with siblings still awaiting or running uses branch-scoped resume). When a chained `plan` or `implement` stage failed because the prior entry-run worktree directory was removed (for example after `jarvis cleanup --abandon` to clear a dirty gate), `pipeline resume` re-resolves the downstream input from the durable prior branch (or pipeline admission base) and dispatches — clearing the worktree no longer permanently strands resume.

Failed `plan` resume owns the harness preamble — shared stale reset, base alignment, reserved harness-blocker clearing, and dispatch. It classifies an existing lane as disposable only when the lane branch has no open PR and no commits ahead of base whose changed paths leave harness workflow staging. Default resume retires and rematerializes such a never-landed lane from base without flags or preceding manual branch cleanup, base merge, or staged-blocker edit, even when its `HEAD` is not descended from base, its acceptance criteria drift from base, or its `.jarvis-plan-stage/intent.md` contains an operator-authored `## Blocker`. Draft-tree operator blockers and reserved harness `Artifact contract check failed:` sections are discarded with the stale draft; they are not landed operator decisions. When stale reset retires the worktree, stderr prints `failed plan resume worktree disposition: retired-and-rematerialized from base` before writer dispatch; when no materialized worktree exists, stderr prints `failed plan resume worktree disposition: reused existing worktree`.

Disposal does not cross landed-work guards. A branch with commits ahead of base that change any path outside harness workflow staging refuses with the tip, commit count, and salvage instruction to hand-finish the branch or run `jarvis cleanup --abandon <branch>`; the worktree and commits remain intact. Uncommitted paths outside `.jarvis-plan-stage/` also refuse before retirement. An operator `## Blocker` committed on base still refuses. Any open PR excludes the lane from disposable classification: an operator blocker associated with a live draft PR refuses, while a lone draft PR without one follows the ordinary stale-reset gates and retirement path; a ready PR or multiple matching open PRs retain their existing refusals. Live run/worktree ownership, claim contention, dirty state outside harness draft staging, and stale-reset preparation failure also preserve the worktree and fail the stage without dispatch.

Use `pipeline resume` when the failed draft should be discarded and redrafted; use `pipeline recover` when the corrected `.jarvis-plan-stage/` tree should be kept, revalidated, and landed without another write step. Branch-scoped resume changes only the named lane, and its reset policy survives detached continuation, claim loss, and daemon restart until dispatch completes. `jarvis cleanup --abandon <branch>` remains the salvage path for preserved refusals or partial teardown, not the default never-landed restart procedure. See [`pipeline-execution.md` § Operator recovery](./pipeline-execution.md#operator-recovery) for admission and refusal detail.
...
Add the branch key when a single approved branch's stage failed on a fan-out pipeline while sibling branches still sit at their own gates — omitting it on such a pipeline with resumable failed `plan` lanes lists those branch keys on stderr (`branch_resume_required`, one key per line after the reason) instead of claiming aggregate `awaiting-approval` or an opaque refusal; unscoped paths without such lanes are unchanged (including claim-only awaiting resume and approved-gate pending-strand dispatch when aggregate derived state is not `awaiting-approval`). Branch-scoped resume touches only that branch's rows; sibling gates and stages are untouched. The CLI prints only the daemon `reason` string for other refusals, not `branchKey`/`stageId`; a `branch_awaiting_approval` refusal means resume raced an unresolved decision on that branch — look up the gate's stage ID via `pipeline list` or `pipeline wait` boundary JSON, approve or reject it, then resume or `pipeline wait` again.
...
```sh
jarvis pipeline recover <pipeline-id> <branch-key> [--reset-despite-dirty] [--reset-despite-landed-criteria]
```
...
jarvis notifications list [--since <cursor|duration|timestamp>] [--kind <incident-kind>]... [--project <name>]
```

`jarvis notifications wait` without `--project` blocks on one daemon `notification_wait` call until the next owed delivery; with `--project`, the CLI loops — each iteration is one `notification_wait` call, skipping non-matching incidents without stdout until a matching incident or RPC error. It prints one minified JSON line `{ incident, deliveryCursor }` on stdout and exits `0`. Use `deliveryCursor` as the next `--since` to chain waits without gaps: the delivery-cursor form of `--since` is exclusive (the incident that produced the cursor is never returned again), while duration and timestamp forms stay inclusive of their window edge. A chaining loop that actually advances:

```sh
cursor="$(date -u +%s)000:run:none:none"   # or a cursor from a prior wait
while line=$(jarvis notifications wait --since "$cursor"); do
  echo "$line"
  cursor=$(echo "$line" | jq -r .deliveryCursor)
done
``` `jarvis notifications list` issues one `notification_list` call, prints sink-shaped `incident` objects as incident-only NDJSON (one per line) without blocking, and exits `0`. Both accept `--since` as a delivery cursor (`deliveredAt:incidentId:transition`), relative duration (`2h`, `90m`, …), or absolute Unix ms / ISO 8601 timestamp, optional repeated `--kind` filters, and optional `--project <name>` to scope wait wake and list stdout to incidents whose `project` field exactly matches one registered project name on a shared daemon; omit `--project` for machine-wide visibility across all projects. `--project` composes conjunctively with `--kind`. Filtering affects CLI output and which incident satisfies `wait` only — the delivery ledger and `notificationSinkCommand` push are unchanged; filtered incidents remain owed and still discharge to the sink. Cursor wire form, RPC params, and ledger semantics: [daemon-host.md § Operator notifications](./daemon-host.md#operator-notifications). Sweep placement and multi-daemon discharge: same section.

**Fan-out lane failures on live pipelines.** On a non-terminal fan-out pipeline, a stage row that settles `failed` while sibling lanes remain `running` or `awaiting` produces a stage-scoped notification with `kind: "stage-failed"` naming that lane's `branchKey` (plus `stageId` and `pipelineId`) — the pipeline stays live and the sink fires for the failed lane explicitly instead of waiting for whole-pipeline terminal. Pair with `pipeline resume <pipeline-id> <branch-key>` for that lane. Derivation detail: [daemon-host.md § Operator notifications](./daemon-host.md#operator-notifications) (Pipeline stage incidents).

#### Deciding a workflow is finished

**Primary path: configure `notificationSinkCommand` in `~/.jarvis/config.json` and let the daemon push.** A live daemon sweeps derived operator incidents after startup reconciliation, on a five-second timer, and after state transitions. Each owed `(incidentId, transition)` fires your shell command once with one JSON object on stdin (`kind`, `pipelineId`/`runId`, `transition`, `cause`, …). Pipeline approval gates, terminal pipeline outcomes, publication failures, wedged settlement, failed fan-out lanes (`stage-failed`, naming `branchKey`), blocked runs, budget-soft-stops, and ad-hoc workflow terminals surface at derived altitude — not as a burst of per-step run rows. See [install-and-config.md](./install-and-config.md#operator-notification-sink) and [daemon-host.md § Operator notifications](./daemon-host.md#operator-notifications). Without a sink configured the sweep still advances the delivery ledger silently; nothing spawns.
...
**Diff-derived verification stall (fixed vs residual).** In-loop and publication-time diff-derived verification no longer re-run the whole-diff CI script union per mutation candidate — the prior `shared/**` aggregate-scope fan-out could exhaust `MAX_VERIFICATION_MS` and strand finalization `in-progress` after correct code landed. Each candidate now runs only its resolved killing-test set via `bun test` (co-located exact-stem and sibling tests when present; direct-importing `*.test.ts` under the surface-prefix scan root only when co-located resolution is empty; see [workflow-runner.md](./workflow-runner.md)); changed registered prompts resolve only mapped render-observer test file(s) from the verification worktree's `shared/prompts/render-observer-tests.ts` (see [write-behavior.md § Diff-derived mutation verification](./write-behavior.md#diff-derived-mutation-verification)); `MAX_CONCURRENT_VERIFIER_TEST_RUNS` (4) caps parallel verifier-launched `bun test` subprocesses; each scoped killing test carries a fixed 30-second wall clock (`MAX_KILLING_TEST_MS`) with process-group isolation, and a non-terminating mutant settles `non_terminating_mutation_failed` instead of waiting indefinitely. Diff-derived scope is narrower than the ready gate by design — a thin killing test can miss what the full gate union would catch. A mis-derived candidate whose `originalText`/columns no longer match the source line is skipped (recorded in `PassResult.skippedCandidates`) rather than crashing the run as `run_execution_failed`; the underlying guard-flip slice bug is tracked in `guard-flip-derivation-crash-is-contained`. **Residual risks:** missing killing coverage (`missing-killing-test`; `importer-discovery-cap-exceeded` when co-located resolution is empty and same-root `*.test.ts` sprawl exceeds the per-file 200-candidate importer bound on an uncovered guard), missing render-observer map entry (`missing-render-coverage`), deadline exhaustion across many cheap candidates within `MAX_VERIFICATION_MS`, or a leaked `bun test` child after the per-candidate timeout (see [Known gotchas](#known-gotchas)). Salvage: inspect `jarvis run log <id>` for `surviving_mutation_reprompt` (in-loop miss), `surviving_mutation_failed`, or `non_terminating_mutation_failed` with those mutation kinds; add co-located or direct-importing killing coverage, reduce same-root `*.test.ts` sprawl under the scan root, add or repair the worktree observer-map entry (no merge or daemon rebuild required — see [write-behavior.md § Diff-derived mutation verification](./write-behavior.md#diff-derived-mutation-verification)), kill orphaned `bun test` children if CPU is pegged (see standing rule below), then let the live agent fix in-loop or `jarvis run resume` on the implement write row after committing fixes. For `missing-render-coverage` on a registered prompt whose diff only bumps frontmatter metadata or deletes in-file body lines (no body `add` lines), ensure mapped observers assert on the unmutated post-change rendered output — sentinel mutation does not apply on those exempt paths (canonical contract: [write-behavior.md § Diff-derived mutation verification](./write-behavior.md#diff-derived-mutation-verification)). Publication-time survivors after ready-gate repair use review `write.mutation-repair` admission instead.
...
**Confirm a `completed` implement row by its PR, not its status.** The historical instance (run `eabc39a7`, 2026-08-03) settled `completed` / `no-work` with `iteration_commit skipReason: "no_file_changes"` — no commit, push, PR, or gate — because it ran in a stale dirty worktree and read that tree's already-ticked spec copy as truth. The completion-honesty guards below now close that path, but the habit is cheap and the next honesty gap will present the same way.
...
**Non-terminating mutants.** A timeout is not proof of a non-terminating mutant. When the mutated killing set exceeds the 30-second floor, the verifier restores the original bytes and times the same set unmutated (once per set, up to a 120-second ceiling): only if the clean run finishes inside the floor does verification settle `non_terminating_mutation_failed` with `nextAction: "resume"` — distinct from `surviving_mutation_failed` (tests pass under the mutation). A slower clean run widens the bound to 2× the measured baseline (capped) and retries; a clean run that exceeds the ceiling settles that candidate inconclusive — read the measured baseline in the `inconclusive:` reason on the run log's `mutation_verification_inconclusive` event — and the run still publishes. Before 2026-09-09 the fixed 30-second bound misreported every candidate in a slow killing set (`workflow-runner-resume.test.ts` runs 32 s clean) as non-terminating, and `resume` was a fixed point; that shape no longer exists. In-loop and publication-time discovery share this settlement: no live-agent reprompt and no review `write.mutation-repair`. Recovery is agent-free `jarvis run resume` on the owning write row after fixing the killing test or mutation site; the resume tail replays mutation re-verification, the ready gate, and publication only.
...
**Operational caveats.** Scoped verifier runs are bounded by `MAX_VERIFICATION_MS`, a per-candidate killing-test bound derived from the unmutated baseline (floor `MAX_KILLING_TEST_MS` 30 s, ceiling 120 s), and a concurrency cap; several surviving sites multiply publication wall clock. Same-file candidates run serially (mutate → scoped test → restore before the next candidate on that file); distinct production files may overlap. While a candidate is under test, `<worktree>/.jarvis-diff-derived-mutations/<sha256>.json` records the applied mutant (`file`, `line`, `mutation`); inspect that directory before reading an inverted guard in production source as agent corruption — the sidecar names harness-applied mutants, and restore clears each record when that candidate finishes (see [write-behavior.md § Diff-derived mutation verification](./write-behavior.md#diff-derived-mutation-verification)). A wedged or slow verifier killing-test child can present as a live run with no agent process until the per-candidate timeout settles `non_terminating_mutation_failed` or the child exits; check `jarvis run log <id>` and the sidecar directory, not only `run list` liveness. Leaked `bun test` orphans after timeout are covered under [Known gotchas](#known-gotchas). A pre-fix same-file concurrency bug could report `surviving_mutation_failed` at a changing site or for a mutation you can kill manually — see the flip-and-test check below.

**Flip-and-test false-positive check.** When `surviving_mutation_failed` names a site you believe is covered, manually apply the reported mutation at that exact source site (or flip the named guard), run only the resolved killing test(s) (co-located tests when present; otherwise direct importers under the scan root), and compare the outcome. If the scoped test fails under your manual mutation, treat the harness report as a false positive (pre-fix same-file concurrency clobbering in-flight mutants, or a live-verification tree read during finalization — not merely "the tree looked broken while the verifier was running"). If the scoped test passes under the mutation, treat it as a genuine uncovered guard (or apply existing timer/dual-constraint runbook guidance when `dualConstraint` is set). When the failure is `importer-discovery-cap-exceeded`, scoped execution was blocked by same-root `*.test.ts` sprawl on an uncovered guard — reduce sprawl under the scan root or add co-located killing coverage; this failure no longer applies when co-located coverage is present.
...
**Missing gate command.** When spawn `ENOENT` or an anchored package-manager/shell failure line matches (see [v1-behaviors.md](v1-behaviors.md)), finalization settles `ready_gate_command_missing` instead of entering autofix or repair. Read `readyGateCommandMissingEvidence` (spawn code or anchored line), `readyGateCommand`, and bounded `readyGateOutput`; lint or test output that merely mentions `ENOENT` does not trigger this settlement. The terminal event also records `readyGateCommandSource` (`"configured"` vs `"default"`), and the operator error message names it alongside the command. When `readyGateCommandSource` is `"configured"` (the project set `projects.<key>.readyCommand` to a missing command), fix that config or add the missing script, then re-dispatch — `nextAction` is `fix_config`. When it's `"default"` (the built-in `bun run ready` itself is unresolvable), config is already correct — the harness can't resolve its own default, so `nextAction` is `stop`; fix the harness/environment instead of project config. Legacy rows with no recorded source keep `fix_config`. `jarvis run resume` is refused in both cases because resume cannot create the command.
...
When operator `pause` stops a linked-implement write loop, the durable row keeps its minted `implement~link-N` `stepId`. `jarvis run resume <run-id>` on that paused row re-enters the linked loop through `reconstructPausedWriteResumeInput`: the daemon resolves the authored snapshot write step, re-reads pinned linked-index routing, and dispatches the active subspec's `expectedArtifactPath` (and `specReadRoot` for external plans). Confirm `iteration_started` on `jarvis run log` after resume. Do not `run kill --force` a paused linked row whose index materialization is still present on disk — resume is the recovery path.
...
Resume consumes the seed the intent split on, exactly as the first-pass landing would: the write step's snapshot records `landingInputs` at dispatch and the resume landing replays that consumption. No manual seed deletion before merging. A review row whose snapshot predates that recording refuses `jarvis run resume` with `landing inputs not recorded on the persisted snapshot; re-run the intent` (`resume_unsupported`) instead of publishing a half-consumed queue — re-run the intent from the seed.
...
**An entered binding leaves an attributed row only when telemetry context, that binding's invocation ID, and its `agent`/`model` metadata were all present for the attempt.** A correctly filtered empty result under those conditions means no binding was entered for that run — not that the binding ran and telemetry silently dropped it. A binding whose `invoke` rejects before returning a result still gets a row: the rejection is normalized to `exit_kind: "error"` with `exit_reason: "exit_code:-1"` (see [`shared-invocation.md`](./shared-invocation.md)) before the row is appended. That normalization is skipped, though, when the invocation's signal was already aborted at rejection time — a timeout or stall abort racing the rejection leaves no attempt and no row — so a genuinely missing row does not by itself rule out an entered binding that crashed under an in-flight abort; cross-check the session transcript's `inbound_stderr` line for that window.
...
When an agent emits a `blocked` token without appending a `## Blocker` section, the harness first checks whether the token misled: on the implement path, if every non-human-only criterion of the active subspec is ticked, the iteration settles as `done` and no blocker is requested (the classic case was `blocked` appearing in the agent's summary prose — since 2026-09-09 only the last line decides the token, so that shape is rarer). Otherwise it reprompts for blocker text. If the agent still fails to provide it, the run reports as `missing_blocker` harness defect (`error.reason: "missing_blocker"`, `error.retryable: true`, `error.nextAction: "resume"`), not bare `agent_blocked`, and the `missing_blocker_detail` log record carries the classified `token` and the `tokenContext` line it was read from — read that before re-running a row that sits over ticked criteria and a completion commit. Applies to both the write (run/plan) path and the implement workflow path.
...
Reach for `jarvis run kill --force <run-id>` only after `run resume <run-id>` refuses (`unsupported_resume_context` or similar), or the row has no resumable write context at all — a stale `paused` row is the common case, since `paused` is not boundary-terminal and never ages out of the default retention window on its own. Paused linked implement rows (`implement~link-N`) with intact index materialization admit through `run resume` ([Paused linked implement rows](#paused-linked-implement-rows-implementlink-n)); force-kill is not the recovery path when that resume succeeds. Do **not** force a row whose owning daemon is mid-`finalization` or mid-`recovery`: the force path only takes the safe abort branch for a live `write-loop`/`workflow` active kind (see the `kill` RPC row in [`daemon-host.md`](./daemon-host.md#rpc-methods-transport-slice)); forcing a mid-finalization or mid-recovery row stamps it `killed` while that work keeps running underneath.
...
Check `~/.jarvis/daemon.log` and `jarvis run log <run-id>`. Plan draft stalls historically threw before agent invoke; the write-loop iteration timeout now arms before the agent spawn, so a stall settles rather than hanging.
...
### Intent operator checkout

Review and landing must use the split external worktree, not the operator checkout. If review dirties the primary checkout, treat it as a harness bug and seed it — the two seeds this note used to point at are both gone (`workflow-composable-collapse` was consumed when the `*-reviewed` aliases were retired).

### Daemon blocked on long git / ready subprocess

Sync subprocess on the daemon event loop was addressed by the responsive-daemon work. Symptom: `jarvis run list` hangs while a run finalizes. Check for `bun run ready` or `git` children on the daemon PID.

## Cleanup: eligibility gate

`jarvis cleanup [<project>]` runs five independent slices in one invocation: merged-worktree retirement, worktree-independent merged-branch ref pruning, stranded open-home spec archival, dead daemon-digest artifact reaping, and expired session-log reaping. A named project scopes the first three project-owned slices through one filtered registry; dead daemon artifact discovery and session-log retention remain global because daemon artifacts and session logs are not project-owned. Bare cleanup intentionally scopes the project-owned slices to every registered project. The dead daemon-digest slice consumes the keyed triplet contract from [Fail-closed daemon reads and digest artifact reaping](#fail-closed-daemon-reads-and-digest-artifact-reaping): the one preview lists every present `.sock`/`.pid`/`.log` companion of a dead digest under `remove:`, the one apply revalidates and removes them alongside the other slices, and live or ambiguously probed triplets stay preserved — there is no daemon-only cleanup entry point. An unknown project is refused before daemon discovery or cleanup survey. The positional project and `--abandon <name>` are mutually exclusive. Each slice previews in `--dry-run`, shares the apply confirmation prompt (`[y/N]`; `--yes` for scripted apply), and continues after partial failure in another slice unless noted below. `jarvis1 cleanup` remains blind to the v2 home; use `jarvis cleanup`.
...
After Git retires a workspace, cleanup resolves its durable workflow or ad-hoc spec identity and archives an eligible completed artifact to `completed/` in the same cleanup invocation; this path needs no rerun. A terminal durable row (`completed`, `failed`, `blocked`, `interrupted`, or `killed`) on the retired `(project, branch)` may supply that spec identity even though it does not retain merged-worktree eligibility. It prunes the consumed ready-intent only when it byte-matches the spec's `intent.md`: for a timestamped spec directory (`20260908T050011Z-<slug>`) the candidate is the slug-named `ready-intents/<slug>.md` (with `ready-intents/<directory-name>.md` as the fallback for unstamped homes); a filename match without identical bytes never prunes. `--dry-run` lists the worktree, archive destination, and that proven prune without changing worktrees, branches, specs, intents, or run rows. A failed retirement does not inspect or move its artifact. If archival is refused (incomplete criteria, an open matching PR, or another materialized owner), retirement remains successful and stdout names the artifact and refusal; resolve that condition, then rerun cleanup.

Cleanup also scans immediate open directories in every registered `v2/spec/` home, even when no workspace is retired. It ignores `completed/`, `seeds/`, `ready-intents/`, dot-directories (for example `.scratch`), and harness workflow staging directories (for example `.jarvis-plan-stage`, `.jarvis-intent-stage`, and other `.jarvis-*` sidecars). For each registered project whose `planSource` publishes externally, it also scans immediate open directories under `~/.jarvis/specs/<safeId>/plans/` that contain `index.md`, excluding `plans/completed/`, dot-directories, harness staging directories, and non-directories; it does not scan `ready-intents/`, `seeds/`, or other siblings at the external specs root. Stranded inspection rechecks each candidate path with `existsSync` immediately before eligibility or skip reporting, so entries deleted between discovery and inspection produce no stdout. When multiple registered projects share one `projectSafeId`, external `plans/` discovery is skipped for that safe ID (no candidates). The same completeness, open-PR, ownership, move, and rollback checks apply; in-repo stranded archival still byte-proves `ready-intents/<name>.md` against `intent.md` before prune, while external plan archival never prunes external ready-intents. Stdout (including `--dry-run`) names candidates and refusals. It never changes durable run rows. A stranded spec is owned only by a discovered managed worktree in the same registered project on its recorded implementation branch; primary checkouts and other resolved branches do not own it. Cleanup rechecks ownership immediately before archival, and refuses archival when a same-project managed worktree has an unresolved or detached branch.

External stranded and retired-worktree archival preview archive lines read `archive: plans/<name> -> plans/completed/<name>` (apply may print absolute paths under the same relative shape). Apply moves the tree to `plans/completed/<name>/` beside the source `plans/` home. Refusal stdout uses `Skipped artifact: <source> — …` for shared guards: incomplete or vacuous criteria, missing linked subspecs, matching open PR on the recorded branch, another materialized worktree on that branch, PR or ownership inspection failure, and archive destination already exists. In-repo stranded archival may also refuse on post-move rollback after a failed ready-intent prune; external plan archival never attempts ready-intent prune, so that rollback reason does not apply. **Ownership is spec-scoped.** A same-project managed worktree on the artifact's recorded implementation branch owns it; a detached or unresolved-`HEAD` managed worktree owns only the artifact its own durable run rows resolve to, for both stranded and post-retirement archival, so one detached worktree no longer blocks every spec in the home; primary checkouts never own. **One skip line per artifact.** Every refusal in one invocation is collapsed to a single `Skipped artifact: <source> — <reason>` line per canonical identity, printed at the end of the invocation after the preview or apply output; when the retirement-archival pass and the stranded pass both refuse the same spec, the line keeps the most specific reason (an ownership refusal over an eligibility reason, a reason naming concrete paths over a bare message). `--dry-run` and apply share that ledger. The former `Skipped stranded artifact:` prefix is retired.
...
**Terminal-claim reclamation.** Terminal durable rows do not retain `(project, branch)` ownership for merged-worktree eligibility, merged-branch ref pruning, or apply-time recheck; only non-terminal durable rows and daemon-live runs block those gates. The same terminal rows may still resolve spec identity for post-retirement archival in that invocation. Cleanup never mutates durable run rows.
...
### Fail-closed daemon reads and digest artifact reaping
...
Cleanup enumerates only exact lowercase `~/.jarvis/daemon-<16hex>.sock` names and treats each keyed `.sock`, `.pid`, and `.log` as one daemon-digest lifecycle unit. Only `ECONNREFUSED`, or `ENOENT` on a present on-disk socket file with no listener, proves the digest dead; preview lists every present triplet artifact under `remove:`, and apply revalidates the digest immediately before removing each listed artifact. `ENOENT` when the socket path is absent to the caller, a successful probe, or an ambiguous probe (timeout, permission error, unexpected error) preserves the entire triplet and reports the socket reason. Socket-less `.pid` and `.log` files are not discovered or removed. If the jarvis home cannot be enumerated, no daemon artifacts are removed.
...
**Four concurrent implements run cleanly; the ceiling is concurrent full-suite gate invocations, not lane count (2026-09-08).** Seven concurrent lanes — four implements plus three pipeline stages — ran at load 6-21 with zero watchdog false-kills and zero idle-output stalls. Fan out implements freely subject to the gate rules below. `plan`/`intent` have short gates and isolated spec dirs — fan those out without limit.
...
**Distrust any gate verdict formed under load — including `ready_gate_out_of_scope` (2026-09-06).** That settlement means "the failing paths also reproduce on `baseRef`", and it is `nextAction: stop`, so it strands the run with no resume path. The base-ref reproduction probe runs on the same loaded machine and can flake the *same test it is checking*, classifying a healthy run's load flake as a pre-existing failure. Observed: a run settled `ready_gate_out_of_scope` naming `v2/src/commands/workflow.test.ts`, which is 104/104 on `main` and 104/104 on the branch once the machine is quiet. Before believing an out-of-scope settlement, re-run the named paths on `main` **and** on the branch with nothing else running. **Known trigger (2026-09-08):** `v2/src/persistence/state-store-wal-concurrency.test.ts`, whose `waitForLockHeld` gives a spawned lock-holder subprocess a fixed 10s to boot Bun, open the database and print its marker — a deadline on process startup, measured under the contention `test:v2` itself creates by running files 10-wide. It failed five times in one session across four branches that do not touch `v2/src/persistence/`, and the fifth stranded a run (`6dc830ec`) as `ready_gate_out_of_scope`. In isolation it is 4/4 on both branch and `main`. The message is always `lock holder did not report jarvis-lock-held within 10000ms`. Seed: spec `wal-handshake-has-no-private-deadline` (shipped [#3648](https://github.com/cbrenner04/jarvis/pull/3648), 2026-09-09). Cleanup: delete this trigger note when that seed ships. The one reliable trigger of that shape on 2026-09-08 — `state-store-wal-concurrency.test.ts`'s private 10s lock-holder handshake deadline (five hits in one session, one stranded run `6dc830ec`) — is retired: the handshake now waits under the file's own test timeout (`waitForStdoutMarker`). The isolation check still applies to any other load-sensitive file: run it alone on an idle machine before believing the verdict.

**One concurrent full-suite gate invocation per daemon process (2026-09-08).** Implement agents invoke `bun run test:*` inside the write step's `iterationCeilingMs` budget. The harness now reserves `TEST_STEP_BUDGET_MS` (15 minutes) before starting a classified gate invocation, refuses with `gate_invocation_refused` / `nextAction: "resume"` when ceiling headroom is insufficient or the sole in-process gate slot is held, and serializes gate starts across lanes (`MAX_CONCURRENT_AGENT_GATE_INVOCATIONS = 1`, enforced by owned leases that only the acquiring lane can release — including on every finalization-repair exit — so the one-gate-per-daemon guarantee holds by construction, not by convention). Refusal aborts the harness invocation after the agent CLI has announced the Bash call — it does not prevent the agent subprocess from running. Shell-tool observability is wired for claude and cursor NDJSON only; codex implements remain unaccounted until a later slice. A lane killed during a gate invocation whose active subspec has only gate acceptance criteria outstanding settles `iteration_timeout` resumable even though the subspec stays in `remainingSubspecPaths`; `loop_finished` carries `gateInvocationCommand` and `gateInvocationElapsedMs` for ceiling kills during an active gate. Load average does not predict gate contention — attribute by what each lane is currently doing. Recovery for `gate_invocation_refused` and resumable gate-only `iteration_timeout` is `jarvis run resume` on the retained workspace.

**A hung gate child can peg a core for hours without any watchdog firing (2026-09-06).** Observed two simultaneously: `bun test <one file>` at 100% CPU for 1h14m whose parent was the **daemon** (a hung gate/verifier child, not a session orphan), and a second at 100% for 1h32m parented to **launchd** inside a worktree whose lane was still live — so verifier spawns leak from *running* lanes, not only across sessions. Neither is visible as a stalled run: the owning row still reads `in-progress`/`live`. Check parentage before concluding which you have (`ps -o ppid= -p <pid>`), because the recoveries differ. The auto-mode classifier blocks the operator agent from `kill`, so hand the kill to the operator's own shell.
...
- **A chained pipeline implement stage cannot tick its own acceptance criteria (2026-09-09).** The stage reads its spec tree from the prior (plan) stage's worktree while the agent writes code in the implement worktree, and the criteria-ticked completion contract reads the *former*. Three lanes in one session settled three different ways from this — `missing_blocker` (spec absent, agent cannot write or even append a `## Blocker`), `contract_miss` (spec present, agent ticked 15/15 in the implement worktree while the plan worktree copy read 0/15), and a `gate_invocation_refused` masking the same shape. **Merging the plan PR first does not work around it**; that only changes which failure you get. Until the seed ships, drive the implement from the spec on `main` with a standalone `jarvis run workflow implement --base origin/main --spec v2/spec/<dir>/index.md` — verified twice to complete end-to-end (criteria ticked, gate green, draft→ready flipped) on specs whose pipeline stage had just failed. Seed: `v2/spec/seeds/chained-implement-cannot-tick-its-own-criteria.md`.
  Cleanup: delete this bullet when that seed ships.
- **Merging anything while a pipeline is live fails its implement stage at admission (2026-09-09).** `base_behind_origin` refuses when the local base is behind its upstream and names `--base origin/main` as a remedy — but a pipeline stage resolves its base from admission context and has no flag surface, so that remedy does not exist for it. Merging stage PRs is how a pipeline lands its own work, so this is reachable by ordinary operation. The stage fails cleanly (no run row, no worktree, no agent invocation). Recovery: `git pull --ff-only origin main` in the operator checkout, then `jarvis pipeline resume <pipeline-id> <branch-key>`. Seed: `v2/spec/seeds/pipeline-implement-stage-cannot-clear-base-behind-origin.md`.
  Cleanup: delete this bullet when that seed ships.
...
- **`pipeline recover` discards your correction (2026-09-08).** The documented procedure — hand-correct `.jarvis-plan-stage/`, then `jarvis pipeline recover` — does not work. Recover admits (`{"kind":"admitted"}`, exit 0), then overwrites the corrected tree with the agent's original draft and validates *that*, settling a failure that can name a file you deleted before invoking. Observed on pipeline `443a6cd9`: corrected tree (renamed a misnamed subspec, deleted a vacuous sibling, rewrote `index.md`), recovered, and the stage settled `plan_stage_invalid` naming `00-daemon.md` with both original files back on disk. Not a git reset — `.jarvis-plan-stage/` is gitignored. Until this ships, a blocked plan stage whose draft needs correcting is a **hand-land**, not a recover. Note the older `stage_resolution_failed` refusal (recover never resolving past an approval gate on `full-review`) no longer reproduces. Seed: `v2/spec/seeds/pipeline-recover-resolves-past-approval-gates.md`. Cleanup: delete this bullet when that seed ships.
- **A branch cut before the v1 freeze still runs v1 tests, and it looks like a flake (2026-09-08).** `#3597` unwired `v1` from every gate, but a PR branched before it keeps its own `scripts/run-tests.ts`, which still calls `v1Tests(...)`. CI then runs the frozen tree and can red-gate on a v1 timeout that has nothing to do with the diff. Re-running CI reproduces it, because the stale scope is on the branch. **Check the merge base before believing a v1 failure** (`git merge-base --is-ancestor c602bd824 <branch>`); the fix is a rebase onto current `main`, not a retry. Observed on #3588, where `v1/test/intent-command.test.ts` timed out on a PR whose only v1 change was two prompt-fixture text lines.
- **`git diff main..HEAD` on a lane lies once you have merged anything (2026-09-08).** A lane branches from `main` at dispatch; every merge you land afterwards makes that diff show *your own merged work as deletions*. Observed on a 50-file lane diff that appeared to delete four seeds merged an hour earlier — it deleted nothing. **Always diff a lane against its merge base:** `MB=$(git merge-base main HEAD); git diff $MB..HEAD`. The same artifact makes `--stat` totals meaningless for review. This is the practical cost of [Concurrency](#concurrency)'s "do not merge while lanes are live"; when you do merge, remember which diff to read.
- **`invocation_error` can report your own prompt as the error, with no telemetry row (2026-09-08).** A `plan` dispatch settled `invocation_error` / `resumable: false` twice in a row where `error.message` was the tail of the *plan draft prompt* ("If you draft a subspec and then rename or rewrite it, delete the old file …"), ending mid-sentence, and `~/.jarvis/telemetry.jsonl` had **zero** rows for the run — so no agent, model, binding or exit reason to attribute it to. Do not read the message as a diagnosis and do not assume quota: quota exits *do* write telemetry rows (53 did earlier the same session). Check the agent CLIs respond (`codex --version`, `cursor-agent --version`, `claude --version`) before concluding an outage; other lanes dispatched fine on the same daemon. Seed: `v2/spec/seeds/invocation-error-reports-the-prompt-and-records-nothing.md`. Cleanup: delete this bullet when that seed ships.
- **Merging source work strands a *concurrent operator session* on a dead socket (2026-09-08).** The daemon key is a digest of the jarvis source tree and `jarvis` is one binary shared by every registered project, so a source merge in the jarvis repo rotates the key for **all** projects. A second operator session working another project (here `sudoku`) then finds its daemon gone and reports "daemon not responding" — its commands were addressed to a socket that no longer exists. Recovery is simply to re-issue the command: any `jarvis` command auto-starts and reaches the current daemon. Confirm before touching anything: one `daemon-entrypoint.ts` PID, one socket, `daemon status` with `loaded` equal to `current`, and `jarvis run list --all` showing no `in-progress`/`paused`/`queued` row. If a run *was* live across the changeover it can be swallowed instead — that is #3595 (`supersede-reaches-a-socketless-resident-daemon`). **Practical rule: while another session is working, restrict yourself to docs- and spec-only merges**, which do not rotate the key.
- **A jarvis command right after merging `v2/src` can report "no daemon is listening" while the daemon is healthy (2026-09-08).** Merging a source change rotates the digest, and a command issued during the changeover prints `Cannot abandon: no daemon is listening; run jarvis daemon start`. Observed twice; both times `jarvis daemon status` immediately after reported `running` with `loaded` equal to `current`, and re-issuing the same command succeeded. **Re-check `daemon status` before starting a second daemon** — see [Daemon lifecycle](#daemon-lifecycle) for why an unnecessary start is worth avoiding. Markdown-only merges do not rotate the key, so this only follows source merges.
- **Multi-subspec `implement` publication ready-flips a prior subspec's closed same-branch PR (2026-08-29):** a spec routes every subspec through one `<timestamp>-<name>` branch, so a re-run for subspec N sees subspec N-1's already-merged PR on that branch. Publication resolves that stale PR and tries `gh pr ready` on it, failing terminally with `ready_flip_failed` / `Only draft pull requests can be marked as "ready for review"` — impl, review, and mutation gates all green, branch complete, no PR. **Recovery:** the branch is done; hand-publish. In the worktree tick the merged prior subspec in `index.md`, `git push`, then `gh pr create` and admin-merge after CI. Observed on the P0 deferred-settlement spec (run `949a26cb`, closed #3054 → hand-published #3069). Seed: `v2/spec/seeds/implement-publication-tail.md` (this is its second half). Cleanup: delete this bullet when that seed merges.
- **Fan-out gates: approve back to back (2026-09-07, revised 2026-09-08).** Approving all of a fan-out's `approve-intent` gates back to back, before any plan consumes its input, dispatches every lane in parallel — verified on two independent pipelines (four concurrent plan stages). Plan resolution is branch-scoped and dispatch binds each lane's plan result by branch key ([`pipeline-execution.md` § Fan-out lanes](./pipeline-execution.md#fan-out-lanes)), so a sibling's already-consumed ready-intent no longer blocks a later lane's dispatch; serial gate approval and hand-driving later lanes standalone are not needed for that shape. **A dependency refusal is different and correct:** when the ready-intents form a strict prerequisite chain, a dependent lane settles `blocked` / `agent_blocked` naming the unmerged sibling spec — one cheap dispatch and a diagnosable row. Read the ready-intents' `## Prerequisites` first: for a known chain, approve the head, and after each prerequisite merges resume the dependent lane with `jarvis pipeline resume <pipeline-id> <branch-key>`.
...
- **`idle_output_timeout` false-kills a productively-working silent agent, worst under test contention (2026-08-28):** the idle-output watchdog killed an implement write step that had *just committed correct work* — cursor/codex don't stream output the watchdog can see, and concurrent implement stages widen the silent window by contending on the test suites. When the terminal row is resumable (`loop_finished.resumable: true` / `error.nextAction: "resume"`), recover with `jarvis run resume` on the retained workspace. When no checkpoint commit exists (`resumable: false`), the committed work may still be real — salvage it (bank the committed subspec, hand-finish the rest) or re-dispatch rather than re-running from scratch on the same silent agent. Related: `implement-resumes-stalled-unmerged-subspec-chain`.
- **Age is the wrong orphan test; CPU and parentage are (2026-09-03).** The `DD-` day-field heuristic below passes the common case. Eleven `bun test --test-worker` processes at ~96% CPU each, aged **24–62 minutes**, stole ~11 cores for over an hour — load sat at 20 with *one* live run on the machine — and manufactured five implement `iteration_timeout`s that read exactly like a concurrency ceiling. A session-start sweep that counts processes rather than attributing them walks straight past this. **Attribute before concluding anything about parallelism:**

  ```sh
  ps -Ao pid=,etime=,pcpu=,comm= -c | sort -k3 -rn | head -20        # who is actually burning CPU
  ps -Ao pid=,command= | grep '[t]est-worker' | awk '{print $1}'      # candidate PIDs
  lsof -a -p <pid> -d cwd -Fn | grep '^n'                             # which worktree it belongs to
  ps -o ppid= -p <pid>                                                # walk up to the root; repeat to launchd
  ```

  **Kill the producer, not just the workers.** Killing eleven workers freed the CPU for seconds before new cohorts respawned: the root was a **3h35m-old `bun run test:v1`** whose parent was `launchd`, orphaned from an earlier session's shell and still re-spawning `--only-failures` retry cohorts into a dead lane's worktree. Trace to a root whose parent is `launchd` (or `1`) and kill that first, then its children. A load average that stays high with nothing live is the tell.

  Since [#3431] daemon start reaps ready-gate process groups whose owning run is not live, so **jarvis-owned gate orphans self-clear on a daemon bounce**; what remains is operator/background test runs and the unbound sibling spawns named below.

- **Leaked `bun test` pool workers also come from the operator's own background test runs (2026-08-11):** not just killed jarvis runs — a background `bun run test:v2`/`bun test` that the operator (or a subagent) launches and that reports "done" can leave its pool workers running, pegging CPU for tens of minutes and starving live runs to `iteration_timeout`. Observed: four v1 `--only-failures` workers at 99%×4 CPU for 24 min timed out an unrelated implement entry run. Per-PID `kill -TERM <pid>` (sandbox disabled) worked — the auto-mode block noted below applies to `pkill` patterns, not always to targeted PID kills. Sweep with `ps -Ao pid,etime,pcpu,command | grep test-worker` after any background test run and before diagnosing a slow run. Jarvis-owned ready-gate orphans from daemon loss are reaped on `jarvis daemon start`; background operator test runs are not.
- **Leaked ready-gate `bun test` children peg CPU for days (2026-08-09):** a killed or abandoned run signals its ready-gate (`bun run ready`) and required-integration (`ready-finalize.ts`) process groups on termination, and daemon start reaps gate groups recorded on runs no live owner holds (daemon-loss orphans). Iteration timeout does not abort that live signal. Three sibling spawns stay unbound and can still leak: the base-ref probe (`createDefaultReproduceReadyGateAtBaseRef`), the diff-derived mutation verifier, and the runtime smoke verifier. Orphans from those can accumulate across sessions, saturate CPU, and drag every concurrent gate — stretching debate reviews past 13 min and wedging a publication step at `iteration_started` with no agent process. **Standing rule: never tolerate these — a single day-old `bun test` orphan is one too many, and three is a hard stop.** Check at session start and any time runs turn slow or wedge (`ps -o etime,command -ax | grep "[b]un test"` — a `DD-` day field is a certain orphan, but **age is not the test**: minutes-old workers at ~96% CPU are the same problem, see the bullet above), and clear unbound verifier orphans *before* launching or diagnosing heavy work. The auto-mode classifier blocks the operator agent from `kill`/`pkill`, so hand it to the operator's own shell: `pkill -9 -f "bun test"` (nothing live depends on day-old orphans; the live gate spawns fresh test procs). Seed: ready-intents `record-verifier-spawn-process-groups` / `reap-verifier-process-groups` (the verifier process-group persistence half shipped 2026-09-08 as spec `persist-verifier-process-groups`) (the original `reap-ready-gate-test-children-on-run-termination` seed was reaped when [#3431] landed the gate-group half; the three sibling spawns had no owner until this one). The diff-derived verifier's own bounded per-call timeout (`MAX_KILLING_TEST_MS`) now has a regression proving its killing-test process group is reaped once verification returns; this does not close the gap above — the unbound-to-run-termination spawns, their cleanup condition, and the CPU/parentage attribution guidance stay as-is. Cleanup: delete this bullet when those three spawns are also bound to run termination.
- **Read the committed diff, not the worktree, when a CI failure will not reproduce locally.** `git -C <worktree> show HEAD:<path>`. The historical instance (PR #2314, 2026-07-30) was the retired `@mutate` checkpoint verifier leaving `HEAD` holding an inverted guard while the working copy held correct source, so the run's own gate and a hand `bun test <file>` both passed while CI failed 26 tests. That mechanism is gone (`retire-implement-mutation-checkpoint-verification`), but worktree-versus-`HEAD` divergence is not unique to it, and the check costs one command.
- **`daemon stop` and `run kill` no longer deadlock over an orphaned row (fixed 2026-09-09).** A durable row that is non-terminal *and* not live used to be refused by both (`active durable runs` / `run_not_active`), and the only recovery was `kill -9 <daemon-pid>` then `jarvis daemon start`. `daemon stop` now asks the daemon for its live run ids and refuses only rows that are live, naming both groups (`live durable runs: …; orphaned (reconciled on stop): …`); after the process exits it settles the orphaned rows through the same reconciliation startup runs (`killed` / `interrupted`, `run_reconciled` reason `daemon_restart`) and prints `reconciled N orphaned run(s)`. An unreachable daemon counts every non-terminal row as live, so a stop never proceeds on silence. The `in-progress` + `not-live` tell on `run list` still has two non-bug causes that this does not change — a superseded same-key daemon working its own runs, and a bound-then-unlinked socket (see [Daemon lifecycle](#daemon-lifecycle)); run those liveness checks before believing a row is orphaned, and never `kill -9` a daemon for this shape.
- **`JARVIS_READY_TIER` is stomped by design, not by accident.** `ready-finalize.ts` spreads `process.env` and then overwrites the key with `"full"`, so setting it locally does nothing. This is deliberate — specced in `completed/20260714T023457Z-v2-ready-gate-runs-full-tier` as an override rather than a `??` default, after a `fast` inherit shipped a lint-red PR. The cost is real and accepted: the full aggregate gate is ~85% of a v2 workflow's wall clock (~13 of ~15 min on a two-file markdown plan spec). Do not "fix" this without revisiting that spec.
