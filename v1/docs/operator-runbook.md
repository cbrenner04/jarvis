...
- **CI ≠ `ready`, and the gap is wider than `lint:md`.** `bun run ready` runs `bun run test` — the **aggregate** suite (`scripts/run-tests.ts`), which routes every agent-mode file through `runV2TestFiles` and its `PER_FILE_TIMEOUT_MS`. **CI never runs the aggregate**: `scripts/ci-test-scope.ts` scopes to `test:v1`/`test:v2` by changed path, and `test:v1 agent` is `bun test --parallel` with *no* per-file timeout. So the one suite the gate depends on is the one suite CI never executes, and a green CI is not evidence the gate will pass. Observed 2026-07-16: the aggregate's 60s per-file budget was smaller than `v1/test/run.test.ts` (~119s), so **`main` was red on the operator's machine while every PR showed green CI** — the mechanism behind "every implement run needs a human finisher". Raised to 180s in #1644; seed `ci-cannot-protect-the-local-ready-gate` carries the real fix (CI/gate parity, splitting the 119s file). **When a gate goes red on a diff that cannot explain it, gate `main` itself before debugging the branch.** Cleanup: delete this paragraph when that seed ships.
- **A test can be green on Linux CI and deterministically red on macOS.** A timing sleep in a daemon test is a coin flip whose odds depend on the machine, and CI's odds are not yours. Observed 2026-07-16 (#1633 → fixed in #1644): the test waited on step 0's run id then slept 25ms, racing the review steps that run under their own ids — the exact assumption the v2 runbook already flags as a gotcha. Wait on an observable condition, never a sleep.
