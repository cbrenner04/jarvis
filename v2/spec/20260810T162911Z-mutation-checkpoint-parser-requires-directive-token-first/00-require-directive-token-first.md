...
- [ ] `v2/src/execution/mutation-checkpoint-verifier.test.ts` — `only directives under a named pin are linked to that criterion` stays green, preserving enclosing-test linkage.
- [ ] `v2/src/execution/mutation-checkpoint-verifier.test.ts` — `target text absent` and `target text ambiguous` (in the `unparseable causes are reported from opened pinning files` block) stay green, preserving target verification for recognized directives.
- [ ] `v2/src/execution/mutation-checkpoint-verifier.test.ts` — `prose @mutate without a directive-shaped occurrence is not selected` and `canonical suffix template quoted as contract prose does not select` stay green, confirming acceptance-criterion selection via the shared `DIRECTIVE_PATTERN` is unaffected by the line-parsing anchor change.
- [ ] `v2/docs/operator-runbook.md` states that later prose mentions and directive-position lookalike tokens are ignored; `v2/docs/v1-behaviors.md` records directive-position recognition and retained malformed reporting.
