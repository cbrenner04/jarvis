...
- Plan `commit: false` mode writes spec output to Jarvis-owned external spec roots and emits absolute-path next-step commands for later `jarvis run` execution instead of creating git commits, pushes, or PR transitions. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/spec-paths.ts`, `v1/docs/plan-mode.md`
