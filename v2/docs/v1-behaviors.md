...
- Prompt mode is strictly single-pass with per-agent fallback: agents are tried in `modes.prompt.agentOrder` order; `quota` falls through to the next agent, `model_config` also falls through, generic `error` halts immediately, and weak-quota upgrades are always allowed (no progress gate). Exit `2` is reserved for chains where every attempted agent ended as `quota`; mixed fallback chains with no success exit `3`. Sources: `v1/src/modes/prompt/run.ts`
...
- Prompt telemetry is written with namespace `<project-key>:prompt` (matching patch mode's `<project-key>:run` and plan mode's plan naming), and mode field is set to `"prompt"`. Prompt mode records one terminal row using the last attempted agent/configured model; all-quota chains report `quota` / `all-agents-quota`, while mixed fallback failure chains report `error` / `agent-failure`. Sources: `v1/src/modes/prompt/run.ts` (`appendTelemetryLine`)
