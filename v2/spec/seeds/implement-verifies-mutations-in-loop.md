...
- A surviving mutation reprompts the live agent — same machinery as the landing/blocker reprompts — with the mutation string and source site, naming both remedies: add or extend a co-located killing test, or (only for a provably behavior-neutral mutation) an exact colocated `// @mutate-equivalent mutation=<JSON string> reason=<JSON string>` directive on the mutated physical line (lexical `//` comment only; `mutation` first with one ASCII space before `reason`; standard JSON escaping; exact full mutation identity including joint acceptance for duplicate identities on one line; decoded reason must contain non-whitespace text; accepted candidates consume bounded inspection slots and appear once each in verifier `acceptedSites` audit evidence). The reprompt consumes the normal iteration budget; budget exhaustion settles exactly as today. Rules out a new bespoke budget or a silent skip.
...
- [ ] A test proves the in-loop reprompt honors an exact `// @mutate-equivalent mutation=<JSON string> reason=<JSON string>` directive (accepts that exact site+mutation, does not reprompt) once [[mutation-gate-equivalent-mutation-escape-hatch]] lands, and that reprompt-budget exhaustion settles as today.
