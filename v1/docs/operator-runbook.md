...
- **CI ≠ `ready`, and the gap is wider than `lint:md`.** `bun run ready` runs `bun run test` — the **aggregate** suite (`scripts/run-tests.ts`), which routes every agent-mode file through `runV2TestFiles` and its `PER_FILE_TIMEOUT_MS`. **CI never runs the aggregate**: `scripts/ci-test-scope.ts` scopes to `test:v1`/`test:v2` by changed path. Roster equivalence (aggregate = union of six scoped slices) and policy parity (timeout, isolation, failure handling) are protected by regression tests (`test/test-slices.test.ts`, `scripts/ci-test-scope.test.ts`), so green CI is evidence the gate can pass. When a gate goes red on a diff that cannot explain it, verify path classification is correct; if correct, the failure is environment-specific not a code issue. See [v2 behaviors § Test execution](../../v2/docs/v1-behaviors.md#test-execution-and-development-workflows) for the durable contract.
