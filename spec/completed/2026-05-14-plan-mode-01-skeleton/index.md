...
`spec/2026-05-14-cli-modes-and-config-v2/` must land before this spec is implemented.
...
- **No worktree, no branch, no commits, no PR.** Those are introduced in
  `spec/2026-05-14-plan-mode-worktree-and-commits/`. A `jarvis plan` invocation in
...
- **No config schema changes in this spec.** Agent order for future plan
  ...
  `spec/2026-05-14-cli-modes-and-config-v2/`. This skeleton may validate that the v2
...
- Run this spec with `jarvis run spec/2026-05-14-plan-mode-skeleton/index.md`.
...
- Changing patch-mode behavior beyond consuming shared mode-entry helpers
  already introduced by `spec/2026-05-14-cli-modes-and-config-v2/`.
