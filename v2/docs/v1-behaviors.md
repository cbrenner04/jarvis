...
- Telemetry records review pass invocations with `patch_phase: "review"` and role-specific iteration counts so they are distinguishable from implementation iterations and role progression is traceable in `~/.jarvis/runs.jsonl`. Actuator invocations are recorded with agent name from the patch agent order. The run summary shows review attempts separately from implementation attempts while preserving correct totals. Sources: `v1/src/telemetry.ts`, `v1/src/modes/patch/review.ts`, `v1/src/run-summary.ts`
