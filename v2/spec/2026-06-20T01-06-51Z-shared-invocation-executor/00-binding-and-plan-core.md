...
- **Advance semantics are per-binding, defaulting to quota-only.** The shared executor advances to the next binding iff `result.kind === "quota"`; its pre-existing `stops immediately on non-quota failure` test stays green and the executor is **not** changed to advance on `error` globally. A binding whose pre-D path advanced to the next agent on additional kinds (e.g. plan-draft continues on a classified hard `error` — `plan-draft-hard-error-continue.test.ts`) supplies an advance predicate to opt in. Each consumer's predicate preserves that path's **exact** pre-D advance/stop-on-`error` behavior, verified per path against its existing tests — no path's error handling changes. Rules out globally changing the executor contract (which breaks the executor's own test and over-applies one mode's behavior to all four).
