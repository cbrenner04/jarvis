...
- [x] Add optional `socketDiscovery` to `CliDeps`, defaulting to `discoverLiveDaemonSockets`.
- [x] Extract or share the TUI run-list merge helper (`isLive` owner preference) for CLI reuse.
- [x] Wire `run list` to discover live sockets, always include `deps.socketPath`, query each live daemon, merge, sort by `runId`, and render with `formatListRunRow`.
- [x] Tests in `v2/src/commands/run.test.ts`: two live daemons with a run on the non-invoking one; duplicate durable rows deduped to the `isLive` owner; one unreachable socket skipped; solo-daemon output unchanged; guard-inversion negative cases. Route per-socket fakes through `deps.connectIpcClient(socketPath)`, as the existing keyed-socket test does.
- [x] Update operator and architecture docs listed below.
