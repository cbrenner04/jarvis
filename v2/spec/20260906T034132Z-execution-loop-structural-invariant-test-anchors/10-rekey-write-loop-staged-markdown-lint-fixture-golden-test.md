...
- [ ] Re-key audit row `ex-wlsl-fixture-golden` per the decision ledger.
- [ ] Replace local `FIXTURES_DIR` usage with shared fixture registry helpers.
- [ ] Route golden loads through loud-failure fixture reads keyed by committed ids.

## Acceptance criteria

- [ ] `v2/src/execution/write-loop-staged-markdown-lint.test.ts` plan/intent staged Markdown lint cases load golden and violation bytes through shared staged-markdown-lint fixture helpers rather than a local `FIXTURES_DIR` and repeated filename literals; it fails against the pre-fix duplicated fixture root and passes after re-key.
