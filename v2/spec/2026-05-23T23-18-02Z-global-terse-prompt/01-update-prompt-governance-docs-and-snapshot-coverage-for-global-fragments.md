...
- [ ] Keep documentation aligned with the narrow design choice from `00`: one shared `global.terse` fragment, no duplicate terseness wording inside step prompts or `patch.rules`, and no new fragment-specific snapshot mechanism.
...
- [ ] Rendered snapshot tests continue to validate the final assembled patch, draft, refine, and review prompt bodies by revision-aware filenames after the terse fragment is introduced, using the concrete revision bumps from `00`.
- [ ] Patch prompt tests that assert exact rendered ordering are updated to reflect the new prefixed terse fragment while still verifying `patch.rules` placement and sibling-block behavior after `<PATCH_RULES>` injection.
