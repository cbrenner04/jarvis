...
- Daemon-independent phases (dead-socket reaping, open-home stranded scan/archival) run when no daemon answers — rules out failing the whole command on connect `ENOENT`.
...
- [ ] With no daemon listening on any key, `jarvis cleanup` reaps dead sockets, scans stranded open-home specs, and marks worktrees ineligible instead of aborting; a regression test in `v2/src/commands/cleanup.test.ts` or `cleanup-cli.test.ts` fails against the pre-fix abort.
