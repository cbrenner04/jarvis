...
- [x] `pipeline-execution.test.ts` contains zero `as unknown as AnyWorkflowStep` casts (reachable on main today across `taggedStep`, inline literals, array casts, and the `plan` builder).
