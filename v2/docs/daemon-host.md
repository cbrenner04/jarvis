...
When any list filter field is set on `list` (today `sinceMs`), both the
50-newest terminal cap and invocation-sibling retention are skipped. Matching
rows are returned newest-first (`created_at DESC`, `rowid DESC`), then capped to
`limit` when provided or **200** when omitted. For `sinceMs`, matching means
