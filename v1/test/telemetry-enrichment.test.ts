...
describe("extractUsageAndCost: estimated usage", () => {
  test("agent-priced cursor model with usage_source=estimated yields cost_source=estimated", () => {
    ...
    const result = extractUsageAndCost({ usage_source: "estimated", usage: usage100x50 }, "cursor", "Composer 2");
  ...
  test("estimated cursor usage with an unknown cursor model yields no-price", () => {
    const result = extractUsageAndCost({ usage_source: "estimated", usage: usage10x5 }, "cursor", "not-a-cursor-model");
    expect(result.usage_source).toBe("estimated");
  ...
  test.each([
    "opencode/glm-5.2",
    "opencode/deepseek-v4-pro",
  ])("agent-reported opencode usage without cost_usd yields cost_source=computed (%s)", (model) => {
