...
- [ ] `v2/src/daemon/pipeline-stage-dispatch.test.ts` — `every terminal pipeline stage-run write carries endedAt`; Mutation checkpoint: the linked terminal-patch mutation proves the inventory rejects a newly finishless classified settlement writer.
- [ ] `v2/docs/daemon-host.md` and `v2/docs/v1-behaviors.md` describe the explicit `endedAt` guarantee for terminal stage-run statuses only, including skipped suffix/default rows, and explicitly exclude approval decisions.
