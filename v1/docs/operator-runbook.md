...
1. **`jarvis1 cleanup`** — removes merged worktrees and archives each completed spec into the `completed/` directory of its home (`v1/spec`, `v2/spec`, configured `targetDir`, or the external `~/.jarvis/specs/<project-safe-id>/completed/` home for `commit:false`). For `commit:false`, it also prunes the consumed external `ready-intents/<branch-slug>.md`. Archival is gated on three preconditions keyed off the resolved spec name (timestamped basename when the spec dir is timestamped): the spec is complete (every non-human-only acceptance criterion checked, via the same `isSpecComplete` semantics as triage; a vacuous-complete spec with no non-human-only criteria is treated as incomplete while an in-flight owner exists), no open implementation PR on that spec name, and no other live patch worktree at `.worktree/<spec-name>/` besides the one just removed. When any precondition fails, the spec stays in place and a skip line names the spec and failing guard; the merged worktree is still removed and cleanup continues to other eligible worktrees (exit `0`). So running `jarvis1 cleanup` after a plan PR merges is safe while implementation on the linked spec is still in flight — the live spec path on `main` is not archived out from under the patch branch. `jarvis1 cleanup --abandon` is the inverse mode for retiring unmerged abandoned runs: it closes one matching draft PR best-effort, force-removes the worktree, deletes the local and remote branch, and leaves the spec in place for re-run; it bypasses the archival guards entirely. Both modes prompt `[y/N]`; use `--dry-run` to preview.
