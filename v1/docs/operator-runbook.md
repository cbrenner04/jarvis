...
- **Complete-but-dirty run.** All non-human-only acceptance criteria are satisfied but the worktree has uncommitted work — `jarvis1 triage <worktree-name> --mark-ready` auto-finalizes (commit, push with upstream setup when absent on the dirty-finalize path, open draft PR if absent, gate once, ready on green). No manual `git push --set-upstream` stopgap when `origin` exists but upstream tracking is unset. Refuses when the branch is behind base (`behind base, resolve then re-invoke`); integrate or rebase onto current base first. Never auto-tick criteria.
