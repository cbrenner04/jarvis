...
- [ ] `resolveReadyTestScope(cwd, baseBranch)` returns `"full"` when the base
...
- [ ] Passing `baseBranch` sets `JARVIS_READY_TEST_SCOPE` for the `bun run
      ready` child process to the classified scope for that branch's diff.
- [ ] `scripts/ready.ts`'s `getReadyCommands` runs one `bun run <script>` step
...
- [ ] Existing `v1/test/ready-script.sandbox-unrunnable.test.ts` and
