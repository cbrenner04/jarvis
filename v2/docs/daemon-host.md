...
When `sinceMs` is set on `list`, both the 50-newest terminal cap and
invocation-sibling retention are skipped. Matching rows — `created_at >= sinceMs`
— are returned in store order (`created_at DESC`, `rowid DESC`), capped to
`limit` when set or **200** newest matches by default before per-row assembly.
Durable rows are still not deleted; only the response is filtered. Passing
`limit` without a filter field does not enter this path.
