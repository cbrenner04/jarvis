...
- [x] `v2/src/execution/write-loop.test.ts` adds pre-fix-failing coverage that a fully attributed
...
- [x] The same test proves a mixed/in-scope first gate can repair once, then a fully attributed
      untouched-path next gate stops further repair and settles `ready_gate_out_of_scope`.
- [x] Inverting repair bypass or per-attempt reclassification turns its corresponding test RED;
      negative cases prove out-of-scope gates never invoke repair.
- [x] `write-loop.test.ts` deadline-kill and bounded-repair tests and
