...
| `kill` | `{ runId: string }` | `{ ok: true }` | Abort the run's signal immediately and record durable status `killed` when the row is not boundary-terminal (`completed`, `blocked`, `failed`). Leaves the worktree dirty. Rejected `run_not_active` if run is unknown, not active, or is a healthy workflow-started run; a wedged workflow run may be reaped (see [Live controls on workflow-started runs](#live-controls-on-workflow-started-runs)). |
| `resume` | `{ runId: string, decision?: "approve" \| "revise" \| "abort", prompt?: string }` | `{ ok: true }` (approve/abort/write resume) or `{ ok: true, stepId: string }` (revise) | Resumes paused, budget-stopped, killed, or retryable-publication workflow write runs only after shared snapshot reconstruction. The matching persisted step must retain non-empty rules, artifact path, agents, model config, and resolvable bindings; the reconstructed input preserves step identity, workflow snapshot, and timeout. Missing or invalid context returns `resume_unsupported` before claim/spawn. Eligible publication retries are `completion_commit_failed`, `ready_gate_failed`, and `ready_flip_failed`; `list` and `wait` expose the matching retryable reason with `nextAction: "resume"`. Ad-hoc stopped runs remain unsupported. Human decision flows are unchanged. |
...
Workflow-started runs are normally non-steerable: `pause` is always rejected,
and `kill` on a healthy in-flight workflow step is rejected `run_not_active`.
This preserves the atomic workflow-step contract.

A workflow step that `list` still reports as live can be killed only once it is
wedged and therefore reapable. Use the same `runId` shown by `list` with
`jarvis run kill <runId>`; successful reaping records durable `killed` status
and retains the worktree (including any dirty state). A subsequent `list` for
that `runId` reports `status: "killed"` and `isLive: false`. A live workflow
step that is not reapable remains rejected `run_not_active`; reapability does
not make workflow runs pausable.
