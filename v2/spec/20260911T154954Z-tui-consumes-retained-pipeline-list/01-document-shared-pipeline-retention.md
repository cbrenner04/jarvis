...
- `v2/docs/v1-behaviors.md` already documents the `pipeline_list` cap, its `sinceMs`/`state` exemptions, and durable-row preservation; add the TUI-inheritance fact as an amendment to that entry rather than a new one.
...
- [ ] `v2/docs/v1-behaviors.md`'s entry is amended to note TUI inheritance.
...
- [ ] The `v1-behaviors.md` entry on the `pipeline_list` cap is amended to state that TUI pipeline-derived surfaces (work tree, counts, attention) inherit the same retention.
