...
- Top-level PR conversation comments should be filtered to recent, non-bot,
  actionable comments rather than the entire issue-comment history. The exact
  time boundary can key off existing harness state, but the implementation must
  choose one deterministic rule and test it.
...
- Render one review prompt that includes:
  ...
  - each actionable inline thread with file path, line, author, body, and
    enough diff context to orient the agent
...
- [ ] Implement deterministic filtering for top-level PR conversation comments,
  including bot exclusion.
...
- [ ] Add tests covering: no open PR, no actionable comments, resolved-thread
  exclusion, bot exclusion, and prompt rendering with inline plus top-level
  feedback.
