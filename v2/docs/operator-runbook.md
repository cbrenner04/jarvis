...
**Merge each stage PR at its gate (operator practice, 2026-08-31).** `approve` advances the gate but does not merge the approved stage's PR, so intermediate stage PRs otherwise stack and need cleanup at the end. Land them as you approve: at `approve-intent`, **merge the intent PR** — this is not optional, because the plan workflow is designed to *move* (`git mv`) its ready-intent into the spec's `intent.md`, which only happens when the ready-intent is on the plan stage's base; if the intent PR is unmerged when plan dispatches, plan falls back to *recreating* the ready-intent in place (a workaround) and the ready-intent strands on the closed intent branch. At `approve-plan`, merge the plan PR — the implement stage's draft then retargets to `main` (the documented merge-first retarget, [Pipeline start](#pipeline-start)), which is fine. Merge the terminal implement PR last after review. Folding this into `approve` is seeded (`merge-pipeline-stage-pr-at-its-approval-gate`). Cleanup: delete this note when that seed ships.
