...
- Guard-flip derivation targets the whole negated boolean sub-expression, including member/call chains, with an `originalText` that slice-verifies — via the same TypeScript-scanner classification #3202 used for operator-flip, not a widened regex. Destructive-family derivation shares this slicing path and is in scope; operator-flip and killing-test/render-coverage resolution are not.
...
Docs: `v2/docs/operator-runbook.md` (Gate trust / mutation verification) — a `run_execution_failed` `Failed to test candidate` is a verifier-derivation crash; salvage the complete worktree by hand-publish. Remove or requalify that note if the fix makes the failure mode unreachable.
