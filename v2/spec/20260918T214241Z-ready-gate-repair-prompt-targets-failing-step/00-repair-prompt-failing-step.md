...
- [ ] Add `GATE_STEP` to `write.ready-repair`; update its render-observer test.
- [ ] Update docs.
...
- [ ] `bun run typecheck` and `bun run test` pass (root tooling `scripts/ready.ts` touched, so full suite).
