...
to its own socket, PID file, and process log, with no interference. Starting a
daemon supersedes every other digest-keyed socket under the same jarvis home:
the broadcast is best-effort and non-blocking — connect/RPC failures are ignored
and never delay the new listener or admission.
...
### Superseded daemon retirement

When a newer keyed daemon starts, it sends `supersede` to every other
digest-keyed socket. The target daemon enters **retiring** state:

- **Admission closed:** `start` (both `{ input }` and `{ steps }`) and `resume`
  reject with `daemon_superseded` before any claim, run row, or worktree
  materialization. Queued rows are not promoted.
- **Observation and steering retained:** `health`, `status`, `list`, `wait`, log
  tail, `pause`, and `kill` keep answering for runs this daemon owns.
- **Exit at idle:** Retiring is not immediate. The daemon stays up while its
  in-memory active-run set is non-empty, then exits through the same shutdown
  poll as RPC `shutdown` and SIGTERM/SIGINT — no operator action required.
  Queued and paused durable rows do not keep a retiring daemon alive; only runs
  actively executing under this process do.

Run ownership, worktree locks, agent children, and log sinks stay with the
admitting daemon and are untouched by supersession.
...
| `resume` | `{ runId: string }` | `{ ok: true }` | Resumes paused, budget-stopped, killed, or retryable-publication workflow write runs only after shared snapshot reconstruction. The matching persisted step must retain non-empty rules, artifact path, agents, model config, and resolvable bindings; the reconstructed input preserves step identity, workflow snapshot, and timeout. Missing or invalid context returns `resume_unsupported` before claim/spawn. Eligible publication retries are `completion_commit_failed`, `ready_gate_failed`, and `surviving_mutation_failed`; `ready_flip_failed` is a terminal non-resumable settlement and is rejected with `terminal_run`; `list` and `wait` expose the matching retryable reason with `nextAction: "resume"` or terminal reason with `nextAction: "stop"`. Ad-hoc stopped runs remain unsupported. Rejected `daemon_superseded` while the daemon is retiring (see [Superseded daemon retirement](#superseded-daemon-retirement)). |
| `supersede` | — | `{ ok: true }` | Flips the daemon into retiring state. Idempotent — repeated calls always succeed. Admission (`start`, `resume`) closes immediately; observation and steering (`list`, `wait`, log tail, `health`, `status`, `pause`, `kill`) keep working for runs this daemon owns. The daemon exits on its own once its in-memory active-run set is empty, reusing the same shutdown poll as RPC `shutdown` and signals. Queued rows are not promoted while retiring. |
| `shutdown` | — | `{ ok: true }` | Requests graceful daemon exit regardless of in-flight runs. Closes admission on the next poll tick and tears down once the shutdown path runs. |
