...
test("resumes failed gate_invocation_refused implement~link-N into linked subspec routing", async () => {
  const worktreePath = mkdtempSync(join(tmpdir(), "daemon-failed-linked-resume-"));
  writeFileSync(join(worktreePath, "index.md"), "- [ ] [One](./one.md)\n- [ ] [Two](./two.md)\n", "utf8");
  writeFileSync(join(worktreePath, "one.md"), "# One\n\n## Acceptance criteria\n\n- [ ] One\n", "utf8");
  writeFileSync(join(worktreePath, "two.md"), "# Two\n\n## Acceptance criteria\n\n- [ ] Two\n", "utf8");
  ...
  const runId = stateStore.createRun({
    ...
    specRef: "main",
    worktreePath,
    branch: "failed-linked/resume",
    specPath: "index.md",
    stepId: "implement~link-1",
    workflowSnapshot: {
      invocationId: "failed-linked-resume",
      steps: [
        {
          stepId: "implement",
          role: "implement",
  ...
  const logsPath = join(tmpdir(), `jarvis-failed-linked-resume-${process.pid}-${Date.now()}.jsonl`);
  let resumedInput: WriteLoopInput | undefined;
  try {
    const localHandlers = logBackedHandlers(logsPath, {
      writeLoopExecutor: async (input, _signal, _pauseSignal) => {
...
test("failed implement~link-1 whose pinned linked index entry is gone refuses resume_unsupported with a recovery clause, never dispatching the write loop", async () => {
  ...
  const runId = stateStore.createRun({
    ...
    specRef: "main",
    worktreePath,
    branch: "failed-linked/malformed",
    specPath: "index.md",
    stepId: "implement~link-1",
    workflowSnapshot: {
      invocationId: "failed-linked-malformed",
      steps: [
        {
          stepId: "implement",
          role: "implement",
  ...
  try {
    ...
    if (response.kind === "error") {
      expect(response.code).toBe("resume_unsupported");
      expect(response.message).toContain("Pinned linked subspec no longer present in index");
      expect(response.message).toContain("fix the persisted workflow snapshot or re-run the spec");
    }
    expect(starts).toHaveLength(0);
  } finally {
    rmSync(worktreePath, { recursive: true, force: true });
...
test("failed gate_invocation_refused implement~link-N resume does not restore surviving-mutation reprompt context", async () => {
  const worktreePath = mkdtempSync(join(tmpdir(), "daemon-failed-linked-mutation-reprompt-"));
  writeFileSync(join(worktreePath, "index.md"), "- [ ] [One](./one.md)\n- [ ] [Two](./two.md)\n", "utf8");
  writeFileSync(join(worktreePath, "one.md"), "# One\n\n## Acceptance criteria\n\n- [ ] One\n", "utf8");
  writeFileSync(join(worktreePath, "two.md"), "# Two\n\n## Acceptance criteria\n\n- [ ] Two\n", "utf8");
  ...
  const runId = stateStore.createRun({
    ...
    specRef: "main",
    worktreePath,
    branch: "failed-linked/mutation-reprompt",
    specPath: "index.md",
    stepId: "implement~link-1",
    workflowSnapshot: {
      invocationId: "failed-linked-mutation-reprompt",
      steps: [
        {
          stepId: "implement",
          role: "implement",
