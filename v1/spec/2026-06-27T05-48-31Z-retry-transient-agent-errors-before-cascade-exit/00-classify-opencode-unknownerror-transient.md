...
- Pin the exact `UnknownError`/500 regex phrasing against a real opencode stderr sample. Deferred to first consumer: exact stderr wording — confirm against an observed opencode failure before finalizing the pattern.
...
- [ ] A non-opencode agent's generic `UnknownError`/HTTP 500 stderr is **not** newly classified transient, and `isTransientNetworkError` git/gh classification is unchanged (scoping case in `v1/test/agents/quota.test.ts`; `v1/test/gh.test.ts` stays green).
