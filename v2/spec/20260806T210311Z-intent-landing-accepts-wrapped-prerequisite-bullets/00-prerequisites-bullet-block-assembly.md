...
- Export a shared bullet-block assembly helper from `shared/spec-parser.ts` with the same walker shape as `parseAcceptanceCriteria` (marker line plus continuations until next bullet or `##`) but parameterized by an explicit bullet-start predicate; document that continuation lines are trimmed with `.trim()` when appended (matching acceptance-criteria assembly). Checkbox (`- [ ]`) and plain `- ` markers use different predicates — rules out claiming equivalence or a second private line-walker in `intent-stage.ts`.
- Only a `- ` at line start with no leading whitespace begins a new prerequisite block; indented `- `, alternate list markers, and other non-marker lines are continuation text inside the current block. Blank lines inside a block are included like acceptance-criteria assembly.
...
- Update `validPrerequisites` to assemble prerequisite bullets via the helper with a plain `- ` predicate before validating first-line bullet shape.
