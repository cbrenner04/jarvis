...
| `status` | — | `{ state: "running", loadedRevision: string }` | Daemon-host liveness only — not run orchestration status. `loadedRevision` is the daemon's startup Git HEAD commit hash, captured once at startup and retained for the process lifetime. The `jarvis daemon status` CLI command fetches this, compares it with the invoking CLI's current Git revision, and reports matching (`running`), mismatched (`stale`), or absent (`stopped`). |
