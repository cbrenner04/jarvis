...
- [ ] A test in `shared/intent-stage.test.ts` proves a prose (non-`none`) Prerequisites body still returns the one-bullet-per-line refusal from `validateIntentStageContent`.
