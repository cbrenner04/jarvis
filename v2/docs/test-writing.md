...
Reserve a real socket round-trip for transport coverage: the `ipc.test.ts` transport suite, plus at most 1-2 round-trip smokes per handler set (one budget per exported factory — `createRunControlHandlers` and `createTailStreamHandler` each get their own) proving JSON marshaling survives the wire. Tail round-trip owners for `createTailStreamHandler` are [`daemon-tail-stream.test.ts`](../src/daemon/daemon-tail-stream.test.ts) and [`tui-log-tail-client.test.ts`](../src/tui/tui-log-tail-client.test.ts). See [`v2/src/ipc/ipc.test.ts`](../src/ipc/ipc.test.ts) for transport coverage.
