...
- [ ] A new test with explicit config fixtures asserts the resolver yields `"repo"` when `specs` is absent (old default preserved, pending subspec 01's flip), `"repo"` when `specs: "repo"`, and `"external"` when `specs: "external"`; it fails against the pre-fix code.
- [ ] A test asserts a `specs` value other than `"external"`/`"repo"` fails validation with a message naming `specs`; it fails against the pre-fix code.
- [ ] Tests assert `plan.commit` and machine `modes.plan.commit` each fail validation with a message containing `specs`; it fails against the pre-fix code.
- [ ] Existing `git: false` + `localPath` worktree-root tests (e.g. `workflow-runner-core.test.ts`, `write-loop.test.ts`) stay green — `external-worktree.ts`'s local-path selection is untouched by the resolver.
- [ ] `v2/src/execution/publication-workflow-steps.ts` resolves the intent/plan spec home through the resolver, and a test asserts a project without `specs` still publishes in-repo, matching pre-fix behavior.
- [ ] Existing `publication-workflow-steps.test.ts` fixtures using `plan.commit` or `git: false` as the spec home are rewritten to `specs`, not deleted.
