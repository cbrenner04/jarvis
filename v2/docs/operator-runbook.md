...
**Notification-sweep event-loop starvation (2026-09-03).** A pre-fix daemon could spend tens of seconds per five-second tick re-deriving every historical incident synchronously on the event loop; overlapping timer callbacks compounded the stall. Symptom: the process binds its keyed socket but IPC reads time out — `jarvis daemon status` reads `stopped` even though `lsof` shows a live listener and agent children are still writing. Retrying `jarvis daemon start` while the starving daemon is still alive starts another generation and stacks CPU spinners; it does not unblock IPC. **Before `kill -9`:** rule out a [superseded same-key daemon](#a-superseded-daemons-live-runs-are-invisible-to-the-cli-and-look-exactly-like-the-deadlock-shape-2026-09-02) (`ps ax -o pid,etime,command | grep '[d]aemon-entrypoint.ts'` shows more than one PID) and the [`daemon stop` / `run kill` deadlock](#known-gotchas) (`run list` shows `in-progress` + `not-live` with no matching live agent). When the superseded-daemon checks are negative and `ps` shows one daemon PID at high CPU with unresponsive IPC, `kill -9 <daemon-pid>` then `jarvis daemon start` is the recovery — same pattern as the deadlock shape, but here the reachable daemon truly owns the runs. Bounded derivation and non-overlapping sweeps (see [daemon-host.md § Operator notifications](./daemon-host.md#operator-notifications)) prevent this recurrence on current harness builds; restart after merging those fixes.
