...
- [x] `v2/src/commands/cleanup-cli.test.ts` — `named cleanup scopes project-owned preview and apply to one registered project`; Keystone checkpoint: its test body carries `// @mutate v2/src/commands/cleanup-cli.ts ": Object.fromEntries(Object.entries(registry).filter(([name]) => name === projectName));" -> ": registry;"` and the mutation turns the regression RED by restoring all-project cleanup.
