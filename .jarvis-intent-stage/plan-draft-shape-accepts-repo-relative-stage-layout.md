...
- [ ] `v2/src/execution/write.test.ts` test `plan-draft completion accepts nested spec/ staging and flattens before normalization` and `checkStagedPlanDraft accepts nested spec/ staging after resolve-and-flatten` stay green (no regression of #3212).
...
- `v2/docs/workflow-runner.md` — accepted plan-draft staging layouts, including repo-relative `targetDir` nested trees and flatten-before-normalize behavior.
