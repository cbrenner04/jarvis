...
- Inline seed generation, refine, and draft must enforce the same rule or the loop can re-enter through a different plan path.
- `v1/test/modes/plan/prompts.test.ts` is the prompt snapshot/test home that must move with the template changes.
- `v1/docs/spec-guidance.md` and `v2/docs/v1-behaviors.md` are the durable doc homes for this prompt-contract change.
...
- Align the durable docs with the new plan-mode rule for acceptance criteria and generated specs.
...
- [ ] `v1/docs/spec-guidance.md` documents the anti-self-reference rule for authored specs.
- [ ] `v2/docs/v1-behaviors.md` records the resulting user-observable `jarvis1 plan` behavior change so v2 parity review does not rely on stale prompt semantics.
