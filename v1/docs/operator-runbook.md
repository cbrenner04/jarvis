...
4. **Pick the 5 most important ready-intents and complete them.** For each: `jarvis1 plan <ready-intent>` to draft the spec (merge it), then `jarvis1 run <spec>` to implement (review + admin-merge). Prioritize by operator impact: gaps that force manual intervention every session, blockers, correctness/safety over polish.
