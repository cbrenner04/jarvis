...
Subspec 02 also carried the operator-runbook and parity documentation, folded in by #2573 after the two documentation-only subspecs were retired as over-split. Still missing: there is no steering — approve/reject/resume, run pause/kill, and log follow are all still CLI-only. The unattributed segment renders but has no FIFO or labelling polish.
