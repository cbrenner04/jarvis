...
External stranded and retired-worktree archival preview archive lines read `archive: plans/<name> -> plans/completed/<name>` (apply may print absolute paths under the same relative shape). Apply moves the tree to `plans/completed/<name>/` beside the source `plans/` home. Refusal stdout uses the same reasons as in-repo archival (`Skipped artifact: <source> — …`): incomplete or vacuous criteria, missing linked subspecs, matching open PR on the recorded branch, another materialized worktree on that branch, PR or ownership inspection failure, archive destination already exists, or post-move rollback after a failed in-repo ready-intent prune (external candidates do not attempt ready-intent prune).
