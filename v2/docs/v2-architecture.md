...
- **Stable log-stream boundary:** the `tail` log stream enters through the same stable public daemon and the same ownership check. A direct-predecessor-owned run's replay and follow bridge to that predecessor's private endpoint over one forwarded stream instead of reading this generation's own (incomplete) log; private endpoints always keep the local stream handler, and no predecessor configured means no ownership lookup at all. See [`daemon-host.md`](daemon-host.md#direct-owner-tail-stream-routing).
