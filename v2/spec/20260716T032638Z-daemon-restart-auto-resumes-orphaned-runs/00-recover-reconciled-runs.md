...
- Auto-resume only reconciled orphans that have a resolvable workflow snapshot/stepId; a reconciled orphan with no resumable snapshot (bare `in-progress`, non-workflow metadata) **stays** `killed` / `unsupported_resume_context` (`retryable:false`, `nextAction:"stop"`) and is never flipped to `failed`. Rules out converting the previously-safe, inspectable snapshot-less state into an irreversible `failed` on every restart before the operator can repair the snapshot.
- Mark an automatic recovery admission failure `failed` and log its diagnostic — this applies only to runs that *had* a resolvable snapshot and whose admission then failed, not to snapshot-less orphans above; rules out leaving an attempted run ambiguously `killed` with only transient startup output.
...
- Replace manual restart-orphan recovery guidance in `v2/docs/operator-runbook.md` with automatic recovery and failure handling, noting that snapshot-less orphans stay `killed` / `unsupported_resume_context` (contract preserved).
