...
- [x] A resolved `cursor` binding invokes the real Cursor CLI command shape with `adapterModel` mapped to the Cursor CLI model slug, prompt as the trailing positional argument, ignored stdin, caller `cwd`, `--workspace <cwd>`, and abort signal handling.
- [x] Cursor binding results classify as `ok`, `quota`, `model_config`, or `error` using agent-parameterized Cursor quota patterns and shared subprocess semantics, with aborts, spawn failures, and zero-exit settlements returning non-quota results.
- [x] A settled resolved `cursor` invocation emits `invocation_completed` telemetry with real `cursor` agent and raw `adapterModel` metadata.
- [x] Cursor quota in an ordered binding chain advances to the next binding, while Cursor `model_config` and generic `error` stop without trying later bindings.
- [x] Unhandled resolved agents still return the existing terminal unwired `error` result and keep metadata.
- [x] `shared/invocation/agents.test.ts` covers Cursor binding behavior and the unwired-agent fallback.
- [x] `shared/invocation/execute.test.ts` fallback and telemetry tests stay green.
- [x] `v2/docs/shared-invocation.md`, `v2/docs/write-behavior.md`, and, when needed, `v2/docs/v1-behaviors.md` describe the landed behavior without duplicating implementation detail.
