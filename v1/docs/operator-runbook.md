...
- **A `Test (v2)` (or `Test (full)`) CI step can hang with zero step progress**, from 15 min up to GitHub's 360-min job timeout (then `conclusion: cancelled`) — root-caused **jarvis-side**, 2026-07-05. Evidence: sampled the 150 most recent completed `ci.yml` runs via `gh api .../actions/runs/{id}/jobs`; 31 steps ran ≥15 min, all `Test (v2)` (12) or `Test (full)` (10, includes the v2 suite) except 9 `Test (v1)` instances clustered tightly at 16.0-16.2 min (a bounded near-threshold duration, not a stall — distinct pattern). `Test (shared)` and all `*-integration` steps showed no comparable durations (max 4.6 min). GitHub-status-page correlation was inconclusive (`githubstatus.com` not reachable from the sandbox network allowlist). Root cause: `test:v2` (agent mode, `scripts/run-v2-tests.ts`) excludes `*.sandbox-unrunnable.test.ts` (real-subprocess tests, already routed to `test:integration:v2` — not the cause here), but 60 call sites across 9 agent-mode files (`v2/src/ipc/ipc.test.ts`, `v2/src/daemon/daemon-{revise,run-failure-capture,start-list,tail-stream,wait-run-completion}.test.ts`, `v2/src/tui/tui-{daemon,log-tail}-client.test.ts`, `v2/src/cli.test.ts`) call `client.nextFrame()` over a real Unix socket with no `timeoutMs` (the API supports one; 3 call sites in `daemon-wait-run-completion.test.ts` correctly pass `2_000`). If a server-side response never arrives, that await never resolves; `scripts/run-v2-tests.ts`/`run-tests.ts` wrap the whole `bun test --parallel` invocation in `spawnSync` with no `timeout` option either, so nothing in the chain bounds it. **Fixed 2026-07-05**: `connectIpcClient(socketPath, defaultTimeoutMs?)` (`v2/src/ipc/client.ts`) takes an optional per-client `defaultTimeoutMs` that bounds `nextFrame()` calls omitting their own `timeoutMs`; left unset, those calls still wait unbounded — the original, unchanged behavior for production callers (`cli.ts`'s `wait` subcommand, the TUI's RPC/log-tail read loops) that were never implicated in the hang. The affected test files now pass `2_000` at their `connectIpcClient()` call sites, so a wedged server-side response in those suites rejects instead of hanging. **Also fixed 2026-07-05**: both `spawnSync` calls in `scripts/run-v2-tests.ts` (per-file loop, `agent` mode's `--parallel` run) now carry a `timeout` + `killSignal: "SIGKILL"` (`PER_FILE_TIMEOUT_MS` / `AGENT_MODE_TIMEOUT_MS`), so any future stall in that layer — not just the `nextFrame()` case above — fails with a named mode/file message instead of hanging the CI job.
