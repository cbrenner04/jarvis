...
describe("v2 cli", () => {
  ...
  test("run workflow intent-reviewed builds seed text with default review passes before one daemon start", async () => {
    ...
    try {
      const code = await main(["run", "workflow", "intent-reviewed", "--seed-text", "Improve API"], cap.io, {
        ...
        connectIpcClient: async () =>
          makeIpcClient([{ kind: "response", id: requestId, result: { runId: "intent-reviewed-1" } }], { sent }),
      });
      expect(code).toBe(0);
    } finally {
      crypto.randomUUID = originalRandomUuid;
    ...
    expect(sent).toHaveLength(1);
    expect(sent[0]).toMatchObject({ kind: "request", method: "start", params: { steps: FAKE_IMPLEMENT_STEPS } });
    expect(cap.read()).toEqual({ stdout: "intent-reviewed-1\n", stderr: "" });
  });
  ...
  test("run workflow implement blocks and returns exit code matching wait result", async () => {
    const cap = captureIo();
    const sent: unknown[] = [];
    const startRequestId = "00000000-0000-4000-8000-000000000007";
    const waitRequestId = "00000000-0000-4000-8000-000000000008";
    let requestIdIndex = 0;
    const requestIds = [startRequestId, waitRequestId];
    const originalRandomUuid = crypto.randomUUID;
    crypto.randomUUID = () => requestIds[requestIdIndex++];
    ...
    expect(code).toBe(0);
    expect(cap.read()).toEqual({
      stdout: `run-999\n${JSON.stringify({ runStatus: "completed", loopOutcomeKind: "complete", iterationsConsumed: 1, resumable: false })}\n`,
      stderr: "",
    });
    expect(sent).toHaveLength(2);
    expect(sent[0]).toMatchObject({ kind: "request", method: "start" });
    expect(sent[1]).toMatchObject({ kind: "request", method: "wait", params: { runId: "run-999" } });
  });

  test("run workflow implement exits with code matching failed run", async () => {
    const cap = captureIo();
    const sent: unknown[] = [];
    const startRequestId = "00000000-0000-4000-8000-000000000009";
    const waitRequestId = "00000000-0000-4000-8000-00000000000a";
    let requestIdIndex = 0;
    const requestIds = [startRequestId, waitRequestId];
    const originalRandomUuid = crypto.randomUUID;
    crypto.randomUUID = () => requestIds[requestIdIndex++];
    ...
    expect(code).toBe(3);
    expect(cap.read()).toEqual({
      stdout: `run-failed\n${JSON.stringify({ runStatus: "failed" })}\n`,
      stderr: "",
    });
  });
