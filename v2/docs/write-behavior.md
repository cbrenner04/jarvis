...
| `jarvis run workflow <name> ...` | Selects a registered workflow builder by name. Only `implement` is registered. Required flags: `--base` and `--spec`. Optional flags: `--branch` (defaults to parent directory basename of resolved `--spec`), `--artifact` (required for non-index specs, ignored for index specs). Project is resolved from the registered project containing the resolved spec path. The `implement` builder supplies `role`/`promptId`/`agents`/`agentModelConfig` and sends one IPC `start` request carrying `{ steps }` | Run ID | `0` on success; `1` on missing/unknown name, invalid flags, spec outside registered projects, or machine-config validation failure — selection, parsing, project resolution, and builder errors occur before daemon connection |
