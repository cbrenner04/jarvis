...
- Inject into `parsed.input.telemetry.operatorSessionId` only when the input doesn't already carry a `telemetry` block, preserving caller-supplied telemetry (e.g. tests) rather than overwriting it.
