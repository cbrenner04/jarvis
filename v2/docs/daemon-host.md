...
Before opening its IPC listener or constructing run-control handlers, the daemon sweeps orphaned ready-gate test process groups recorded on durable run rows. `listReadyGateSweepCandidates` selects every row with a non-null `ready_gate_pgid`, regardless of run `status`, and classifies each by `owner_identity` liveness using the same `isOwnerAlive` probe as run reconciliation and `forceKillOwnerAdmits`: a row is live only when `owner_identity` is non-null and names a still-live process; null owners and dead owners are not live. For each non-live candidate the daemon signals the recorded process group with `process.kill(-pgid, "SIGTERM")` then `process.kill(-pgid, "SIGKILL")` after the shared 50ms grace (`ESRCH` and `EPERM` are swallowed), then clears the row with `setReadyGatePgid(runId, null)` even when the group is already gone. Live-owner rows are left untouched. Dead-owner rows are signaled without age or recorded-at bounds — recycled-pgid false positives on those rows are accepted. Sweep failure aborts startup before IPC serves. Ordering: run reconciliation completes; revision and executable digest load; this sweep; then `createRunControlHandlers`; then the IPC listener opens.

Startup order after run/log reconciliation and the ready-gate sweep: the IPC listener opens; then the pipeline-continuation sweep (below) runs and is awaited to completion; then orphaned pipelines settle; then reconciled runs are admitted through `resume`; then the operator-notification sweep runs once (so restart-settled incidents are delivered); then a five-second timer re-runs that sweep for the daemon's lifetime. Health and all other IPC calls remain available while startup work runs.
...
**Delivery-ledger pull contract.** Winning ledger inserts persist the same sink stdin JSON in `incident_json` (omitted on insert leaves null). Rows with null `incident_json` — legacy key-only ledger rows from before payload persistence — are excluded from pull queries; no backfill. Sink discharge and the `notification_wait` / `notification_list` RPCs below read the same `operator_notification_deliveries` rows and share one cursor wire form: `deliveredAt:incidentId:transition`, an inclusive lower bound on `(delivered_at, incident_id, transition)`. Store query args, stable sort order, returned sink-shaped objects, and encode/decode helpers: [`state-store.md` § `listDeliveredNotificationIncidents`](./state-store.md).

**`notification_wait` / `notification_list` RPCs.** Both read the delivery ledger only — never live derivation — so a result is something the sweep already recorded and discharged. Params are identical: exactly one of `sinceCursor` (the `deliveredAt:incidentId:transition` wire form above) or `sinceMs`, plus an optional `kinds` array. Supplying both, neither, a non-array `kinds`, an **empty** `kinds`, or a non-string member is rejected `invalid_params` — an empty array is a caller error, not a match-nothing filter. Kind filtering is enforced at the RPC layer, so a non-matching delivery neither satisfies a wait nor appears in a list.

`notification_wait` returns one `{ incident, deliveryCursor }` for the first owed delivery at or after the cursor. It answers immediately when the ledger already holds one; otherwise it blocks. `notification_list` returns the ordered set of matching rows in the same shape, without blocking. `deliveryCursor` is the row's own encoded cursor, so a consumer chains the next call from the result it just handled rather than reconstructing one from sink JSON.

**Waiter wake.** The sweep wakes registered waiters after recording a delivery — on the winning insert *and* on observing a row another daemon already delivered, so a loser in the multi-daemon race still resolves its own waiters. Each wake re-runs the waiter's own filter against the store, so a woken waiter returns only a delivery it actually matches and stays blocked otherwise. Clients never poll between ticks. A waiter registered while a matching row already exists resolves on registration rather than waiting for the next sweep, closing the gap between the pre-registration check and registration itself.

Aborting the client connection settles the wait with `notification_wait aborted` and unregisters it; no `daemon_superseded` guard applies, so a wait armed against a retiring daemon keeps answering the way `run wait` does.
...
**Incident altitude.** Notifications name derived operator boundaries — pipeline awaiting-approval, pipeline terminal (with cause), publication failure, wedged `settlement_deferred` stages, failed fan-out lanes on live pipelines, blocked runs, budget-soft-stops, ad-hoc workflow terminals — not every raw step-run terminal inside a pipeline stage failure.

**Pipeline stage incidents.** `collectPipelineIncidents` derives stage-scoped incidents beside pipeline-level boundaries. A `running` stage row with redrivable deferred settlement emits `kind: "stage-settlement-wedged"` with `transition: "settlement_deferred:entry_run_dead"`, naming `stageId` and `branchKey`. On non-terminal pipelines, each `failed` stage row emits `kind: "stage-failed"` with `transition: "failed"`, also naming `stageId` and `branchKey` — a fan-out lane that settles `failed` while sibling lanes keep the pipeline live surfaces its lane explicitly; that stage incident suppresses the failed stage's entry-run terminal incident. When derived pipeline state is terminal, failed stages contribute through pipeline-level terminal incidents and entry-run suppression only — no duplicate `stage-failed`. `stage-settlement-wedged` and `stage-failed` are mutually exclusive on one stage row.
...
Callers supply `socketPath` explicitly. There is no production default or max concurrent client cap in the library. Startup distinguishes **liveness** (whether a connect probe sees an accepting peer) from **occupancy** (whether the path is already taken at bind time). Liveness alone does not authorize removal: `ENOENT` classifies absent and removes nothing, because a caller that cannot resolve the path receives `ENOENT` for a socket a daemon is actively serving and deleting on that false negative strands the running daemon. `ECONNREFUSED` classifies stale and is unlinked before bind. A connect that succeeds, or a probe timeout after 250ms, classifies live; a peer that accepts is refused with `DaemonSocketInUseError` rather than unlinked. When the initial probe times out without a connect, a longer reprobe (2s) runs before bind; if that reprobe still times out, bind proceeds without refusing — occupancy is decided at `listen`. Bind order is: initial probe → longer reprobe on timeout-class live → stale-only removal → `listen` → on `EADDRINUSE`, one post-bind reprobe: unlink and retry bind once only when the reprobe returns stale; reprobe `absent` never authorizes unlink (the live-peer-behind-absent-probe guard). Unlinking a live peer's socket leaves that daemon holding a bound-but-unlinked inode: it keeps serving established connections while every later `connect()` resolves by path and fails `ENOENT`, so the CLI reports `stopped` for a healthy daemon and every run it owns becomes unreachable. The CLI, [`jarvis tui`](./write-behavior.md#tui-cli) (monitor over the production socket; dock `start` admits via `pipeline_start`, not IPC `start`), [`jarvis tui log <run-id>`](./write-behavior.md#tui-cli) (IPC tail consumer over the same socket), and daemon lifecycle commands key socket, PID, and process-log paths by the invoking executable digest: derived from the SHA-256 digest of tracked blobs under `v2/src/**`, `shared/**`, and repo manifests, with a 16-hex-char leading slice to stay within the macOS `sun_path` limit (104 bytes). The keyed path format is `~/.jarvis/daemon-<key>.sock`, `~/.jarvis/daemon-<key>.pid`, and `~/.jarvis/daemon-<key>.log`. This allows multiple daemons keyed by different executable digests to coexist: each connects to its own socket, PID file, and process log, with no interference.
...
`jarvis cleanup` probes only exact lowercase keyed socket names and reaps the `.sock`, `.pid`, and `.log` lifecycle unit only after an immediate `ECONNREFUSED` revalidation, or `ENOENT` on a present on-disk socket file with no listener; `ENOENT` when the path is absent to the caller and ambiguous probes preserve the triplet. See [`operator-runbook.md` § Fail-closed daemon reads and digest artifact reaping](./operator-runbook.md#fail-closed-daemon-reads-and-digest-artifact-reaping).
...
Fresh `{ steps }` workflow starts, daemon pipeline dispatch (`defaultPipelineDispatch` → `handleWorkflowStart`), and `pipeline_recover` (after recovery-specific validation and effect-free target resolution) all enter `admitWorkflowStart` in `daemon-workflow-admission-handlers.ts`. That boundary reclaims stale workflow registry claims, applies the same queued/live ownership and memory checks as standalone workflow `start`, acquires the linked `(project, branch)` registry claim and an `activeRuns` entry, then calls lifecycle-specific durable admission. A lifecycle refusal or exception before execution begins rolls common acquisition back; admitted lifecycles retain distinct execution and settlement — workflow runs stay `kind: "workflow"` through `startWorkflowRun`, while recovery stays `kind: "recovery"` until detached attempt, settlement, and continuation finish and its log closes.

## Run-control handler modules

`createRunControlHandlers` in `daemon.ts` wires four modules through one `RunControlHandlerContext` (transport contracts stay in the table above).

| Module | RPC family | Context / deps |
| --- | --- | --- |
| `daemon-run-control-context.ts` | (shared seam) | `createRunControlHandlerContext` |
| `daemon-run-lifecycle-handlers.ts` | `start`, `list`, `pause`, `resume`, `kill`, `wait`, `dismiss`, `undismiss` | context; `handleWorkflowStart`, `pipelineDispatch`, `pipelineWait` |
| `daemon-workflow-admission-handlers.ts` | `check_workflow_start_claim`, `implement.recover`; exports `handleWorkflowStart`, `admitWorkflowStart` | context; `resumeFinalizationOnly` from lifecycle |
| `daemon-pipeline-handlers.ts` | `pipeline_*`; startup `continueContinuablePipelines` | context; pipeline dispatch/wait, `admitWorkflowStart`, stage resolve, recovery/publication/stale-reset injectables |
...
**Git-disabled chained plan artifacts.** When a plan stage published to the Jarvis-owned external plan home, its recorded artifact is a directory under `~/.jarvis/specs/<projectSafeId>/plans/<name>/`. `resolveChainedImplementSpecPath` normalizes that to the tree's `index.md` (absolute paths are used as-is rather than re-joined onto the prior worktree), and `resolveImplementStage` then resolves the same external identity standalone implement uses — `resolveExternalPlanSpecIdentity` — stamping `externalPlanSpec: true`, `absoluteSpecPath`, and `specReadRoot` on the implement input, with `projectRoot` / `projectName` taken from the registered owner and publication `baseRef` staying the default branch. It does **not** set `preflightGitRoot` / `preflightBaseRef` for that case, so the non-Git plan workspace is never subjected to a `baseRef` membership check. Chained stages whose prior workspace is an ordinary Git worktree keep the existing `preflightGitRoot` / `preflightBaseRef` path unchanged. A complete external tree returns `implement.already_complete` at stage resolution, before any implement worktree or workflow steps are created. The admission predicate, canonical paths, and execution boundary are owned by [workflow-runner.md § External plan implement admission](workflow-runner.md#external-plan-implement-admission) — not restated here.

**External ready-intent downstream-input resolution.** When the prior entry-run worktree is absent or unusable, chained plan resolution for a listed `downstreamInputs` path or single-file `specPath` under `ready-intents/` walks ordered absent-worktree fallbacks: admission-root filesystem presence, git-branch rematerialization when the path exists on `prior.branch`, then external `~/.jarvis/specs/<projectSafeId>/ready-intents/` when inverted `effectivePublishGit` is false (project `plan.commit`, then machine `modes.plan.commit`, then `true`). External acceptance reuses the `jarvisHome()/specs/` containment gate chained external-plan implement applies; owner-safe-ID containment refuses paths under another registered project's external home. The external `ready-intents/` directory (or the file's parent for a single listed file) is the `readRoot` — no git worktree rematerialization for external-only artifacts. Effective publication routing and standalone ready-intent admission predicates are owned by [workflow-runner.md § Authoring helper and presets](workflow-runner.md#authoring-helper-and-presets) — not restated here.
...
Seed/artifact hand-off: the first workflow stage (by authored position) builds with admitted `PipelineContext.seedPath` as file `seed` or inline `PipelineContext.seed` as `seedText` (never both; `seedPath` wins when both are stored) and `PipelineContext.cwd` as its read root. Every later workflow stage builds from the immediately preceding workflow stage's recorded artifact (approval stages are skipped when walking back to find it): resolution loads the prior stage's entry run via `store.loadRun(artifact.entryRunId)` and sets preset `cwd` to that run's `worktreePath`. Artifact `specPath` is worktree-relative for intent/plan handoff: `readyIntent` for `plan`/`plan-reviewed*` presets; chained implement normalizes directory `specPath` to `<dir>/index.md` in `resolveImplementStage` (see [operator-runbook § Pipeline start](./operator-runbook.md#pipeline-start)). Never joined to admission `cwd` and never absolutized in the store. When the preceding artifact carries `downstreamInputs` with length ≥ 2, the first chained stage after that splitting artifact resolves one preset per listed file path (each bound as `readyIntent`); length 1 binds that path only; absent `downstreamInputs` keeps file `specPath` single-resolution. When the prior worktree is present, each listed path must exist there; when it is absent or unusable, resolution uses the ordered absent-worktree fallbacks above — never directory `specPath`. Later chained stages (e.g. implement after per-branch plan) resolve from the branch-local preceding artifact only and do not re-iterate intent `downstreamInputs`. Chained plan and implement set worktree/publication `baseRef` to the repository default branch via `getBaseBranch(context.cwd)`, not `prior.branch`; `prior.branch` remains for artifact rematerialization and implement preflight spec-availability (`preflightBaseRef` on `preflightGitRoot`). Chained implement routes linked-index reads and writes against `specReadRoot` on the prior worktree (preset build may emit absolute `specPath` and absolute `expectedArtifactPath`); before completion publication, `landImplementSpecTreeFromReadRoot` copies the routed spec tree into the default-branch implement worktree so index ticks and subspec edits appear on the implement draft PR. In-flight pipelines admitted under stacked `baseRef` may still hit publication-time base retarget when the requested base vanishes on `origin` (`git ls-remote --heads` empty or errored); settlement records `requestedBase` and `resolvedBase` on the stage artifact when publication succeeds after retarget, or on `failureDetail` when publication still fails. New admissions do not stack on the prior stage branch. The `fast` integration case is the inter-stage worktree handoff proof: it seeds every stage artifact on real stage worktrees and asserts the ready-intent and plan spec tree are absent from the operator checkout.
...
- Any other rollup status (`failed`, `blocked`, `killed`, `interrupted`, `paused`, or anything else the wait primitive returns) records `status: "failed"`, `endedAt`, and a `failureDetail` — never an artifact reference — when the linked entry run is no longer live. While the entry run is still live (`isLiveEntryRun`, including `paused` and other non-terminal durable statuses such as `budget-soft-stopped`), settlement declines to terminalize: the stage row stays `running` without `endedAt` and records deferred `failureDetail: { code: "settlement_deferred", reason: "entry_run_still_live", entryRunId, rollupStatus }` until a later adopt/settlement attempt succeeds. `pipeline_list` surfaces that deferred detail on `running` rows as stored. Re-settlement is driven by existing adopt/continue/recovery paths when the linked entry run is no longer live: `continuePipeline` and `adoptRunningWorkflowStage` re-invoke adopt/settlement (`adoptAndSettlePipelineStage` / `settlePipelineStageFromEntryRun`) only while holding durable stage admission; a refused admission re-reads and returns without settlement. `pipeline_resume` drives the same re-settlement on a live daemon when derived `running` reflects exactly this wedge (`resumeDrivesDeferredSettlement`); it does not re-invoke adopt/settlement for other derived-`running` shapes, which still refuse `pipeline_not_resumable`. Success (`rollupStatus === "completed"`) settlement is unchanged: it still terminalizes even when the entry run row reads live until the success patch lands. When the entry run row is present and settlement is not deferred, `failureDetail` mirrors the full `composeRunOperatorError` result (`reason`, `retryable`, `nextAction`, and optional detail fields) from the settled entry run with its terminal log context (`findTerminalLogRecord` on the entry run's log tail loaded by the daemon's `logReader`); when the entry run row is absent, a hand-built `{ reason, retryable, nextAction }` harness-failure shape is used. The daemon `wait` RPC awaits an in-flight workflow promise when one is registered, then reports rollup status from durable sibling rows; after restart or on the adopt path, `wait` may resolve a non-`completed` rollup from durable non-terminal entry-run state without awaiting — settlement re-checks liveness before terminalizing on that rollup rather than trusting it alone.
...
**Durable `pipeline_stage_admission` (cross-continuation):** dispatch and adoption atomically claim the same durable `pipeline_stage_admission` partition keyed by `(pipelineId, stageId, branchKey)` through `StateStore.claimPipelineStageAdmission` before dispatching steps or waiting on an existing linked entry run. The claim is held until partition completion — entry-run settlement or the existing live-entry early-exit paths — and released through `releasePipelineStageAdmission` with matching holder identity; release never runs while `wait()` is outstanding. A refused dispatch or adoption claim re-reads the stage and linked entry-run rows, then returns without dispatch, settlement, or a terminal stage patch. This coordinates overlapping `continuePipeline` callers across processes and daemon restarts. Distinct from in-memory `dispatchClaims` in `pipeline-execution.ts`, which coordinate fan-out siblings within one `runPipeline` invocation only. After daemon restart, a held `pipeline_stage_admission` row may remain when the owning entry run did not settle and release did not run; clear the row or wait for settlement/release before expecting another dispatch or adoption for that stage partition.
...
`derivePipelineState` is the sole aggregate source for in-flight continuation eligibility: `isPipelineContinuable`, unscoped `resumePipeline`, and `recoverContinuablePipelines` read that derivation instead of re-walking stage rows with parallel precedence. Branch-scoped `resumePipeline` keeps `resolveBranchResumeAdmission` as the pinned exception (see [Pipeline stage-scoped resume](#pipeline-stage-scoped-resume)).

`isPipelineContinuable` returns true when `isPipelineSettlementPending` is true (every authored stage satisfied but terminal publication has not succeeded) regardless of derived `pending`, so restart can finish never-attempted settlement. Otherwise it requires derived `pending` plus `approvalOutcomePermitsActivation` (no `awaiting`/`rejected` approval rows) and `reopenedFailurePermitsActivation` (no remaining `failed` rows). Approved gates with a pending workflow successor and reopened failed continuations both satisfy these guards when derived state is `pending`.

### Pipeline approval decisions

`pipeline_approve` and `pipeline_reject` target one authored `stageId` under a `pipelineId`, optionally scoped by `branchKey` when multiple branch rows exist. The handler resolves that `stageId` (and `branchKey` when supplied) to a single durable stage row, then admits the decision through `StateStore.commitApprovalDecision` on the row's stable `PipelineStageRecord.id` — never by pipeline ID alone. When multiple non-`skipped` branch rows exist at the stage and `branchKey` is omitted, the handler refuses with `branch_key_required`. Missing or empty `pipelineId`/`stageId` → `invalid_params`. A retiring (superseded) daemon rejects both methods with `daemon_superseded`, matching other mutating pipeline RPC retirement.
...
`pipeline_resume` is the sole daemon-owned stage-scoped resume entry point. It composes `derivePipelineState`, `reopenFailedPipeline`, `claimPipelineContinuation`, and `continuePipeline` in `pipeline-execution.ts` — never translating resume into `pipeline_start` or run-level `resume`.
