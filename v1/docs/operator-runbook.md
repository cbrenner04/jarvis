...
- The intermittent `daemon-wait-run-completion.test.ts` staller (#1170, #1171) was root-caused to a leaked Linux inotify `FSWatcher` in `FsAppendWake` (`v2/src/persistence/log-stream.ts`) and **reduced** by `.unref()`-ing every watcher and abort-poll timer it creates (#1191). It is **not fully eliminated** — a residual intermittent per-file timeout on this file still recurs on Linux CI (observed on #1204, whose diff does not touch that file; a same-branch re-run went green). Recovery meanwhile: re-run the `Test (v2)` job; a same-commit green confirms the flake, not a regression. The structural fix is seed `v2-source-simplification` (11), which deletes the `FsAppendWake` watcher entirely and replaces the wait fanout with a per-waiter poll-only follow — its acceptance must confirm this file stops intermittently timing out.
