...
describe("daemon (real process)", () => {
  test("startDaemon spawns detached process that serves health", async () => {
  ...
  test("getDaemonStatus reports running for live daemon", async () => {
    const metadata = await startDaemon(SOCKET_PATH, { pidPath: PID_PATH });
  ...
  test("getDaemonStatus reports stopped after stopDaemon", async () => {
    const metadata = await startDaemon(SOCKET_PATH, { pidPath: PID_PATH });
  ...
  test("WorktreeOwnershipRegistry claim and release", () => {
