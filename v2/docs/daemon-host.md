...
Callers supply `socketPath` explicitly. Each CLI invocation derives one
executable-tree digest and uses `/tmp/daemon-<digest>.sock` plus matching keyed
PID, process-log, durable-state, and run-log paths. The full digest remains the
identity; the short socket path fits macOS limits. All CLI IPC and lifecycle operations use that key;
the legacy `daemon.sock` is neither probed nor changed.
...
| `status` | — | `{ state: "running", loadedRevision: string, loadedExecutableDigest: string, recovery: { pending: boolean, reconciled: number, resumed: number } }` | Daemon-host liveness only. The keyed socket selects the executable-compatible daemon; status never gates dispatch or starts a bounce. Recovery is confined to that daemon's keyed durable state. |
...
**CLI default:** The CLI pins keyed process logs alongside the selected daemon
