...
| `supersede` | — | `{ ok: true }` | Idempotent signal that a new daemon has taken over for this executable digest. The daemon closes admission (`start` and `resume` reject with `daemon_superseded`), stops promoting queued runs, but continues to serve `health`, `status`, `list`, `wait`, log tail, `pause`, and `kill` for runs it owns. On-flight runs continue to normal completion. The daemon exits autonomously once all in-flight runs settle and there are no active runs remaining (see [Retiring daemon lifecycle](#retiring-daemon-lifecycle)). Always returns `{ ok: true }`, even if already superseding. |
...
### Retiring daemon lifecycle

When the `supersede` RPC is called, the daemon transitions to a retiring state:

- **Admission closure:** `start` (both write-loop and workflow forms) and `resume` reject with `code: "daemon_superseded"` and `message: "Daemon is retiring and not accepting new work"`. No durable run row, worktree claim, or materialized worktree is created for rejected calls.
- **Observation and steering retained:** `health`, `status`, `list`, `wait`, log tail streams, `pause`, and `kill` continue to answer for runs the daemon owns, unchanged.
- **Queued promotion suppression:** Queued runs are not promoted while retiring, ensuring the daemon doesn't pull new work while trying to exit.
- **Idle exit trigger:** Once all in-flight runs settle and the active-run set becomes empty, the daemon autonomously initiates shutdown (the same path as `shutdown` RPC or SIGTERM/SIGINT). Process exit goes through an injectable seam on `DaemonStartupDeps` (defaults to `process.exit`).
- **Idempotence:** The `supersede` RPC is idempotent; multiple calls return `{ ok: true }` without error, and the retiring state does not change or revert.
- **Run ownership:** Run worktree locks, agent child processes, and log sinks remain with the daemon that admitted them; `supersede` does not affect ownership, only admission.
