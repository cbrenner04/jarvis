...
**Pipeline stage dispatch:** after stage resolution and any stale-reset preflight, `advanceWorkflowStage` stamps resolved workflow steps through the shared `stampWorkflowStepsWithMachineConfig` export from the pipeline admission `configPath` before `dispatchPipelineStage`; fan-out branch dispatch uses the same stamp. Stamped write steps carry resolved `fixCommand`/`readyCommand` (when configured), write-path iteration bounds (`iterationTimeoutMs`, `iterationCeilingMs`, `idleOutputMs` when armed), and review steps carry `roleTimeoutMs` and configured `idleOutputMs` — the same layer CLI `run workflow` applies via `prepareWorkflowSteps`. Unconfigured project commands stay unstamped on the step object and still resolve to `bun run fix` / `bun run ready` at execution.
