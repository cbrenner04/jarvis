...
- [x] `daemon-pipeline-observation.test.ts` — inverting the branch-row projection guard or the `awaiting-approval` `branchKey` guard makes the two-branch regression fail; negative cases prove collapsed projection and anonymous boundaries are absent.
