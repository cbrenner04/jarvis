...
No raw attempt transcripts or exit codes appear in this contract. `invocation_error` may include the persisted bounded final-binding stderr tail as `message`.
...
| `message` | Optional bounded diagnostic for `ready_gate_failed` terminal command evidence or the final-binding stderr tail on binding-chain `invocation_error` |
