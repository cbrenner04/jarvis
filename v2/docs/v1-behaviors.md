...
- Plan prompts are intentionally data-delimited with sentinel delimiters (e.g., `<<<INTENT_BEGIN>>>`, `<<<INTENT_END>>>`) so intent/spec text is treated as content, not executable instructions. Template rendering is non-recursive: placeholders in the template source are substituted exactly once, and literal placeholder-looking text in injected values (e.g., `<SPEC_GUIDANCE>` appearing in intent) is preserved as data without recursive substitution. This allows spec-governance and prompt-documentation content to reference exact placeholder names. Sources: `v1/src/modes/plan/template-renderer.ts`, `v1/src/modes/plan/intent-draft.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/review.ts`, `v1/src/commands/plan.ts`
...
- Plan `commit: true` mode creates a dedicated `plan/<name>` branch and `.worktree/plan-<name>` worktree, opens/updates a draft PR after the fresh-run refine handoff (or after `plan: draft` on resume paths), rewrites PR header/footer on each plan commit, and attempts auto-ready transition only when an open PR is still draft. `ensureDraftPr` scopes to the current branch's open PR only (closed/unrelated PRs are not reused). Sources: `v1/src/commands/plan.ts`, `v1/src/worktree.ts`, `v1/src/modes/plan/pr.ts`, `v1/src/pr.ts`
...
- Plan-mode invocation rows use `mode: "plan"` with `plan_phase` (`intent`, `refine`, `name-only`, `draft`, `review`). Intent and refine rows set `outcome` to a terminal state after harness validation (`success`, `refined`, `skip`, `blocker`); failed attempts omit `outcome`. Draft/review/name-only rows are unchanged. Sources: `v1/src/telemetry.ts`, `v1/src/modes/plan/plan-telemetry.ts`, `v1/src/modes/plan/intent-draft.ts`, `v1/src/modes/plan/refine.ts`, `v1/docs/plan-mode.md`
