...
**When codex is depleted and the operator is a Claude session, prefer cursor for impl runs (observed 2026-07-11).** With codex out, patch primary falls to claude-haiku, which shares the Claude pool with the operator's own orchestration session and stalled to a zero-output iteration-timeout (exit 8) — repeatedly, even serially. `jarvis1 run --agent cursor:"Composer 2.5" <spec>` sidesteps it entirely: cursor is a separate pool, so it neither contends with the operator's Claude session nor depends on codex quota, and it carried every impl run it was given to completion that day. Reach for the cursor override the moment a claude-haiku patch run stalls with no output.
