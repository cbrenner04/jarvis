...
- A source-line equivalent-mutation directive carrying the verifier-generated mutation string and a non-empty reason accepts that candidate without requiring a vacuous killing test.
...
- Pin one source-line directive grammar whose payload contains the exact generated mutation string plus a non-empty reason; rules out reason-only comments and parser-dependent guesswork.
...
- [ ] A verifier regression applies the exact directive to a changed guard, receives a pass with the accepted file, line, mutation, and reason, and fails against the pre-fix verifier that reports the mutation as surviving.
...
- `v2/docs/workflow-runner.md` — directive contract, exact-site scoping, accepted-site result, and restructure-first ordering.
