...
- **Idle-timeout escalation (patch implementation and review actuator)**: when the idle-output watchdog fires and at least one later rung remains, the current agent is shifted and the next entry retries. Patch implementation uses `modes.patch.agentOrder`; review actuator uses `modes.patch.subRoleAgentOrder.reviewActuator` (falling back to `modes.patch.agentOrder`). The advance emits `<agent>: idle timeout; escalating to next agent` (review actuator prefixes with `review:`). Patch implementation returns exit `8` with `watchdog-idle-timeout` only after the final rung stalls (or on fix-up iterations, which do not escalate). Review actuator terminal idle exits `11` (`review-incomplete`). Shrink, plan, review debate, and wall-clock timeouts stay terminal with no cascade.
