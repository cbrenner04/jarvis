...
- [ ] A shared template-repo helper exists under `v2/src/testing/` with a co-located unit test proving two committed-mode copies are independent repos sharing the template's HEAD commit and content.
- [ ] The committed-mode unit test also proves a copy keeps the template's git user identity, so a commit made in the copy succeeds.
- [ ] The uncommitted mode has a unit test proving a copy has an initialized `.git` with identity configured and no commits.
- [ ] `v2/src/execution/intent-output.test.ts` `createRepo()` uses the helper's committed mode; no per-test `git init`/`config`/`commit` remain in it.
- [ ] `v2/src/execution/intent-output.test.ts` stays green.
- [ ] `v2/src/execution/intent-output.test.ts` test count is unchanged or higher versus the merge base.
