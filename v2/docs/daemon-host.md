...
durably created — gets a `workflow`-kind entry keyed by that step's `runId`.
Healthy workflow entries remain non-pausable and non-killable: `pause` and
`kill` are rejected with `run_not_active`. If `list` reports a workflow run as
live but marks it reapable because it is wedged, `kill` accepts the same `runId`
and records durable `killed`, retaining the worktree. A later `list` reports
that run with `isLive: false`, keeping list and kill coherent. Unknown and
non-live run IDs remain rejected with `run_not_active`.
