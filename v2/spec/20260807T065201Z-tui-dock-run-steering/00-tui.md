...
- [ ] `tui-entry.test.tsx` test `typed resume-run re-issues wait after successful resume` observes `rewaitOnSuccess` (wait state returns to pending or equivalent after resume RPC) and fails against the pre-fix code.
