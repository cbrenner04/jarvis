...
`failed`, `blocked`, `interrupted`, and `killed` — ordered by `created_at`
descending with `rowid` as a tiebreak. All other statuses (`in-progress`,
`queued`, `paused`, `budget-soft-stopped`) are exempt: they are always
...
invocation-sibling retention are skipped. Filters apply conjunctively to durable
store rows before per-row assembly. Matching rows are returned newest-first
(`created_at DESC`, `rowid DESC`), then capped to `limit` when provided or
**200** when omitted. `sinceMs` means `created_at >= sinceMs`; `project`,
`branch`, and `specPath` match the durable columns exactly (case-sensitive);
`status` matches the durable row `status` (one terminal status from
`TERMINAL_RUN_STATUSES`). A dimension-only query (any of `project`, `branch`,
`specPath`, or `status` without `sinceMs`) still uses this filtered path.
Durable rows are still not deleted; only the response is filtered.
