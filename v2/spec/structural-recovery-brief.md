...
**Parallelization (measured 2026-09-06).** Seven concurrent lanes ran at load 6-21 with zero watchdog false-kills — the lane count is not the ceiling. The two real ones are gate-shaped: a `shared/**` implement's gate runs all six slices and cannot share the machine, and `ready_gate_out_of_scope` can be manufactured by load when the base-ref probe flakes the test it is checking (terminal `stop`, so it strands healthy work). Attribute by process family before believing any ceiling; never run local suites beside a live gate. Approve fan-out gates serially: head lane first, land, then resume ([[plan-bases-off-a-declared-prerequisite-branch]] + [[pipeline-fan-out-lanes-serial-chained-bases]] are the structural answer). **Do not merge while lanes are live** — every lane branched pre-merge is stale-based and its review/repair steps rewrite files the merge landed.
