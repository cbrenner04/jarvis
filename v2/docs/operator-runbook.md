...
Failed `plan` resume owns the harness preamble — shared stale reset, base alignment, reserved harness-blocker clearing, and dispatch — without a preceding manual branch cleanup, base merge, or staged-blocker edit. When stale reset retires an existing lane worktree, stderr prints `failed plan resume worktree disposition: retired-and-rematerialized from base` before writer dispatch; when stale reset is a no-op (no materialized worktree to retire), stderr prints `failed plan resume worktree disposition: reused existing worktree`. Harness-only `.jarvis-plan-stage/` draft dirt auto-clears through that preamble; uncommitted paths outside `.jarvis-plan-stage/` refuse before destructive retirement. Branch-scoped resume resets and dispatches only the named lane — sibling approval gates and stages stay untouched. Reset policy survives detached continuation, claim loss, and daemon restart via a durable row marker until dispatch completes. The worktree is preserved and the stage fails without dispatch when a live run/worktree owns it, staged `intent.md` still carries a non-reserved operator `## Blocker` (reserved harness `Artifact contract check failed:` sections are cleared with draft dirt), stale-reset preparation cannot run, `HEAD` is not descended from base, or landed acceptance criteria differ from base. Resolve the named state first; reserve `jarvis cleanup --abandon <branch>` for those preserved refusals or partial teardown, and use `pipeline recover` instead when the corrected staged tree should be landed rather than discarded.
