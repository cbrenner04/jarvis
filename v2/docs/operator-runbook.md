...
| `jarvis run list --since <duration\|timestamp>` | History query past the default fifty-terminal-run window; duration units `d`/`h`/`m`/`s` (e.g. `2d`, `90m`) or absolute Unix ms / ISO 8601; optional `--limit <n>` caps newest matches (default **200** when filtered) |
| `jarvis run list --limit <positive-integer>` | Optional explicit cap on filtered history queries; compose with `--since`. Bare `--limit` alone keeps the live fifty-terminal retention window |
...
Use `jarvis tui` for the live window. For terminal runs older than the default `run list` retention window, query history with `jarvis run list --since 2d` (or `90m`, `2026-07-01T00:00:00Z`, etc.); add `--limit 50` to cap results or rely on the default **200**-row bound on filtered queries. Returned run IDs work with `run log` and `tui log` on the same daemon.
