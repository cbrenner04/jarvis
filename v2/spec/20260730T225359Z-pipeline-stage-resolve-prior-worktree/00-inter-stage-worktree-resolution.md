...
- Chained implement derives `baseRef` from the prior entry run rather than looking it up from a working directory; the concrete value and implement preflight root are subspec 01 — rules out base-ref lookup on `PipelineContext.cwd` while `cwd` points at the plan worktree.
