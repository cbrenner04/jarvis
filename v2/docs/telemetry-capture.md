...
`exit_reason` and each `warnings` entry are capped at 4096 chars (`TEXT_FIELD_CAP_CHARS` in `shared/invocation/execute.ts`) at record-build time — cursor quota exits can embed multi-megabyte transcripts in `stderr`. A field over the cap becomes a `` `[truncated, dropped ${droppedChars} chars] ` `` marker followed by the field's tail, marker + tail together equal to the cap (marker counts toward it; the tail, not the head, is kept since the actionable quota/error line is usually at the end). `warnings` array length and under-cap entries are unaffected. For `exit_reason`, the full untruncated `stderr` text is separately written to the session log (when one is attached and the write succeeds) under the `inbound_stderr` tag — join it to the telemetry row by `run_id` plus the session-log open timestamp embedded in its filename (`<namespace>-<timestamp>.log`), not `attempt_id`. That backstop is best-effort (a session log is optional and its append silently no-ops on failure) and time-bounded to the log's cleanup window (default 14 days, `cleanup.sessionLogRetentionDays`) — not permanent. `warnings` text has no equivalent full-text backstop: `logBindingInbound` never writes `warnings` to the session log.
