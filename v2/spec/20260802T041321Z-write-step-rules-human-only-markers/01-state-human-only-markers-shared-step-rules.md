...
- Cover each rendered consumer: v1 patch `patch.prompt.body` and `patch.prompt.shrink`; v2 legacy
  `write.execute`, workflow implement `patch.prompt.body` / `patch.prompt.shrink`, ready repair
  `write.ready-repair`, mutation repair `write.mutation-repair`, workflow intent
  `intent.prompt.split`, and workflow plan `plan.prompt.draft` step completion.
...
- [x] Rendered `patch.prompt.body` coverage in `v2/src/execution/write.test.ts` fails against the
      pre-change rules and passes after its isolated `STEP_RULES` section independently names all
      supported human-only marker strings, case-insensitive substring recognition, and free
      placement anywhere in the full bullet block.
- [x] `v2/src/execution/write-prompt.test.ts` fails against the pre-change rules and passes after an
      ...
      shared constant.
- [x] `v1/test/prompt.test.ts`, `v1/test/modes/patch/shrink.test.ts`, and
      `v1/test/prompts/rendered-snapshots.test.ts` pin the v1 patch body, shrink, and refreshed
      fixture text from the shared rules, with focused assertions scoped to `STEP_RULES` rather
      than another patch injection.
- [x] `shared/prompts/intent-split.test.ts` and `shared/prompts/plan-draft.test.ts` independently
...
- [x] `v2/docs/write-behavior.md` and `v2/docs/v1-behaviors.md` describe the shared write-step
      contract and its v1 patch plus v2 intent, plan, legacy write, implement, ready-repair, and
      mutation-repair consumers, consistently with the parser's existing full-bullet,
