...
The implement stage now flips its own PR ready and emits a per-lane `stage-succeeded` incident (#3970), so the ready half is served. What remains is the per-lane `merge` terminal action and the spurious pipeline-level `failed` (`pipeline-execution.ts` still commits `multi-branch terminal publication is not defined for fan-out pipelines`).
...
- Superseded-PR closing ([[superseded-pipeline-pr-hygiene]]) applies per lane to that lane's plan PR; the shared intent PR closes only after every lane's publication succeeded. Rules out closing the intent PR while a sibling lane still needs it.
...
- `v2/docs/pipeline-execution.md` — branch fan-out execution and terminal publication; remove the "not defined for fan-out" boundary.
