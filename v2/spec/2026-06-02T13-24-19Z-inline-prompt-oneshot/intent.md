...
- Dedicated agent/model config lives at `modes.prompt.agentOrder`; reject a top-level `prompt` config block to preserve the existing mode-scoped config shape and tooling.
- `jarvis1 --prompt` requires effective `git: true`; reject `git: false` or `--cwd` execution because the command contract includes worktree, commit, push, and PR side effects.
- `jarvis1 --prompt` resolves its target repo from `--repo` when provided, else from the current working directory using the existing registered-project/ad-hoc-git rules; reject inferring the repo from prompt text or last-used-project state.
- Harness owns commit, push, and PR creation; reject delegating `git`/`gh` side effects to the agent so attribution, PR shape, and failure handling stay deterministic.
