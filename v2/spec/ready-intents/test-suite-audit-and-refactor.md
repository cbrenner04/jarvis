...
- The determinism convention (agent-runnable tests: no real process spawn / wall-clock) is merged (flaky-tests-serial-retry-and-determinism) as the refactor target.
- ~32 `*.test.ts` files use real-process/timing primitives (the audit's starting inventory).
- The suite is currently green and must stay green throughout (mechanical, correctness-preserving).
