...
`--target-dir` also gates seed *input*, not just spec output: `jarvis1 intent`/`plan` on a file seed requires the seed to live under `<targetDir>/seeds/`, so a seed under `v2/spec/seeds/` needs `--target-dir v2/spec` (or the default) — passing `v1/spec` is rejected with `raw seed files must live under v1/spec/seeds/`.
...
- **Quota depletion by your own fan-out is acceptable when the work is useful.** A fan-out leading every run with the same account the operator session uses drains that quota and cascades down the agent order — self-inflicted, not another operator. Don't artificially reorder agents or cap concurrency just to preserve quota (owner guidance, 2026-06-26).
...
**Observed actuator tiers (2026-06-29, claude-sparing config).** cursor (Composer 2.5) is a solid primary — plans + implements most specs solo — but can stall (idle-watchdog exit 8) or blow the 30-min iteration timeout on a complex spec. **Correction (2026-07-17): cursor is NOT free.** It is a paid subscription like the others; its cost is merely **untracked** (Jarvis records `cost_usd: null` / `usage_source: unavailable` for cursor, same hole as codex, with no recovery procedure — seed `cursor-usage-is-untracked`). Do not treat cursor as cost-free when choosing an actuator. `opencode/deepseek-v4-flash-free` is **not viable solo** (cascades to cursor for what it can't finish; "unusable" per another repo's operator) — cheap filler only. `opencode/glm-5.2` is a **capable paid escalation** (~$2.50/run metered when opencode reports cost): it solo-completed an impl cursor timed out on. Optional `prices.json` row enables harness cost attribution on estimated-usage and agent-usage-without-cost enrichment. When cursor stalls/times-out on a hard spec, escalate to GLM 5.2. To swap actuators per-invocation without global `config.json` churn, see seed `per-run-agent-override-flag`.

**Retracted (2026-07-13): "claude-haiku stalls because it shares the Claude pool with the operator session."** This entry previously told operators to avoid claude for impl runs when the operator is a Claude session. That diagnosis was wrong. The zero-output stalls were `claude-streams-output-to-watchdog` — Jarvis spawned claude with `--output-format json`, a batch envelope emitted once at exit, so the watchdog never observed claude mid-iteration and every claude patch run rode `iterationTimeoutMs` to exit 8. It was a missing measurement, not pool contention and not a slow model. Fixed: claude now streams (`v1/src/agents/claude.ts:68`). Claude is a valid patch primary again; cursor and codex overrides remain available on preference, not necessity.
...
- **`model at capacity` is the same class as `model_config` — terminal, no cascade.** codex failing with `ERROR: Selected model is at capacity` exits 3 (agent-error) and does not advance to the next rung; the run dies (observed 2026-07-11). When it hits, drop codex from every order (including `subRoleAgentOrder.*`) until capacity returns; a re-run resumes.
...
**Green tests alone are not a gate — `bun test` does not typecheck.** A hand-finish verified with only `bun run typecheck` + scoped tests still misses what CI's check job runs: biome format, `noExcessiveCognitiveComplexity`, and the guard scripts. Run `bun run check` (exit 0 required) — plus `bun run lint:md` when markdown changed — before pushing hand-finished work; `bun run ready` covers all of it when runnable. Biome `noNonNullAssertion` findings are level-`warn`; don't mistake them for the failing errors listed alongside them.
...
- **Run stranded at blocked with unsatisfiable AC.** A generated spec with a non-human-only acceptance criterion asserting CI status, PR body, review state, or other GitHub/network-only facts passes plan-draft validation but strands every implement run at `blocked` when the agent cannot tick them from the worktree. **Fix the spec (do not re-run hoping the agent will work around it):** edit the problematic AC to mark it human-only with `(Manual)`, `visual inspection only`, or `no automated guard` if the verification genuinely cannot be automated, or rewrite it as a satisfiable worktree-verifiable outcome. Once fixed on `main`, re-run the spec. Example: "PR body lists the breaking changes" → "PR body lists the breaking changes. (Manual)" for post-merge evidence, or "Implementation handles the breaking change correctly" for a satisfiable criterion.
- **Ready-gate failure (exit 10).** Verification was red. Jarvis does not retry, invoke an agent, reset, force-push, discard work, or promote the PR. Fix the issue, then use `jarvis1 triage <worktree-name> --mark-ready` or rerun the spec. `triage --merge` retains its CI-backed flake recovery after its strict local gate is green.
...
- **Red tests on an agent's commit may be correctly red.** The tempting read — "the code is right, the fixtures are stale" — is usually backwards: an agent that produced working code rarely also produces tests that fail for no reason, so a red test on new code is evidence about the code. Trace the failure to the implementation and prove the code correct before "fixing" any test to match observed behavior (2026-07-14: sixteen "stale" red CLI tests were correctly flagging an RPC transport that destroyed its shared socket on every call).
...
**`gh pr merge --admin` does not refuse a red PR.** Admin overrides approval *and* the CI requirement, so a red-CI PR merges silently and reddens `main` (observed 2026-07-14: #1513 merged with failing checks). Confirm `gh pr checks <n>` is green *before* every admin-merge — the command will not stop you.

**Never chain the check and the merge in one command.** `gh pr checks <n> | head && gh pr merge <n> --admin` merges even when checks FAILED — the pipeline's exit code is `head`'s, and `--watch` completing does not mean green (it completes on failure too; observed 2026-07-10, PR #1269). Gate on a bare `gh pr checks <n>` (exits nonzero on failure) as its own command, then merge only after seeing it succeed. Same trap when verifying lint: piping through `tail -2` can truncate away the error lines — verify by exit code, not eyeballed output.
...
- **A gate flake caused by CPU contention may come from an orphaned stray process, not a genuinely concurrent session.** Before assuming a co-running operator/agent is the cause, check `ps aux` for a long-running, high-CPU process whose parent PID is `1` (reparented, its launching shell is gone) — that's a stray left behind by an earlier crashed/abandoned run, not intentional load, and it can pin a full core for hours (observed 2026-07-05: a `bun test` process ran 9-11+ hours at 100% CPU, `ppid 1`). Isolate-retest the specific failing file(s) (`bun test <file>` alone); green in isolation confirms contention, not a regression. Flag the stray process to whoever owns that terminal; don't kill it yourself unless it's clearly yours.
  - **Session-start sweep for leaked test fixtures (stopgap, 2026-07-17):** `*.sandbox-unrunnable.test.ts` spawns `ignore-term.sh`/`hang-agent.sh` fixtures under `$TMPDIR/jarvis-run-*/` that **ignore SIGTERM**, so a crashed/killed test run leaks them and they pin a core each. Observed: three ran **3.5 days** at 100% CPU. They are clearly leaked jarvis test garbage (safe to kill). At session start, sweep and SIGKILL them: `kill -9 $(pgrep -f 'jarvis-run-.*(ignore-term|hang-agent)\.sh') 2>/dev/null`. Delete this stopgap when seed `test-fixtures-survive-teardown-and-leak` ships (teardown will SIGKILL its own children).
...
The recurring v2 symptom set: every daemon-socket `jarvis` verb (`daemon status`, `run list`, `run log`/`wait`/`kill`, `pipeline *`) returns a false `connect ENOENT …daemon-*.sock` or `stopped` while the daemon is live; `gh` fails TLS (`x509`/OSStatus cert errors); `git push` and other network git is blocked. Run those verbs sandbox-off from the start rather than spending a round-trip on the sandboxed attempt, and never conclude daemon state from a sandboxed probe.
...
- **`bun run ready`** — the full completion gate. Built-in `ready` is strict verification-only; autofix is configurable via per-project `fixCommand` (default `bun run fix`). On a `full`-tier gate Jarvis runs autofix first (or skips when the resolved package-manager script is absent from `package.json`), commits and pushes any resulting dirty output (message `chore: apply pre-ready check:fix`) **before** verification, then runs strict `ready` (or the project's `readyCommand`) against the committed tree. Autofix and verification are each bounded by `iterationTimeoutMs` (10 min default); hung commands hard-fail with a named message like `bun run ready exceeded 600000ms budget (gate: completion-ready)` instead of hanging. Green verification means every resolved test step passed; when the diff is no-test-impact only (`v1/docs/**`, `v1/spec/**`, `v2/docs/**`, `v2/spec/**`, `reports/**`), the resolved test scope is empty and the gate skips test steps while still running `check`, `typecheck`, and `lint:md`. A failed test step may retry only with identical argv, never a narrower command. Green verification with non-empty porcelain auto-commits and pushes harness-owned churn (message `chore: apply post-ready verification output`) without re-running verification. Residual still-dirty porcelain after that post-verification commit aborts (exit 6) rather than flipping the PR ready. A `git: true` patch completion with implementation work runs this shared full gate once; only green can reach `criteria-complete` or PR readiness. Red verification exits `10` as `ready-gate-failed`, with no retry, fix-up agent, recovery reset/push, or PR promotion. Non-bun repos or repos without a `fix` script must set `fixCommand` in `~/.jarvis/config.json`. The authoritative ready/fix split and ordering live in [`v2/docs/v1-behaviors.md`](../../v2/docs/v1-behaviors.md). `jarvis1 triage <spec-path> --merge` runs the same local gate before CI and retains its CI flake recovery; spec/doc-only plan PRs with an empty resolved test scope pass through without a hand `--admin` fallback.
...
**Timed-out subspec recovery.** After one qualifying terminal iteration timeout, use `jarvis1 plan --recover <relative-subspec> <index.md>`. It opens a separate spec-only draft PR with replacements; do not hand-split the selected tree. Recovery refuses pending checkpoint work, so resume or explicitly abandon that patch work first. Three consecutive timeouts still append the normal patch-run split blocker.
