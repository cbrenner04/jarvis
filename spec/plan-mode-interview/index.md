...
`spec/plan-mode-worktree-and-commits/02` with an agent-proposed kebab-case
...
- **Depends on `spec/plan-mode-draft-and-review/` being merged.** Real
...
- **Interactive mode (`jarvis plan` with no args).** This spec
  ...
  from `spec/plan-mode-draft-and-review/`.
- **Agent-proposed spec name.** The deterministic name derivation
  from `spec/plan-mode-worktree-and-commits/02` is replaced with an
  ...
  `spec/plan-mode-worktree-and-commits/02-spec-name-proposal.md` (suffixing
...
- **Blocker convention applies during interview** too: the agent may
  ...
  `spec/plan-mode-draft-and-review/03`.
...
- Run this spec with `jarvis run spec/plan-mode-interview/index.md`.
