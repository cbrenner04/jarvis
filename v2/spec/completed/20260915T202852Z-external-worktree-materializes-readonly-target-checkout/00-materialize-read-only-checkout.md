...
- [x] A test asserts a git-less run with no read-context request preserves today's `mkdirSync`-only stage behavior (no checkout); it fails against a design that forces a checkout unconditionally.
