...
/** Re-exported so callers reach the store and its default path from one module. */
export { getDefaultStateStorePath };

/** Options for opening the state store. */
export type OpenStateStoreOptions = OpenDatabaseOptions;
...
/**
 * Opens the state store over a migrated SQLite database.
 *
 * Database lifecycle (path resolution, connection, migrations) lives in
 * `db.ts`; this boundary just wraps the connection in the repository API.
 */
