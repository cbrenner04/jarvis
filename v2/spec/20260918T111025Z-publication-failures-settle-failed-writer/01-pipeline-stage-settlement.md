...
- [x] A test proves a pipeline branch with a `failed` `completion_commit_failed` entry row is surfaced as a failed stage and admitted for recovery, consistent with its resumability; it fails against the pre-fix code for the same reachability gap.
