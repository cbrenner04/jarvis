...
**A second daemon on the same key is refused, not swapped in.** `startIpcServer` probes the socket path before binding; when a live daemon answers (or is too busy to answer within 250ms) the new process exits non-zero with `A daemon is already listening on <path>`. That refusal is the guard working — the running daemon keeps serving. Only a genuinely stale entry (`ECONNREFUSED`) is removed and rebound; a probe that cannot resolve the path (`ENOENT`) removes nothing. That asymmetry matters because a sandboxed caller — any `jarvis` command run from an agent session — gets `ENOENT` for a live socket it cannot resolve, and unlinking on it deletes a healthy daemon's entry. Before this check, every `daemon start` — including the auto-start any `jarvis` command performs when it cannot reach a daemon — unlinked the incumbent's socket first, which stranded every run on the machine behind a `connect ENOENT` while the daemon itself stayed alive and healthy.
...
- **Approve every fan-out gate before any sibling plan lands (2026-09-07).** Plan-stage resolution verifies the *whole* `downstreamInputs` list, so a lane whose sibling's ready-intent was already consumed into a spec tree fails in under 25s naming a file that belongs to the finished lane. Approving the gates **serially** — head lane, land, then the next — is what creates that state. Approving all of a fan-out's `approve-intent` gates back to back, before any plan consumes its input, dispatches every lane in parallel: verified on two independent pipelines (four concurrent plan stages). Use simultaneous approval; fall back to standalone drives only for lanes whose sibling already planned. Seed: `v2/spec/seeds/fan-out-plan-resolution-is-all-or-nothing-across-lanes.md`. Cleanup: delete this bullet when that seed ships. **Two distinct failures, do not conflate them (2026-09-08).** Simultaneous approval fixes only the *resolution* failure. Confirmed again on a four-lane fan-out: all four `approve-intent` gates approved back to back dispatched four plan stages in parallel, each with a real run row and agent invocation — the sub-25s no-run-row shape did not occur. Two lanes then settled `blocked` / `agent_blocked` naming an unmerged sibling spec. That is the *dependency* refusal and it is correct: those ready-intents were a strict prerequisite chain. A blocked lane costs one cheap dispatch and leaves a diagnosable row; a resolution failure leaves nothing. So when the split is a known chain, approve only the head and drive dependents standalone as each lands — approving all four buys parallel dispatch you cannot use. Read the ready-intents' `## Prerequisites` before choosing.
