...
- Callers may mark a lane disposable (never landed: no PR, no unpushed real commits beyond harness staging); disposable retirement bypasses descendant and landed-criteria refusals and proceeds to rematerialize from the current base; rules out requiring per-gate `--reset-despite-*` overrides for dead lanes.
