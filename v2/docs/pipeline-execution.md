...
- Plan recovery (`resolveBlockedPlanStageRecoveryTarget`) uses the same `fanOutPlanResultForBranch` branch-key binding for fan-out-shaped resolutions; named non-`default` lanes with single-path `{ steps }` use `singleStageResolutionSteps`.
