...
When a `surviving_mutation_failed` outcome names a mutation site inside a `setTimeout`/`setInterval` callback in `v2/src/daemon/**` or `v2/src/execution/**`, the error message names both gates: (1) a both-direction test that kills the mutation on that changed line, and (2) the determinism-guard prohibition on real-timer waits in the matching test suite. The fix is to extract a pure predicate from the guarded callback and test the predicate directly without a real-timer wait. Resume the run after extracting; adding a timed test violates the determinism gate and will strand the run again.
