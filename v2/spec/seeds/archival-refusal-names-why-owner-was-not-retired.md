...
**Folded from `ready-intents/` 2026-07-27.** Diagnostic text only — ship with the next cleanup
eligibility or refusal-message change; do not block the recovery batch or pipelines.
...
- `v2/docs/operator-runbook.md` — § Cleanup: eligibility gate: ownership refusals name the owner's
  retirement-refusal reason.
- `v2/docs/v1-behaviors.md` — archival refusals name the owner's specific retirement-refusal reason,
  not just the ownership fact.

## Prerequisites

Satisfied on `main`: cleanup refuses spec archival when another materialized worktree owns the spec;
eligibility gate returns a reason string per ineligible worktree; terminal durable runs no longer
block worktree retirement (#2196).
