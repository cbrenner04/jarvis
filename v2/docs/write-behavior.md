...
| `jarvis run list --since <duration\|timestamp>` | Optional `--since`: relative duration (`<positive-integer><d\|h\|m\|s>`, subtracted from query time) or absolute Unix ms / ISO 8601; cutoff is inclusive on `created_at`; bypasses the default fifty-terminal-run retention window; optional `--limit <positive-integer>` caps newest matches (default **200** when any filter is present) | Same row format as `jarvis run list` | `0` on success; `1` with `invalid_since` before any `list` RPC when `--since` is invalid; `1` with `invalid_limit` when `--limit` is invalid |
| `jarvis run list --limit <positive-integer>` | Parsed at the CLI and sent on the `list` RPC; alone it does not bypass terminal retention — only filtered queries (`--since`, and later dimension filters) apply the default **200** cap or an explicit `--limit` | Same row format as `jarvis run list` | `0` on success; `1` with `invalid_limit` before any `list` RPC when `--limit` is invalid |
