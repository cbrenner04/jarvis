...
- Deterministic detection tie-break: the **`test` script command string is authoritative** — match the runner named in it (e.g. `vitest run` → vitest) and ignore devDependencies when the script names a known runner. devDependencies are consulted **only** as a fallback when the script names no recognized runner. So a Bun repo with `vitest` in devDeps but a `bun test` script resolves to bun, not vitest. Rules out: scanning devDependencies first or unioning both signals (conflicting signals → wrong `-u` flag → false greens or errors).
- A workspace/delegating test script (turbo, nx, lerna, `npm-run-all`, etc.) that names no recognized leaf runner is **unresolved → gate skips → fall through to base-ref**. Rules out: treating the delegator as a runner and guessing an update flag it does not accept.
...
- Accepted cost of the update/re-test split: the update pass runs the raw runner (e.g. `bun test --update-snapshots`) while the re-test runs the package script `bun run test`, which may carry different scope/preload/flags. An update narrower than the re-test could leave a still-stale snapshot the re-test then hits, returning non-green — the fail-safe (fall through to base-ref) limits but does not eliminate this. Accepted over `bun run test -- -u` (rejected above for unreliable arg-forwarding). Rules out: claiming the two passes cover identical scope.
- Mixed failures (stale snapshot + real failure together) are an intended layering, not a gap: the update pass refreshes the snapshots, the re-test still fails on the real failure → seam returns false → `00` falls through to base-ref validation. The gate is not all-or-nothing on the snapshot signal. Rules out: a reader assuming a stale snapshot anywhere forces a churn reject.
...
- [ ] Implement the update-snapshots command resolver: configured override → detect from the `package.json` test script (authoritative; runner it names wins), devDependencies only as fallback (bun/vitest/jest), workspace/delegating scripts and unrecognized runners → unresolved.
...
- [ ] Add tests: configured override beats detection; detection maps bun/vitest/jest; the test-script string wins the tie-break over conflicting devDeps; workspace/delegating script and unknown runner → unresolved (false, gate skipped); end-to-end stale-snapshot repo green after update → blocker rejected; real-failure repo still red → blocker proceeds to base-ref path.
...
- [ ] A per-project configured update-snapshots command takes precedence over detection; with no configured command, a bun/vitest/jest target repo is detected from its `package.json`; an unrecognized runner — including a workspace/delegating script (turbo/nx) naming no leaf runner — leaves the command unresolved and the gate does not reject (control falls through to base-ref validation).
- [ ] Detection is driven by the `test` script command string (the runner it names wins); devDependencies are consulted only when the script names no recognized runner — a repo whose script says `bun test` but lists `vitest` in devDeps resolves to bun.
