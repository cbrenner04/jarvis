...
/**
 * Ordered, append-only schema migrations. Each version is applied once and
 * recorded in `schema_migrations`; never edit a shipped version in place — add
 * a new one.
 */
...
/**
 * Opens the SQLite database, creating parent directories, enabling foreign
 * keys, and applying any unapplied migrations. Returns the raw connection; the
 * repository layer wraps it. This is the only place schema lifecycle lives.
 */
...
/** Applies every unapplied migration in one transaction, tracked by version. */
