...
**`false`:** Plan specs are authored in Jarvis-owned storage at `~/.jarvis/specs/<project-safe-id>/<spec-dir>/` outside the target repository. No git worktree, branch, commits, or PR are created. Plan mode runs directly in the target repo root. The generated spec includes a `repo:` binding so `jarvis run` can resolve the target repository later. Use this mode when specs should not be committed to the repo or when you want to generate and immediately execute a spec without the PR review cycle. Re-runs are self-cleaning: if an incomplete run ticked acceptance criteria or appended `## Blocker` in that external spec, Jarvis resets those stale mutations before the next agent invocation, even when the patch run itself uses `git: true`.
