...
`,
    ...
    const subspecShas = execSync(
      ...
      .trim()
      .split("\n");
    const firstSubspecSha = subspecShas[0];
    const secondSubspecSha = subspecShas[1];
    expect(readFileSync(prBody, "utf8")).toBe(
      `${expectedBody}\n\n---\n\n- ${firstSubspecSha} 00 - One \u2014 fake-claude\n- ${secondSubspecSha} 01 - Two \u2014 fake-claude\n\nWritten by fake-claude through Jarvis.`,
    );
    expect(
...
`,
    ...
    const degenerateSha = execSync("git log --format=%h -n 1 degenerate", {
      cwd: projectRoot,
      encoding: "utf8",
    }).trim();
    expect(readFileSync(prBody, "utf8")).toBe(
      `${expectedDegenerateBody}\n\n---\n\n- ${degenerateSha} 00 - One \u2014 fake-claude\n\nWritten by fake-claude through Jarvis.`,
    );
  });
