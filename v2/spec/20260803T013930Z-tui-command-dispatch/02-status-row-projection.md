...
- The status row keeps the existing leading segments (`<active-count> active · <profile>@<digest> · refresh <interval>`) and appends retained feedback suffixes in fixed order: when `lastRpcError` is present, append ` · error: <lastRpcError>`; when `lastCommandResult` is present, append ` · result: <lastCommandResult>` after any RPC suffix — rules out hiding either feedback or reversing their order.
...
- [ ] `v2/src/tui/tui-monitor-lines.test.ts` proves when both `lastRpcError` and `lastCommandResult` are retained the status row shows ` · error: …` before ` · result: …`, a successful refresh clears only the RPC suffix, and narrow-width projection still exposes both suffixes when they fit after the fixed prefix.
