...
- **Branch protection + can't self-approve.** Every PR is `BLOCKED` until `gh pr merge --admin`;
  ...
  — an admin-merge skips the completion gate's lint (a dead `PreflightResult` reached `main` this
  way).
- **`check:fix` vs `check:fix:unsafe`.** Safe `check:fix` leaves `noImplicitAny`/`noExplicitAny`/
  unused-var fixes needing `--unsafe` or a hand edit.
...
- Anything being automated in the sibling seeds (transient-retry, serial-retry, lint convergence,
  plan-PR auto-ready, actuator floor). The runbook documents the *residual* manual path, and links
  to those for the automated path.
...
- `session-report.md` §4 (tooling & workflow feedback) — source of these items.
