...
**A surviving keystone means an inert headline change (2026-08-07).** Each runtime-behavior subspec carries one ticked `Keystone checkpoint:` criterion whose `// @mutate` directive reverts the headline to baseline semantics. When that revert leaves the scoped suite green, completion refuses with `Inert headline change` — distinct from hollow guard checkpoints (`Hollow mutation checkpoints`) and from premise-smell hollow (a second hollow on a different guard). Missing keystone when guard checkpoints exist refuses with `Missing keystone checkpoint`; more than one ticked keystone criterion refuses with `Multiple keystone checkpoints`. Plan-draft authors the keystone criterion and directive; headline-only missing-keystone enforcement without guard checkpoints is deferred.
