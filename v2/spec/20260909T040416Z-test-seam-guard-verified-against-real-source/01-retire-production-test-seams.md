...
- [x] `bun run scripts/guard-production-test-flags.ts` exits 0 on the clean tree with `00`'s structural detection in place, and the `reports every seam present in the scan roots` meta-test finds zero candidates.
- [x] `v2/src/execution/workflow-runner-resume.test.ts` ready-gate repair fence cases stay green after bypass removal.
- [x] `v2/src/execution/write-loop.test.ts` repair-fence rejection tests stay green, including the case that previously asserted a bypassed run completes.
- [x] `v2/src/execution/diff-derived-mutation-verifier.test.ts` stays green through the rename.
