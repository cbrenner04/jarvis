...
- Resolve the entry run id from the row `resumeFinalizationOnly` receives: read `run.workflowSnapshot?.invocationId`, then take the row from `store.findRunsByInvocationId(invocationId)` whose `stepId` is the snapshot's step-0 `stepId` — not `[0]` by `created_at`, which can tie. Skip the rewrite when `run.workflowSnapshot` is undefined or no row resolves.
