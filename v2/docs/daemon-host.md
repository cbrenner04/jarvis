...
| `pipeline_list` | — | `{ pipelines: Array<{ pipelineId, name, state, createdAt, finishedAtMs, stages: Array<{ stageId, branchKey, status, workflowInvocationId, startedAt, endedAt }> }> }` | Parameterless durable snapshot of every admitted pipeline without following live transitions. Empty store → `{ pipelines: [] }`. `createdAt` is the durable pipeline row admission timestamp (ms). `finishedAtMs` is `null` while derived `state` is non-terminal; for terminal states it is `terminalPublicationSucceededAt` when set, otherwise the maximum non-null stage `endedAt`, otherwise `createdAt`. Stage `startedAt` and `endedAt` are milliseconds since epoch; each is `null` when unset on the durable row. Stage order follows stored authored `position` then `branch_key`. Derived `state` uses `derivePipelineState` (see [Pipeline snapshots](#pipeline-snapshots)). |
...
```json
{ "pipelines": [{ "pipelineId", "name", "state", "createdAt", "finishedAtMs", "stages": [{ "stageId", "branchKey", "status", "workflowInvocationId", "startedAt", "endedAt" }] }] }
```
