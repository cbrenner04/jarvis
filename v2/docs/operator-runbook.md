...
Cleanup enumerates `~/.jarvis/daemon-*.sock` and treats each socket's keyed `.sock`, `.pid`, and `.log` as one daemon-digest lifecycle unit. A connect probe that receives `ECONNREFUSED` or `ENOENT` proves the digest dead; preview lists every present triplet artifact under `remove:`, and apply removes each listed artifact. A successful or ambiguous probe (timeout, permission error, unexpected error) preserves the entire triplet and reports the socket reason. Socket-less `.pid` and `.log` files are not discovered or removed. If the jarvis home cannot be enumerated, no daemon artifacts are removed.
