...
- Inline-draft, refine, and draft prompts all forbid self-referential deliverables: acceptance criteria must verify target state outside the active spec directory (code/tests/docs/operator behavior/generated evidence), so criteria that only grade active-spec prose are invalid even without saying "the spec." Draft and inline-draft also instruct that acceptance criteria state observable behavior and stay silent on schema, tables, files, and shapes unless the structure itself is the contract (for example, a public API or wire format). Sources: `prompts/plan/inline-draft.md`, `prompts/plan/refine.md`, `prompts/plan/draft.md`, `v1/docs/spec-guidance.md`
