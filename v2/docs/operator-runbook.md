...
- **Test doubles must not call production behavior (shipped 2026-07-22):** Fixtures under `v2/src/testing/**` are statically guarded by `scripts/guard-test-double-production-calls.ts` (runs as part of `bun run check`). Forbidden: importing and calling production functions to compute a double's response (e.g., a workflow double that calls the real dispatcher). Allowed: type-only imports, imported constants never called, and allowlisted builder/entry-point calls (`main`, `openStateStore`, `startDaemon`, `isProcessAlive`). See [`v2/docs/test-writing.md` § Do not call production behavior in test doubles](./test-writing.md#do-not-call-production-behavior-in-test-doubles).
