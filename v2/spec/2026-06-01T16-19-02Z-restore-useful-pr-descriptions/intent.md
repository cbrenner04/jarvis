...
- Patch-mode default narrative comes from the checked subspec headings in the current spec tree, not from commit subjects/trailers or generated prose; this supersedes the placeholder fallback and rules out summarizing future unchecked work.
- This subspec fixes patch-mode PR bodies only; do not also redesign plan-mode PR descriptions here, because bundling both modes would turn one regression fix into a broader product rewrite.
