...
Subspec 02 also carried the operator-runbook and parity documentation, folded in by #2573 after the two documentation-only subspecs were retired as over-split. Still missing: there is no steering — approve/reject/resume, run pause/kill, and log follow are all still CLI-only. Slice 4 also left `waitState` polled but never rendered and the right-pane fallback unwindowed — seeded as [seeds/tui-waitstate-is-polled-but-no-longer-rendered](seeds/tui-waitstate-is-polled-but-no-longer-rendered.md); fold it into slice 6.
