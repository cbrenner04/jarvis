...
- Continuation refuses dirty trees, non-descendant `HEAD`, live owners (existing gates, reused unchanged), or ticked criteria absent from the branch's own commits (new guard, protects against forged/mismatched tick state); each refusal names its fix.
...
- [ ] A test drives a lane where a checked criterion has no corresponding commit on the branch, and asserts continuation refuses with a fix-naming message instead of continuing; it fails against the pre-fix CLI, which has no forged-tick-state check.
