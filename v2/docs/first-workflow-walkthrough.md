...
atomically to completion within its step timeout. However, if the step wedges
(iteration timeout or orphaned tracking), the same `runId` shown in `list`
becomes reapable and can be killed with `jarvis run kill <run-id>`, reaching
durable `killed` status with the worktree retained. Pausing/resuming are not
supported for workflow-started implement runs.
