...
- Establish the canonical `createFakeWriteLoopExecutor` (with settle/abort/pause
  ...
  scope, per intent).
- `startRun`, `listRuns`, and `mockWriteLoopInput` in `v2/src/testing/run-control.ts` take
  the object `createRunControlHandlers` returns and call `.start`/`.list` directly, instead
  of sending frames over an `IpcClient`.
...
- [ ] `startRun`/`listRuns` in `v2/src/testing/run-control.ts` invoke handlers directly
      (no `IpcClient` in their implementation).
