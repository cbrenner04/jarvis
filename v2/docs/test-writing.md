...
`resumePopulatedIntentPublication` (`IntentFinalizationResumeDeps.runner`) and `executeWriteLoop` (`WriteLoopInput.stagedMarkdownLintRunner`) forward an optional runner to the reviewed staged-lint gate. The daemon resume/recover tests and `write-loop-staged-markdown-lint.test.ts` stub it. Measured on operator hardware (2026-09-16): `write-loop-staged-markdown-lint.test.ts` (8 tests) 11.2s → 10.0s; `intent-output.test.ts` (15 tests) 15.95s → 1.31s.

## Real-lint structural guard

`scripts/guard-real-lint-in-unit-tests.ts` (wired into `bun run check`) scans unit-slice test-file sources (`.test.ts`, excluding `.sandbox-unrunnable.test.ts`) for calls into a fixed list of markdown-lint entry points — `runMarkdownlintAutofix`, `lintStagedMarkdown`, `validateIntentStage`, `repairIntentStageContent`, `landIntentWorkflowOutput`, `recoverPlanStage`, `resumePopulatedIntentPublication`, `executeWriteLoop` (the last two thread `runner` / `stagedMarkdownLintRunner` into the reviewed staged-lint gate) — that lack an injected `runner`. Each of these defaults to `realAsyncSubprocessRunner` (a real `bun markdownlint-cli2` spawn) when no runner is threaded through, so an unguarded call site reaches the real binary. The scan is text-based (import-binding + call-site match), not a runtime spawn trap: it catches an unexercised or newly-added call site, not only ones a test run happens to execute.

A file with pre-existing calls outside the seams' scope is exempted by an explicit file-level allowlist in the guard: `shared/intent-stage.test.ts` and `v2/src/execution/staged-markdown-lint.test.ts` (direct real-binary coverage), `v2/src/execution/workflow-runner-review.test.ts` (real-binary review-actuator reprompt assertions), and `v2/src/daemon/daemon-start-list.test.ts`, `v2/src/execution/write-loop.test.ts`, `write-loop-idle-watchdog.test.ts`, `write-loop-intent-landing.test.ts`, `write-loop-session-log.test.ts` (`executeWriteLoop` calls that never reach the lint gate). `workflow-runner-resume.test.ts`, `daemon-resume.test.ts`, `daemon-pipeline-recover.test.ts`, and `pipeline-stage-recovery.test.ts` now inject stub runners and are not allowlisted; the guard test pins that they are flagged. A single call site that provably never reaches the spawn (an error branch that returns before validation runs) can carry a `// guard-real-lint-in-unit-tests: <reason>` marker on its own line or the line above instead of an injected runner — see the three rogue-path rejection tests in [`intent-output.test.ts`](../src/execution/intent-output.test.ts).
