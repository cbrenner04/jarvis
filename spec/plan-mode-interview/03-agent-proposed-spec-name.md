...
`spec/plan-mode-worktree-and-commits/02`. The agent has more context
...
- **Validation.** After the interview (or naming-only) phase, plan
  ...
  - If absent or invalid (bad chars, reserved, too long), fall back
    ...
    `spec/plan-mode-worktree-and-commits/02-spec-name-proposal.md`
...
- **PR opens against the final branch.** The PR-open step from
  `spec/plan-mode-worktree-and-commits/05` already runs against the
...
- **Frontmatter is preserved through later phases.** Draft and
  ...
  `spec/plan-mode-draft-and-review/`) considers a modified
