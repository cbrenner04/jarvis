...
- To flip a PR from draft to ready, run `bun run ready` instead—this is the draft→ready gate distinct from the per-iteration `typecheck`/`test` loop. The ready script enforces a 10-minute wall-clock deadline across all four steps (`install`, `typecheck`, `test`, `check`); override via `JARVIS_READY_TIMEOUT_MS` (in milliseconds) if a step legitimately requires more time.
