...
Idle abort on **patch implementation** returns exit code `8` with `exitReason: "watchdog-idle-timeout"` only when no later `agentOrder` rung remains (or during fix-up iterations, which do not escalate). When at least one fallback rung remains, the harness shifts the stalled agent off `activeAgents`, emits `<agent>: idle timeout; escalating to next agent`, records a non-terminal `kind: "timeout"` row with `exitReason: "watchdog-idle-timeout-fallback"`, and retries the same subspec on the next rung. Idle timeouts are **not** classified as quota. Review, shrink, plan, and prompt phases keep the prior terminal idle abort (exit `8`, no cascade). Per-iteration wall-clock timeout (`iterationTimeoutMs`) and whole-run timeout (`runTimeoutMs`) remain terminal with no agent cascade.
