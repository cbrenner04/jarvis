...
- Review phase flow is: baseline `bun run ready` (install → check:fix → typecheck → test → check) → N debate cycles, each cycle = three read-only reviewer attempts (adversary → defender → judge) + injected executor writing the verdict, spec-tree edits reverted including untracked additions, non-empty changes committed per role and per executor invocation → final `bun run ready` → `gh pr ready`. Prior cycle verdicts are passed as context to the next cycle's adversary. Sources: `v1/src/modes/review/run.ts`, `v1/src/modes/patch/review.ts`, `v1/docs/run-loop.md`
...
- Quota exhaustion during any review role attempt triggers fallback to the next configured review agent; if all agents are exhausted, the cycle exits `2`. `model_config` exits `3`. Sources: `v1/src/modes/review/run.ts`, `v1/src/modes/patch/review.ts`
