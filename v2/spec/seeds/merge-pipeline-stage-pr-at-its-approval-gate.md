...
**Merge-at-gate alone is insufficient (2026-08-31, second dogfood — pipeline `45f78585`, attribution feature).** Merging the intent PR (#3229) at the approve-intent gate *before* approving did NOT make the plan stage move the ready-intent. The plan branch forked from the pipeline's admission base (`61c75c4e`, the main at pipeline start), NOT from the updated main containing #3229 — so the ready-intent was absent from the plan stage's base, the plan recreated `intent.md` from the prior worktree again, and #3229's ready-intent was left orphaned on main. Root cause: the pipeline plan stage's base is pinned at admission, so it never rebases onto a mid-pipeline intent merge. The move-semantics fix therefore needs BOTH the intent landed AND the plan stage's base re-resolved to current main (post-intent-merge) — merging the intent PR by itself is a no-op for the pipeline plan.

## Decisions

- The pipeline must ensure the ready-intent is on the plan stage's base so the plan workflow moves it. This requires TWO things together (proven by the 2026-08-31 dogfood: doing only the first is a no-op): **(1) `pipeline approve` lands (squash-merges) the approved intent stage's PR to its base as part of approval, AND (2) the successor plan stage re-resolves its base to that updated main at dispatch, rather than the base pinned at pipeline admission.** Only then does the plan stage materialize off a base carrying the ready-intent and move it. Rules out the recreate workaround, the stranded intent branch, and the false assumption that merging the intent PR alone suffices.
