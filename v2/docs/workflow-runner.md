...
`executeTerminalPublication` in `v2/src/execution/terminal-publication.ts` is the sole executor for a resolved `terminalAction` (`leave-draft`, `ready`, or `merge`). It consumes existing worktree and PR evidence (`worktreePath`, `branch`, `baseRef`, `prNumber`, `prUrl`) and does not re-run completion publish (push, draft create, or body refresh). Production wiring is deferred to pipeline settle (`settle-pipeline-terminal-action`); this slice ships the executor and its contract only.

**Completion ↔ terminal composition:** completion publication (`publishCompletionArtifacts` → `createCompletionPublisher` → `runReadyFinalizer`) and terminal publication are separate boundaries. When `terminalAction` is `leave-draft`, settle/upstream must not run completion ready finalization before terminal publication — completion must leave the PR draft. For `ready` and `merge`, terminal publication assumes completion already recorded `prNumber`/`prUrl` and re-runs only the ready gate plus idempotent ready flip at settlement, intentionally excluding completion's mutation/smoke verifier stack.

**Settle handoff:** after every workflow and required approval stage succeeds, the daemon consumer invokes `executeTerminalPublication` with the final implement stage's PR evidence and the admitted definition's `terminalAction`. Pipeline success requires terminal-action success; failure settles `failed` with the action and wrapped error retained while PR evidence stays intact.
