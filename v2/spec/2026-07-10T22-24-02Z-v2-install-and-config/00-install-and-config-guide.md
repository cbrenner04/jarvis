...
- [x] `v2/docs/install-and-config.md` exists and reads as a fresh-checkout→running-daemon walkthrough ordered: prerequisites → install → config → daemon → recovery.
- [x] Prerequisites section names Bun, an authenticated `gh`, and at least one agent CLI on PATH, and shows cloning + symlinking the `jarvis` binary alongside the existing `jarvis1` shim, matching the two `bin` entries in `package.json` (`jarvis` → `v2/src/cli.ts`).
- [x] The `jarvis config` section documents `show`, `path`, and `set-agents` with outputs matching committed behavior: `show` prints the machine `agents` one name per line (newline-separated, not JSON) or `No machine agent override configured.`; `path` prints the fully-expanded absolute config path (`<home>/.jarvis/config.json`, no tilde substitution — `~/.jarvis/config.json` may appear only as annotated shorthand); `set-agents <csv>` takes bare comma-separated agent names (rejects a segment containing `:`), replaces the full `agents` array while preserving other top-level keys, and prints the landed order as JSON (`{"agents":[...]}`).
...
- [x] The `jarvis daemon` section documents `start`, `status`, and `stop` with the committed stdout/exit-code contract (`start` → `{"pid":...,"socketPath":...}`; `status` → `running`/`stopped` with exit 0 when running, 1 when stopped; `stop` → `stopped`) and states that `jarvis daemon status` reporting `running` (exit 0) is the up-confirmation step.
- [x] The guide lists the on-disk locations (`~/.jarvis/config.json`, `~/.jarvis/daemon.sock`, `~/.jarvis/daemon.pid`, and the committed `config/machines/` profiles).
- [x] The recovery section maps each error to the command that surfaces it, not one flat "load-time" bucket. Config-load errors surface at `jarvis config show`: machine config is not a JSON object, invalid JSON in the machine config, and the `agents` validation failures (non-array/empty-array/non-string-entry/empty-string-entry/duplicate). Model-resolution errors surface later at `jarvis run`: missing `machineProfile` key, a machine profile not found (typo'd profile name → `Machine profile '<name>' not found at <path>`), and a machine profile missing its required `models` key.
- [x] The recovery section covers the daemon-start failures an operator hits: daemon already running on the socket (re-run of `jarvis daemon start`), the readiness timeout, and a missing PID-file directory.
...
- [x] The guide cross-links `agent-model-config.md` (config/model store), `write-behavior.md` (daemon CLI), and `daemon-host.md` (socket transport) instead of restating their contracts.
