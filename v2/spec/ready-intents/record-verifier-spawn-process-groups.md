...
- `recordVerifierProcessGroup` / `clearVerifierProcessGroup` generalized record/clear API exists.
...
Only the ready-gate and required-integration invocations record an in-flight process group on the owning run. Three sibling finalization spawns — base-ref reproduction probe, diff-derived mutation verifier scoped `bun test`, and runtime smoke `bun run <entrypoint>` — spawn detached test trees with no durable group id, so daemon-startup sweep and `make-run-kill-rpc-report-settlement` pre-quiescence signalling cannot find them after the owning run dies.

## Decision ledger

- Each of the three spawns records its process group through the same `processGroup.onGroupId` callback pattern the ready gate already uses, cleared in settlement `finally` via `clearVerifierProcessGroup`; rules out signal-only binding without a durable id.
- Durable pgid recording only at these spawn sites; live run termination signals recorded groups via daemon pre-quiescence pgid signalling (`make-run-kill-rpc-report-settlement`), not per-spawn `AbortSignal` binding like ready-gate/required-integration; rules out duplicating gate spawn-level signal wiring on the three siblings.
...
- [ ] `v2/src/execution/diff-derived-mutation-verifier.test.ts` proves each concurrent scoped verifier `bun test` spawn up to `MAX_CONCURRENT_VERIFIER_TEST_RUNS` records its own process group on the owning run row without overwriting siblings; it fails against the current unrecorded spawn path.
