...
| dm-pipeline-recover-plan-fixture | v2/src/daemon/daemon-pipeline-recover.test.ts | pipeline_recover admits and lands a corrected non-first fan-out branch without redrafting | corrected plan subspec landing satisfies staged-markdown lint-clean contract | `lintStagedMarkdown` on landed durable spec returns `{ kind: "clean" }`; registry-backed stage seed via `writeLintCleanPlanStage` / `readReviewMdLintFixture` | behavioral | n/a | | |
| dm-resume-intent-lint-fixture | v2/src/daemon/daemon-resume.test.ts | admits a populated-stage intent finalization landing_failed row instead of unsupported_resume_context (+ sibling populated-stage intent finalization cases) | lint-clean intent stage markdown seeds landing/republication paths | `lintStagedMarkdown` on `.jarvis-intent-stage` returns `{ kind: "clean" }`; registry-backed stage seed via `writeLintCleanIntentStageFile` / `readReviewMdLintFixture` | behavioral | n/a | | |
| dm-rchg-forbidden-weakmap-symbols | v2/src/daemon/daemon-run-control-handler-guard.test.ts | daemon production sources omit activeRunsByHandler and activeRunForHandler | daemon production tree must not reintroduce handler WeakMap back-channel symbols | `DAEMON_RUN_CONTROL_HANDLER_FORBIDDEN_SYMBOLS` scanner policy export via `scanDaemonRunControlHandlerForbiddenSymbols`; production sources discovered by recursive walk of `v2/src/daemon/**/*.ts` excluding tests | behavioral | n/a | | |
...
| dm-startlist-terminal-settlement-guard | v2/src/daemon/daemon-start-list.test.ts | daemon production terminal writers are restricted to atomic settlement | daemon must not call legacy `commitGuardedKill`; only admitted `setRunStatus` transition is in-progress; reconciliation admission must not UPDATE status | `scanDaemonTerminalSettlement` semantic writer identities over daemon production tree discovered by recursive `v2/src/daemon/**/*.ts` glob; `locateSymbolSlice` on `beginRunReconciliation`; regression pairs regex argument-equality brittleness and vacuous `indexOf` slice when end anchor moves | behavioral | n/a | | |
...
| dm-workflow-start-admission-seam | v2/src/daemon/daemon-workflow-start.test.ts | workflow starts, pipeline dispatch, and recovery share daemon admission | workflow start, pipeline dispatch, and recovery route through `admitWorkflowStart` without bypassing ownership/memory | `admitWorkflowStart` call-site property assertions over daemon production sources discovered by recursive glob; brace-matched admission definition; regression pairs symbol-pin brittleness with call-site routing guard | behavioral | n/a | | |
...
| dm-pipe-dispatch-ended-at-ast | v2/src/daemon/pipeline-stage-dispatch.test.ts | every terminal pipeline stage-run write carries endedAt | terminal `store.updateStage` status writes include `endedAt: Date.now()` | AST discovery over dispatch-owner production sources with `endedAt` presence property assertions on terminal status writes | behavioral | n/a | | |
| dm-pipe-recovery-plan-fixture | v2/src/daemon/pipeline-stage-recovery.test.ts | recoverPipelineBranchStage > * | recovery lands lint-clean plan subspec body | `lintStagedMarkdown` on landed durable spec returns `{ kind: "clean" }`; registry-backed stage seed via `writeLintCleanPlanStage` / `readReviewMdLintFixture` | behavioral | n/a | | |
...
| dm-wlbinding-callers-allowlist | v2/src/daemon/write-loop-binding-source-guard.test.ts | only allowlisted modules call resolveWriteLoopBindings | `resolveWriteLoopBindings` caller surface is restricted | discovered `v2/src` production callers compared to resolver export/call surface from `export function resolveWriteLoopBindings` owners plus import-linked callers; regression pairs hardcoded allowlist brittleness | behavioral | n/a | | |
| dm-wlbinding-source-markers | v2/src/daemon/write-loop-binding-source-guard.test.ts | daemon binding resolution re-loads from the machine profile unless the snapshot replay test hook is set | binding resolution re-loads from machine profile unless `forceSnapshotAgentModelConfig` | `locateSymbolSlice` on `resolveWriteLoopAgentModelConfig` in modules exporting `resolveWriteLoopBindings`; behavioral `forceSnapshotAgentModelConfig` inversion; regression pairs `daemon.ts`-only substring pins | behavioral | n/a | | |
