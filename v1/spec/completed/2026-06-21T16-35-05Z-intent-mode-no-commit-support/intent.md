...
- `v1/src/commands/intent.ts` — `:404-405` requires `plan.commit=true` (errors under `commit:false`).

## Prerequisites

- `jarvis intent` requires `plan.commit=true` and errors under `commit:false` (`v1/src/commands/intent.ts:404`).
- A no-commit external-spec write design exists for plan/run to mirror.
