...
- Draft validation order: blocker (if any) → index.md existence → numbered subspecs presence. Blocker check runs first so prerequisite gate failures short-circuit all other checks. Blocker indicates failed prerequisites, missing intent details, or other unrecoverable plan conditions; when present, plan exits `1` without proceeding to index/subspec validation. Sources: `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/run.ts`
