...
The returned run id's status reported by daemon `wait` and `list` operations reflects this rollup for workflow entry runs: `wait` awaits the full workflow completion and returns the rollup status; `list` reports the rollup status for the entry row, while other step rows report their own durable statuses. When the stopping row is hidden `~shrink` with `surviving_mutation_failed`, the entry payload also takes its terminal outcome, error detail, and iteration count from that row. Other hidden-finalization outcomes and later authored failures are not projected. This ensures callers reading the workflow's entry run ID get the implemented workflow terminal result.
