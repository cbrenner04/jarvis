...
**Prompt-echoed stderr.** Some agents echo the dispatched prompt (or its tail) on stderr instead of a diagnostic. When the final binding's stderr contains the dispatched prompt, the write loop marks the persisted detail `echoedInput: true` and omits `error.message` — the operator summary falls back to the generic `invocation_error` reason instead of echoing the harness's own prompt back. The raw bytes are not lost: `jarvis run log <run-id>` always carries an `invocation_failure_diagnostic` record (`stderrTail`, `echoedInput`) for every terminal invocation-failure settlement with non-empty final stderr, echoed or not — read that record when `error.message` is absent on an `invocation_error` row.
