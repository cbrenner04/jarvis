...
- **One-run actuator probes:** use `jarvis1 run --agent <name>[:<model>]` or `jarvis1 plan --agent <name>[:<model>]` instead of editing `modes.patch.agentOrder` / `modes.plan.agentOrder` in config for a single experiment. Persisted `agentOrder` stays unchanged; plan review panel keeps the pre-override snapshot (see [agents.md](./agents.md)).
