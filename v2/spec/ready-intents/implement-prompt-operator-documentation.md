...
- Implement-owned artifacts and v2 id wiring are landed — registry, call sites, and render paths match `implement.prompt.body` / `implement.rules`.
...
- Cross-link `v2/docs/documentation-standard.md` instead of duplicating fragment-policy prose; rules out two divergent ownership narratives.
...
- [ ] `v2/docs/prompts.md` names all four review-role families, states the shared terse-skeleton conventions, and records that patch review prompts are frozen while plan and implement converged to the intent-family style.
