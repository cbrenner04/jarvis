...
`createRunControlHandlers` in `v2/src/daemon/daemon.ts` assembles run-control RPC behavior from four modules plus the shared context seam. RPC contracts stay in the transport table above; this section maps source files only.

| Module | RPC family | Primary context / deps |
| --- | --- | --- |
| `v2/src/daemon/daemon-run-control-context.ts` | (none — shared seam) | `createRunControlHandlerContext` builds `RunControlHandlerContext`: `registry`, `activeRuns`, `store`, `writeLoopExecutor`, `failureReporter`, `pipelineWaitObserver`, memory headroom, settle-delay state, review-progress maps |
| `v2/src/daemon/daemon-run-lifecycle-handlers.ts` | `start`, `list`, `pause`, `resume`, `kill`, `wait`, `dismiss`, `undismiss` | `RunControlHandlerContext`; deps: `handleWorkflowStart` from workflow admission, `pipelineDispatch`/`pipelineWait` callbacks for pipeline stage dispatch |
| `v2/src/daemon/daemon-workflow-admission-handlers.ts` | `check_workflow_start_claim`, `implement.recover`; exports `handleWorkflowStart`/`admitWorkflowStart` consumed by lifecycle and pipeline modules | `RunControlHandlerContext`; `resumeFinalizationOnly` from lifecycle for `implement.recover` |
| `v2/src/daemon/daemon-pipeline-handlers.ts` | `pipeline_start`, `pipeline_approve`, `pipeline_reject`, `pipeline_resume`, `pipeline_recover`, `pipeline_dismiss`, `pipeline_undismiss`, `pipeline_list`, `pipeline_wait`; startup seam `continueContinuablePipelines` | `RunControlHandlerContext`; deps: `pipelineDispatch`, `pipelineWait`, `admitWorkflowStart`, `resolveStage`, recovery/publication/stale-reset injectables |
...
`pipeline_resume` (`daemon-pipeline-handlers.ts`) is the sole daemon-owned stage-scoped resume entry point. It composes `derivePipelineState`, `reopenFailedPipeline`, `claimPipelineContinuation`, and `continuePipeline` in `pipeline-execution.ts` — never translating resume into `pipeline_start` or run-level `resume`.
