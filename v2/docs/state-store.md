...
- `createRun` — insert a run (`in-progress`, zero attempts); accepts optional `stepId` for multi-step workflows; returns its ID.
...
- In one successful `write-write` workflow run, the store records separate run
  and attempt history for both `(project, branch, stepId)` entries. If step one
  takes multiple write-loop attempts before completing and step two then
  completes in a different number of attempts, those histories remain isolated
  rather than collapsing into one shared workflow record.
