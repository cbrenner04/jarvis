...
When a write-step idle-output stall settles `idle_output_timeout` after the boundary checkpoint produced a fresh `iteration_commit` `commitSha`, terminal `loop_finished.resumable: true` projects `error.nextAction: "resume"`. Recovery is `jarvis run resume` on the retained branch and worktree — not `resetStaleWorkspace`, not a full workflow re-dispatch. A stall with no checkpoint commit (`resumable: false`) stays `nextAction: "stop"`; re-dispatch the workflow after inspecting `jarvis run log`.
