...
function harness(overrides: { live?: boolean; durable?: boolean; failRemove?: boolean } = {}) {
  ...
  const runner: AsyncSubprocessRunner = {
    async runAsync(command, args) {
      ...
      if (overrides.failRemove && command === "git" && args[0] === "worktree" && args[1] === "remove")
  ...
  return {
    ...
    deps: {
      ...
      listDurableRuns: (): Run[] =>
        overrides.durable
          ? [{ project: "demo", worktreePath: "/home/worktrees/demo/plan/nested", status: "in-progress" } as Run]
          : [],
      listLiveRuns: async (): Promise<DaemonListRunRow[]> =>
        overrides.live
          ? [{ runId: "live", project: "demo", branch: "plan/nested", status: "in-progress", isLive: true }]
          : [],
    },
  };
}
...
describe("cleanupMergedWorkspaces", () => {
  test("discovers slash-nested merged worktrees and dry runs without mutations", async () => {
    ...
    expect(result.candidates).toEqual([
      { project: "demo", root: "/repo", path: "/home/worktrees/demo/plan/nested", branch: "plan/nested" },
    ]);
  ...
  test("removes only the worktree and local branch, isolating failures", async () => {
    ...
    expect(success.calls).toContainEqual([
      "git",
      ["worktree", "remove", "--force", "/home/worktrees/demo/plan/nested"],
    ]);
