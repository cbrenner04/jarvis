...
- [x] `v2/src/execution/diff-derived-mutation-verifier.test.ts` gains regression `keeps an independent guard nested inside a double-negated operand`, proving `!!foo(!bar)` still derives `!bar`; it fails against a bare `return` after the double-negation check, which abandoned the whole operand subtree.
- [x] `v2/src/execution/diff-derived-mutation-verifier.test.ts` gains regression `respects line-scoped guard admission boundaries`, which proves a negated expression on an unchanged line yields no guard candidate, a negated expression whose AST span crosses multiple lines yields no candidate and no partial span, and guard-first ordering with deduplication is preserved when a guard and an operator candidate share a changed line. Corrected 2026-09-13: this criterion previously claimed the regression fails against the pre-fix derivation, which is untrue — every one of its sub-cases derives identically under the deleted regex (the unchanged line matched nothing either way, the old `!(?:\([^)]+\))` alternative required a closing paren on the same line so the multi-line case also yielded nothing, and the ordering case derived the same two candidates). It pins the boundary contract against future regression; the derivation change is discriminated by the member-call-chain, parenthesized-operand and double-negation criteria, which assert the derived span.
