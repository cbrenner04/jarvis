...
**3. Jarvis's slice layout asserted as fact — `prompts/plan/draft.md:57`.** Names `bun run test:v2` / `test:integration:v2` for `v2/**`, "the v1 pair" for `v1/**`, and "all six" for `shared/**`, prefixed *"for this repo"* and suffixed *"following target-repo `AGENTS.md`"*. The hedges are real but the concrete script names are what an agent acts on, and no other repo has these surfaces.

**Correctly parameterized, for contrast:** `prompts/patch/rules.md:27,29` says *"Use commands from target repo `AGENTS.md`"* and *"Resolve scope … exactly as target-repo `AGENTS.md` specifies"*, naming no scripts. That is the pattern the three above should follow. (`patch/rules.md:30-31` still hardcodes `bun run typecheck` / `bun test`, but patch is v1-only and lower stakes.)
