...
- Plan-draft flat and nested spec-path layouts, implement actuator optional sections, v1 plan-review path substitution, v1 verdict-actuator flat/nested layout, and v1 patch optional-section omission are expressed through template variants or optional sections instead of post-render patches.
- `stripOptionalSection` and `.replace(` on assembled prompt strings are absent from `shared/prompts/` and the v1 plan-review, verdict-actuator, and patch prompt assembly paths.
...
- [ ] `shared/prompts/no-prompt-surgery-guard.test.ts` fails when `stripOptionalSection` or `.replace(` appears on assembled prompt strings under `shared/prompts/` or in `v1/src/modes/plan/review.ts`, `v1/src/modes/plan/verdict-actuator.ts`, or `v1/src/modes/patch/prompt.ts`.
