...
- Exit 2 (`all-agents-quota`) requires every attempted agent to have returned `quota`. A chain that ends without success but mixed quota with a `model_config` fallthrough exits 3, not 2; the exit-2 quota message must only fire when no non-quota failure occurred. Rules out: emitting the all-agents-quota message/exit on a mixed chain where at least one agent failed for a non-quota reason.
