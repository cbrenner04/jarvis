...
The snapshot covers a top-level entry-name listing of `sessions/` (not full recursive — it can hold ~1.24M files) and a bounded-depth recursive listing of `specs/`, plus `telemetry.jsonl`'s size/mtime. It is a read-only diff, not a write fence: a concurrent operator daemon legitimately writing `sessions/` or `telemetry.jsonl` to the real home while the suite runs can trigger a false positive; an allowlist for that case is deferred until one is observed. Both functions are pure (`homeDir` is always caller-supplied) and tested with temp-dir fixtures in `test/real-home-guard.test.ts`.
