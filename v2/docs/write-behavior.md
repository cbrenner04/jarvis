...
Before the first ready-gate repair iteration, Jarvis derives and freezes an allowset from committed `<baseRef>...HEAD` paths plus the resolved spec scope. A directory spec scope, and the parent of an `index.md`, allows descendants; a standalone spec file allows only itself. After each repair iteration and before its repair completion commit or republish, Jarvis validates the exact candidate set that commit would stage — additions, deletions, type changes, tracked ignored changes, submodules, and both rename sides, read with NUL-delimited Git output — against that frozen allowset. Candidate paths are normalized to repository-relative form without lossy decoding or repository escape; the first out-of-scope path is reported in deterministic byte order with JSON-escaped evidence. A violation returns retryable `completion_commit_failed` naming that path and does not run the repair completion commit or republish. Primary completion, mutation repair, and the bounded repair loop itself are outside this fence; in-scope repairs keep the existing three-attempt repair behavior.

Jarvis persists the frozen allowset on the durable run row as soon as it is derived, before repair completion can fail, together with rejection provenance when a violation lands: the normalized offending path and `completion_commit_failed`. Completed-run retry and `jarvis run resume` reconstruct that persisted fence instead of re-deriving an allowset from the dirty worktree. Recovery enforces the fence only when rejection provenance is present (`rejectedPath` or `completion_commit_failed`): before their generic completion commit or publication tail runs, those routes validate the exact candidate set that commit would stage against the persisted allowset. When no fence was ever established, recovery skips enforcement. When a fence column holds JSON that cannot be parsed or validated, recovery fails closed as retryable `completion_commit_failed` rather than treating the fence as absent. A successful ready-gate repair that left no rejection provenance does not keep the stale frozen allowset enforced on later generic completion.

Review-mutation recovery (`resumeReviewMutationFinalization`, including `implement.recover` and write-row `ready_gate_out_of_scope` admission that shares the same tail) is a separate production route: it resolves the completed write-step sibling row and recovers that row's persisted fence and rejection provenance before staging, publishing, or replaying the publication tail. When rejection provenance is present, a rejected out-of-scope path stays dirty and yields `completion_commit_failed`; this route cannot commit or publish it. Mutation-repair commits are outside the ready-gate repair fence even when a persisted allowset exists without rejection provenance.
