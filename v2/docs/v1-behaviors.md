...
- Fresh plan mode requires a positional seed. The single positional argument is treated as a file only when the path exists; otherwise it is treated as inline text, including path-like strings such as `./missing.md`. There is no fresh no-argument mode. Sources: `v1/src/commands/plan-args.ts`, `v1/src/commands/plan.ts`
- Fresh file and inline seeds share one entry path: jarvis seeds `intent.md`, preserves the exact raw seed in a dedicated block, preserves non-`name` frontmatter keys for file seeds, runs one intent-draft pass that rewrites `intent.md`, and expects that pass to propose `name:` in frontmatter or fall back to deterministic derivation. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`, `prompts/plan/intent-draft.md`
- Intent-draft, refine, and draft prompts all forbid self-referential deliverables: acceptance criteria must verify target state outside the active spec directory (code/tests/docs/operator behavior/generated evidence), so criteria that only grade active-spec prose are invalid even without saying "the spec." Sources: `prompts/plan/intent-draft.md`, `prompts/plan/refine.md`, `prompts/plan/draft.md`
...
- Seeded plan flows run intent draft, then `plan: intent` (when `commit: true`), then refine (`--refine-turns`, default `1`). Draft and review run in the same invocation when `commit: false`, or after `jarvis1 plan --resume-draft <intent>` when `commit: true`. Review cycles are controlled by `--review-passes` (default from `modes.review.passes`, currently `2`; each cycle = debate role sequence) with explicit per-cycle stderr milestones. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`, `v1/src/modes/plan/commits.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/review.ts`, `v1/docs/plan-mode.md`
- For fresh `commit: true` seeded plans (file or inline), the run stops after `plan: intent` (and `plan: refine` when `--refine-turns > 0`): jarvis opens/updates the branch's open draft PR, prints review-then-`--resume-draft` next steps, and exits `0`. Draft/review run only after `jarvis1 plan --resume-draft <intent>`. `commit: false` flows continue through draft and review in one invocation. Sources: `v1/src/commands/plan.ts`, `v1/docs/plan-mode.md`
- When `--refine-turns 0` is set on a fresh seeded run, jarvis still runs the intent-draft step, commits `plan: intent`, then skips refinement entirely. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`, `v1/src/modes/plan/commits.ts`
...
| --- | --- | --- | --- |
| inline (text arg) | `true` | seed `intent.md` -> intent draft -> `plan: intent` -> refine (optional) -> PR handoff | exits `0`; operator reviews PR then runs `--resume-draft` for draft+review |
| inline (text arg) | `false` | seed `intent.md` -> intent draft -> refine -> draft -> review | exits `0`; prints `jarvis1 run <indexPath>` next step |
| file (path arg) | `true` | seed `intent.md` -> intent draft -> `plan: intent` -> refine (optional) -> PR handoff | exits `0`; operator reviews PR then runs `--resume-draft` for draft+review |
| file (path arg) | `false` | seed `intent.md` -> intent draft -> refine -> draft -> review (into external spec root) | exits `0`; prints `jarvis1 run <indexPath>` next step |

Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`

- `--resume-draft` rejects genuine `## Blocker` sections in `intent.md` but ignores legacy jarvis-generated review-gate blockers. Sources: `v1/src/modes/plan/blocker.ts`, `v1/src/commands/plan.ts`
