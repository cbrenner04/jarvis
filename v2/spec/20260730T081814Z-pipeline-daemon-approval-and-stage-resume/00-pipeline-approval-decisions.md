...
- [ ] A regression in `v2/src/daemon/pipeline-execution.test.ts` fails on baseline and then proves a pipeline that reaches an approval gate records `awaiting` on that row and dispatches no later workflow stage until `pipeline_approve` applies a matching decision.
