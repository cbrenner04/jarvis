...
export function loadPrices(path?: string): Prices {
  ...
  try {
    ...
    throw new Error(
      `Failed to read prices file at ${resolvedPath}: ${(err as Error).message}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON in prices file at ${resolvedPath}: ${(err as Error).message}`,
    );
  }

  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error(
      `Prices file at ${resolvedPath} must be a JSON object, got ${typeof parsed}`,
    );
  }
  ...
  if (obj.version !== 1) {
    throw new Error(
      `Prices file at ${resolvedPath} has unknown version ${JSON.stringify(obj.version)} (expected 1)`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `models field in prices file at ${resolvedPath} must be an object`,
    );
  }

  const models: Record<string, PriceRow> = {};
  for (const [modelId, rowRaw] of Object.entries(
    ...
    if (
      ...
      throw new Error(
        `Price row for model ${JSON.stringify(modelId)} must be an object`,
      );
    }
    ...
    if (typeof inputRate === "number" && inputRate < 0) {
      throw new Error(
        `input_per_mtok for model ${JSON.stringify(modelId)} must be non-negative, got ${inputRate}`,
      );
    }
    ...
    if (typeof outputRate === "number" && outputRate < 0) {
      throw new Error(
        `output_per_mtok for model ${JSON.stringify(modelId)} must be non-negative, got ${outputRate}`,
      );
    }

    // Validate optional cache fields
    ...
    if (typeof sourceUrl !== "string") {
      throw new Error(
        `source_url for model ${JSON.stringify(modelId)} must be a string, got ${typeof sourceUrl}`,
      );
    }
    ...
    if (typeof asOf !== "string") {
      throw new Error(
        `as_of for model ${JSON.stringify(modelId)} must be a string, got ${typeof asOf}`,
      );
    }

    // Build the price row, preserving all fields including unknown ones
