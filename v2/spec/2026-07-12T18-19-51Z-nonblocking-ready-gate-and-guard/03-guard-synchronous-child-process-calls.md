...
With every daemon-reachable subprocess awaited, make regression impossible: a static guard fails the normal gate when `v2/**` or `shared/**` code reintroduces a synchronous child process.
...
- Rejected constructs: importing `execSync`, `execFileSync`, or `spawnSync` from `node:child_process`, and `Bun.spawnSync`. Additionally, `v2/**` may not import `SubprocessRunner` or `realSubprocessRunner` — the sync seam is reachable from the daemon otherwise.
...
- [ ] The guard exits non-zero and names file, line, and construct when a synchronous child-process call is added to a non-test `v2/**` or `shared/**` module, and exits zero on the current tree.
- [ ] The guard exits non-zero when `v2/**` non-test code imports the synchronous `SubprocessRunner` seam.
...
- [ ] A unit test for the guard covers a violating fixture, an allowlisted fixture, and a test-file fixture.
