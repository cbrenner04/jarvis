...
- [ ] `write-loop.test.ts` test `ready-gate repair autofix invokes configured fixCommand` stays green (configured `fixCommand` still uses `runFixCommand`; scoping deferred).
- [ ] `write-loop.test.ts` test `ready-gate repair autofix greens a formatter-only red gate without repair iterations` stays green.
- [ ] Mutation checkpoint: `write-loop.test.ts` links `// @mutate` inverting the scoped built-in autofix guard; inverting turns `ready-gate repair autofix ignores pre-existing out-of-diff lint findings` red.
