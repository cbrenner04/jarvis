...
**Status counts.** Status begins `N running · N awaiting gate · N failed · N done`, counting one item per distinct retained pipeline ID and one per genuine ad-hoc workflow group or standalone row. Every pipeline snapshot comes from the one stable connection, so classification needs no cross-socket dedup or merge. A reachable undecided gate is `awaiting gate`, other pending or active work is `running`, success is `done`, and rejection, failure, or interruption is `failed`. An ad-hoc group is `running` when any member is active or its workflow rollup is non-terminal, `done` only for a fully terminal completed rollup, and otherwise `failed`; ad-hoc work is never `awaiting gate`. Workflow invocations matched to pipeline stages count only through their pipeline; queued rows remain Queue-only and contribute to no status count. All four counts stay leftmost before invoking `profile@socket-digest`, refresh, and both retained feedback channels — `· error: <rpc-error>` first, then `· result: <command-result>` — so right truncation removes metadata and feedback before primary work status. Neither feedback channel hides the other. Connection, `list`, and `pipeline_list` failures retain last-good rows and snapshots; only a fully successful refresh clears the error, and refresh never clears a retained command result. Retained rows without a connected client cannot be killed until reconnection succeeds.
...
Dismissed pipelines and dismissed runs are both hidden by default. Press **`D`** in tree focus to show them for this session only — the one toggle covers runs and pipelines together, re-requesting both `list` and `pipeline_list` with `includeDismissed: true`; dismissed rows paint with a `(dismissed)` marker, and **`D`** again hides them without waiting for snapshot eviction (including rows retained from an earlier list result or in flight during a toggle-off refresh). The toggle is not persisted; every new monitor session starts hidden. `jarvis tui log <run>` streams a dismissed run's log the same as an undismissed one, regardless of the toggle. Dismiss via `jarvis run dismiss` / `jarvis pipeline dismiss` (see [operator-runbook.md](./operator-runbook.md#pipeline-dismiss-and-undismiss)).

## Stable connection and reconnection

`jarvis tui` and `jarvis tui log` hold exactly one connection, to the stable socket — no client-side socket discovery and no cross-socket ownership map. When dispatch moves to a new executable digest, the new daemon binds the same stable public address and its own `list`/`pipeline_list` handlers already fold a live draining predecessor's runs and pipelines into their answer, so the monitor keeps showing both generations' work through that one connection with no client-side merge; once the predecessor exits naturally, its runs drop out of the stable daemon's own answer. `jarvis tui log <run-id>` streams from the stable socket directly, with no owner lookup: every generation shares the durable run-log store, so the stable daemon can tail any run's log regardless of which generation executed it.

**Transport loss recovery in `jarvis tui log`:** on a mid-stream transport loss (daemon restart, network hiccup), the tail automatically re-opens against the same stable socket and resumes from the last appended record sequence, with no duplicate or lost records and no operator action. If reconnection attempts are exhausted (default: 5 retries with exponential backoff, 100 ms to 2 s), the session shows `tail_resume_exhausted` error feedback and exits with code 1. Operator quit during a retry wait returns cleanly with exit code 0.

The monitor's stable-socket client is not exempt from eviction: when its `list()` RPC fails, the stale client is closed and cleared, and the next refresh tick attempts to reconnect to the same `deps.socketPath` — so if the daemon behind that socket restarts, the TUI reconnects automatically once it starts answering again. There is no fallback to any other socket at any point: a route the stable daemon cannot serve surfaces as the daemon's own error.
