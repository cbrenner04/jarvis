...
- **PR CI scopes the `Test` job by changed path** (`scripts/ci-test-scope.ts`): a `v1/**`-only diff runs `test:v1`, `v2/**`-only runs `test:v2` + `test:integration:v2`, `shared/**` runs all three, and root-tooling/unmatched/unresolvable-base diffs fall back to the full `bun run test`. So a PR's CI may show only a subset of test steps under one `Test` job — that's expected, not a skipped check. Pushes to `main` always run the full suite.
