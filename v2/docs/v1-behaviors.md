...
- Review behavior snapshots current spec markdown into prompt context, forbids `intent.md` rewrites except blocker append, validates `index.md` still exists, and can skip committing when no changes were produced. Plan review routes through the shared review runner in `v1/src/modes/review/` with a plan adapter; review agents resolve from `modes.review.agentOrder ?? modes.plan.agentOrder` and cycle counts from `resolveReviewPasses(cfg, --review-passes)` (`modes.review.passes`, default `1`, where each cycle runs adversary → advocate → adjudicator → actuator). Plan reviewers (adversary/advocate/adjudicator) are read-only on the spec; any spec edits during reviewer roles are reverted by the harness. Per-role artifacts are stored as temporary files and injected into successor roles (e.g., adversary findings → advocate, advocate response → adjudicator). The adjudicator's verdict drives one refine-loop invocation (the verdict actuator), which applies the verdict as a spec refinement task under the existing plan write boundary. Verdict is persisted as `verdict-plan.md` in the spec folder, overwritten each cycle. Per-role commits are emitted with messages `review: <role>` for reviewer roles and `review: actuator` for the actuator. Empty verdicts skip actuator invocation and do not create an actuator commit. Sources: `v1/src/modes/plan/review.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/review/run.ts`, `prompts/plan/review-*.md`, `v1/src/commands/plan.ts`, `v1/src/config.ts`
