...
**TUI and CLI cross-daemon observation:** Both `jarvis tui` and `jarvis run list` automatically discover and aggregate runs from all live keyed daemons. When dispatch moves to a new digest (via recompiled executable), both tools automatically find runs from the old (superseded) and new (superseding) daemons on the next refresh or invocation. No restart is required. Once the old daemon exits naturally, its runs are removed, and both tools continue uninterrupted. Deduplication prefers rows reported `isLive` by any daemon, so in-flight runs on either daemon are visible. In contrast, `jarvis run wait` remains scoped to the daemon the run was originally launched on. This allows both TUI and `run list` to track work across digest transitions while `run wait` remains focused on a specific daemon.
