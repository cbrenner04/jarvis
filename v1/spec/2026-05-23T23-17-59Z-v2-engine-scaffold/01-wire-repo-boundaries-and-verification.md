...
order updates that explain why the scaffold exists.
...
- Update the minimal root-facing docs needed for command discoverability if that
  note was not completed as part of the first subspec.
...
- [ ] Root-facing documentation tells readers that `jarvis1` is still the real
      v1 harness command and bare `jarvis` is reserved for the intentionally
      minimal v2 scaffold, without broadening into a larger v2 usage guide.
...
- Update one existing root-facing doc, likely [README.md](/Users/christopherbrenner/Work/jarvis/.worktree/plan-v2-engine-scaffold/README.md),
  with the minimal command-discoverability note described above.
