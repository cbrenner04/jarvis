...
`loopOutcomeKind` in `ready_gate_failed`, `ready_gate_out_of_scope`, `surviving_mutation_failed`, `completion_commit_failed`,
`iteration_commit_failed`, or resumable `iteration_timeout` outranks last-attempt store detail; otherwise last-attempt detail wins over conflicting
