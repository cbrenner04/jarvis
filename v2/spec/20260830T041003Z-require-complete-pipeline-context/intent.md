...
Artifact contract check failed: Plan subspec 01-pipeline-admission-context-gate.md has a multi-surface ## Acceptance criteria bullet: `daemon-pipeline-start.test.ts` — `pipeline_start persists supplied context before returning pipelineId` stays green, and a sibling test asserts `runPipeline` receives reloaded persisted `configPath`/`cwd` when RPC bytes differ; the sibling fails against the pre-fix handler that forwards RPC `context` directly.
