...
- [x] Extend the v1 config schema/defaults with `modes.review.{passes,agentOrder}` and validation for non-negative `passes` plus the same `validateAgentOrder` contract used by `modes.patch`/`modes.plan` for `agentOrder` when provided (reuse that path; do not add a review-only rule).
- [x] Add `jarvis1 run --review-passes <n>` with the same non-negative integer validation style as plan mode.
- [x] Add one resolver for effective review passes and review agent order from CLI overrides, `modes.review`, and `modes.plan.agentOrder`.
- [ ] Reuse existing patch-mode model/quota selection once a review agent/model pair is resolved.
- [x] Add unit coverage for config loading and CLI parsing failures and override precedence.

## Documentation updates

- [x] Update `v1/docs/config.md` for the new `modes.review` block, its fallback to `modes.plan.agentOrder`, the shared `validateAgentOrder` validation of `agentOrder`, and the default review-pass count.

## Acceptance criteria

- [x] `jarvis1 run --review-passes 0 <spec>` disables the review loop without changing implementation iteration behavior or requiring config edits.
