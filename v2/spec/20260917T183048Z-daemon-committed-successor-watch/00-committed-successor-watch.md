...
- [ ] A test asserts a successor that still holds an open listener but answers the watch's `health` RPC too slowly is never displaced: the rebind attempt's bind fails with the address in use, `removeUnansweredSocketPath` classifies the still-listening successor `live` (per `v2/src/ipc/server.test.ts`'s "refuses to unlink a live peer socket" guarantee), the socket is left alone, and the watch reschedules without marking the transaction `rolled_back` or logging a rebind trigger.
- [ ] A test asserts `close()` stops the watch: no further probe, rebind, or log line occurs on the next tick after `close()` runs.
- [ ] `v2/src/daemon/daemon.ts` exports a pure predicate deciding whether a committed-handoff watch tick must rebind, tested in both directions without a real-timer wait.
