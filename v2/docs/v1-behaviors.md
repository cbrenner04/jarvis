...
- Seeded plan flows run intent draft, then `plan: intent` (when `commit: true`), then refine (controlled by `--refine-turns`, default `1`), then draft phase, then review cycles (controlled by `--review-passes`, default `1`, where each cycle = debate role sequence) with explicit per-cycle stderr milestones. Sources: `v1/src/commands/plan.ts`, `v1/src/modes/plan/intent-draft.ts`, `v1/src/modes/plan/commits.ts`, `v1/src/modes/plan/refine.ts`, `v1/src/modes/plan/draft.ts`, `v1/src/modes/plan/review.ts`
