...
**Accepted-equivalent audit.** After publication-time mutation verification accepts exact `@mutate-equivalent` directives, run `jarvis run log <run-id>` and inspect each `accepted_equivalent_mutation` record's file, line, full mutation, and reason against the PR diff before merging. Events are written once verification passes, even when a later publication-tail step fails terminally. No record is emitted when the verifier accepted no sites.
