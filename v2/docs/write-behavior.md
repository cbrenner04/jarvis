...
| `jarvis run list --since <duration\|timestamp>` | Optional `--since`: relative duration (`<positive-integer><d\|h\|m\|s>`, subtracted from query time) or absolute Unix ms / ISO 8601; cutoff is inclusive on `created_at`; bypasses the default fifty-terminal-run retention window | Same row format as `jarvis run list` | `0` on success; `1` with `invalid_since` before any `list` RPC when `--since` is invalid |
| `jarvis run list --since ... --limit <positive-integer>` | Optional `--limit` on filtered queries (`--since` and others): returned results capped to the given limit or default 200 if omitted; newest-first order. Bare `--limit` without filters is ignored (no effect on unfiltered `run list`) | Same row format as `jarvis run list` | `0` on success; `1` with `invalid_limit` before any `list` RPC when `--limit` is invalid |
