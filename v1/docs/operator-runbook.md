...
- **Stuck-red completion (exit 10).** The completion gate failed repeatedly (either identically or with changing failures), the fix-up commits have been discarded (reset to the first-red baseline and force-pushed with `--force-with-lease`), and the PR is left at the original completed work without the chase edits. Recovery: fix the underlying issue (e.g., a linting rule, a missing import, a flaky test). If the gate should now pass, use `jarvis1 triage <worktree-name> --mark-ready` to re-run the gate once and promote the PR on green; otherwise rerun `jarvis1 run <spec>` to retry the fix-up iteration. The discarded edits remain accessible via git reflog if needed for reference.
