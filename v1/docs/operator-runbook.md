...
- **One-run actuator probes:** use `jarvis1 run --agent` or `jarvis1 plan --agent` instead of editing config `agentOrder` (see [agents.md](./agents.md#per-run---agent-override)).
- **Prompt agent verification:** use `jarvis1 prompt --agent <name>[:<model>]` to probe one agent without mutating `modes.prompt.agentOrder` (see [agents.md](./agents.md#prompt---agent-override) and [specless-prompt.md](./specless-prompt.md)). Quota may still fall through to config suffix agents. Cross-mode repeatable override for `run`/`plan` is tracked in seed [per-run-agent-override-flag](../spec/completed/2026-06-30T17-57-01Z-agent-order-override-flag/index.md) (shipped for patch/plan; `intent` remains separate).
