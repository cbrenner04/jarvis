...
**Manual subspec-split recovery (stopgap — remove once `plan-subspec-one-iteration-sizing` ships).** The plan loop sometimes drafts a subspec too large to complete in one 10-min iteration on *any* agent tier — it times out (exit 8, often with zero/truncated file activity) across haiku and opus alike. Because fresh runs each time out only once, the auto-split blocker (which needs three consecutive timeouts *within one run*) may never fire, so the split falls to the operator. Recovery: `jarvis1 cleanup --abandon <spec-name>` the stuck worktree, branch off `main`, split the oversized subspec into 2–3 independently-testable subspecs (`git mv` to renumber, update `index.md` links, fix cross-references), PR + admin-merge (spec-only), then re-run fresh. Split by code path — builder / runtime wiring / validator are separate subspecs. Observed 2026-07-11 on three subspecs across `intent-workflow-reviewed` and `plan-workflow-draft`. Seeded as `v1/spec/seeds/plan-subspec-one-iteration-sizing.md`.
