...
`runV2TestFiles` executes an ordered schedule of bounded-pool batches of `bun test <file>` children. The first batch contains unclassified and `poll-until-done` files, the second contains `subprocess-spawning` files, and each load-sensitive file gets a later single-file batch in roster order; each batch fully drains before the next starts. `test:v2` and `test:integration:v2` inherit this seam, and `scripts/run-tests.ts` routes its integration phase through it for the same per-file timeout and captured-output attribution. Every integration file matches `.sandbox-unrunnable.test.ts`, so that slice remains serial. `test:cost` (`scripts/measure-test-cost.ts`) stays serial because it is a measurement tool, not the gate.

The concurrency limit defaults to half of `availableParallelism()` (floor 1) — `defaultConcurrency` in `scripts/run-v2-tests.ts`. This is deliberately headroom-driven, not tuned to the pooled phase's performance knee: the pooled phase plateaus around 4 concurrent files (bounded by the slowest pooled file, then `v1/test/run.test.ts` at 108.8s; the v1 tree is now frozen and outside every slice), well below half of an 18-core box's parallelism. The extra headroom guards against self-saturation, the condition under which known load-dependent test flakes reproduce — trading a wider margin for less contention risk, not chasing peak throughput.
...
Wall clock: theoretical floor is `158.7 + max(pooled / N, 108.8)` ≈ 267s (158.7s is the isolated sandbox-unrunnable phase from subspec 02, 108.8s was the pooled-phase floor set by the since-frozen `v1/test/run.test.ts`). Measured on quiet operator hardware (2026-07-26, five consecutive `bun run test` runs): 321-330s, mean 326s, ≤335s regression bar — **superseded** by the 2026-08-18 audit below, which moved two more heavy files into the no-co-runner lane. Measured on quiet operator hardware (2026-08-18, three consecutive `bun run test` runs): 388-389s, mean 388s; **388s (mean, 388-389s range)** is the current aggregate `bun run test` wall clock, with **≤398s** as the regression bar. One run hit a pre-existing sandbox-environment-only failure unrelated to this change (`v1/test/idle-hang-fixtures.sandbox-unrunnable.test.ts` polls for a real spawned process via a process-table probe the coding-agent sandbox blocks; confirmed failing identically on `main`) that stops the aggregate runner from admitting further files, so these three runs drove the same `agent`/`integration` rosters `bun run test` uses directly through `runV2TestFiles`, with that one file's roster position excluded (its own isolated cost is ~4s either way, negligible to the total). For comparison, the pre-change aggregate `bun run test` was 697s (2026-07-26, before the concurrency change) and a separate, differently-measured `bun run test:cost` pass was 848.6s (2026-08-28, see "Measured aggregate cost" below; 574.4s on 2026-07-26 over the then-229-file roster) — each figure labeled by the command and date that produced it.

Stop semantics carry across batches: a plain (non-timeout) failure stops every mode, including `agent`, from admitting new files or starting later batches; files already in flight are still awaited and reported. A timeout does not stop `agent` mode, which keeps admitting and reports every timed-out file by name; a timeout in any other mode stops admission. A failed aggregate therefore reports the first failure boundary, not necessarily every failing file. Each child's per-file timeout is armed independently at spawn, and output blocks print in settle order within a concurrent batch.

### Declared isolation classes

A v2 test file that conflicts with a specific other kind declares one exact top-level marker: `export const TEST_ISOLATION_CLASS = "poll-until-done";` or `export const TEST_ISOLATION_CLASS = "subprocess-spawning";`. The scheduler reads that one-line declaration textually; conflicting or unrecognized declarations fail the run. Poll-until-done suites cannot share a batch with subprocess-spawning suites because nested subprocess pools can starve scheduler polls until Bun's 30000 ms test timeout even when both files pass alone. This contract applies only to the v2 lane; the shared lane remains one `bun test --parallel` invocation.

### Load-sensitive isolation

Some files need solitude under machine load even without a conflict with one declared kind. `isLoadSensitive` in `scripts/test-slice.ts` covers every `*.sandbox-unrunnable.test.ts` file plus the explicit `LOAD_SENSITIVE_FILES` roster; each explicit entry names its observed failure. Use that roster for a file that must have no co-runner, and use `TEST_ISOLATION_CLASS` for a file that conflicts only with the other declared class. Load sensitivity wins when both apply. Changes to `LOAD_SENSITIVE_FILES` are operator execution-policy decisions, not ready-gate repair. `isSandboxUnrunnable` remains the separate slice-partition key.

**Lane join rule:** a file joins `LOAD_SENSITIVE_FILES` when a dated observation already on record shows it red under concurrent load and green idle — either a CI/full-suite failure or a local concurrent-load run reproducing it — not on local reproduction alone; a quiet operator box that fails to reproduce a loaded-runner flake doesn't disqualify a candidate that already has recorded evidence. The entry's comment records that observation. A candidate that survives the audit's local load rounds with no dated evidence stays pooled.
...
**2026-08-25 (durable #2181 fix):** the `v2/src/execution/workflow-runner.test.ts` monolith (~12k lines / 224 tests, isolated above for tipping the per-file budget) was split into concern-grouped co-located files beside `workflow-runner.ts` — `workflow-runner-core`, `-intent`, `-plan`, `-publication`, `-resume`, `-debate`, `-review`, `-review-standard`, `-validation` (`.test.ts`) — sharing `workflow-runner.test-support.ts`, with the full 224-case inventory preserved; those split files are well under the per-file health budget, so the monolith's `LOAD_SENSITIVE_FILES` entry was removed and the split files run pooled; `SUPPORTED_HEALTHY_FILE_BUDGET_MS` returned to `180_000` (the #2992 stopgap is reverted). **Post-extraction (2026-09):** resume-machine production code and tests co-locate beside `workflow-runner-resume.ts` as `workflow-runner-resume*.test.ts`. **2026-09-16 split:** the primary co-located resume test file (~5.2k lines) was split by `describe` group into `workflow-runner-resume-review-dispatch`, `-review-mutation-repair-fence`, `-review-mutation-markdown-fence`, `-recover-plan-stage`, `-recover-plan-stage-review-failed-admission`, `-reconstruct-paused-write`, `-resolve-write-non-terminating`, `-intent-finalization-seed-consumption` (`.test.ts`), sharing `workflow-runner-resume.test-support.ts`; each split file runs well under the 40s per-file cap that triggered the split (`bun test <file>` run alone), and the full leaf-title inventory is preserved. `workflow-runner-resume-inventory.test.ts` pins merge-base parity (`test.each` expansion) across per-source buckets from merge-base `workflow-runner-resume.test.ts` (full file), `workflow-runner-plan.test.ts` (`describe("recoverPlanStage")` only), `recover-review-failed-plan-draft.test.ts` (`recoverPlanStage review-failed admission` in full), and the zero-case `workflow-runner-publication.test.ts` bucket; the destination scan (every `workflow-runner-resume*.test.ts` sibling) now spans the split files above. Split a sibling further only when it approaches the 40s per-file cap; isolate a specific split file (with dated loaded-red/idle-green evidence) only if one later proves load-sensitive.

`runV2TestFiles` gives each load-sensitive file a single-file batch after both declared-class batches, so it has no co-runner in either direction. `agent` mode keeps admitting past its timeout and stops on a plain failure; every other mode stops after either.

### Ready-gate step budgets

`bun run ready` (`scripts/ready.ts`) arms each step with its own fixed budget, not a shared remainder of one deadline. `TEST_STEP_BUDGET_MS` is **15 minutes** (900000ms) and `JARVIS_READY_TIMEOUT_MS`'s default `DEFAULT_TIMEOUT_MS` is **30 minutes** — both were sized against the pre-concurrency 697s aggregate `bun run test` and are **deliberately unchanged here**: the concurrent runner's measured 388s mean (see "Wall clock" above, 2026-08-18) is 43% of `TEST_STEP_BUDGET_MS`, only a slightly narrower margin than the superseded 326s figure's 36%, and still well under half the step budget — re-sizing a gate budget is its own reviewable risk. Re-sizing is tracked as a follow-up: [cbrenner04/jarvis#2181](https://github.com/cbrenner04/jarvis/issues/2181). Each scoped test step (`test:v1`, `test:v2`, `test:integration:v2`, …) is its own step with this same budget — a `shared/**` diff that scopes to all three slices runs three separate steps, and it is the **run ceiling**, not this per-step budget, that must have enough headroom to cover their sum. Non-test steps get smaller fixed budgets (`INSTALL_STEP_BUDGET_MS`, `CHECK_STEP_BUDGET_MS`, `TYPECHECK_STEP_BUDGET_MS`, `LINT_MD_STEP_BUDGET_MS`). A step's budget does not shrink because prior steps ran long — it is armed fresh, capped only by `min(stepBudgetMs, ceilingMs -
runElapsedMs)`.

`JARVIS_READY_TIMEOUT_MS` (default `DEFAULT_TIMEOUT_MS`, 30 minutes) is the **run ceiling only** — a backstop over the whole `bun run ready` invocation, not a per-step timeout. It is sized so a flake-retry still arms a fresh full test budget: with the current measured aggregate `bun run test` at 388s (mean, 2026-08-18) and the other steps seconds each, a run with one serial test retry is ~13 minutes — well inside the unchanged 30-minute ceiling (previously ~12 minutes against the superseded 326s figure, ~24 minutes against the 697s pre-change figure). It deliberately does not cover the budget worst case (~38 minutes, every step consuming its full budget plus a retry); if the suite grows into that range the retry's budget is clamped by the ceiling and the kill is attributed to "run ceiling" in stderr. When the ceiling binds before a step's own budget would, the kill is attributed to "run ceiling" in stderr instead of "step budget". When a step budget itself is the binding limit, raise the relevant `*_STEP_BUDGET_MS` constant in `scripts/ready.ts` — there is no per-step env knob. Update `TEST_STEP_BUDGET_MS` (and `DEFAULT_TIMEOUT_MS` accordingly) if measured full-suite duration drifts — and re-run `bun run
...
#### Measured aggregate cost (2026-08-28, operator hardware)

One `bun run test:cost` run over the full aggregate roster (288 files, 0 unparsed, 0 timed out) measured: wall clock 848.6s, summed in-file execution 846.7s, residual 1.9s (0.2% of wall clock). Residual stays uniform per-file (2-11ms typical, one 50ms outlier at `v2/src/persistence/state-store.test.ts` — still within spawn-jitter range, not signal). The slowest file is `v1/test/run.test.ts` at 117.3s serial, consistent with its historical 109-120s band. The 2026-08-25 workflow-runner split is reflected: the nine `workflow-runner-*.test.ts` files measure 0.3-22.0s each (slowest `workflow-runner-resume` at 22.0s), all far under the per-file health budget, replacing the deleted monolith's 24.8s single-file entry from the prior (2026-07-26, 229-file, 574.4s) baseline.
...
## Staged-lint seam on plan resume

`recoverPlanStage` (`v2/src/execution/workflow-runner-resume.ts`) accepts an optional `runner?: AsyncSubprocessRunner` on its `PlanStageRecoveryRequest`, forwarded through `lintPlanRecoveryStage` into `lintStagedMarkdown` (`v2/src/execution/staged-markdown-lint.ts`); the production default stays `realAsyncSubprocessRunner`, resolved inside `lintStagedMarkdown` itself.

`workflow-runner-resume-recover-plan-stage.test.ts` carries one unit test ("uses the injected runner for plan-recovery staged lint") that stubs the markdownlint call and asserts recovery observes the stub's reported violation, proving the seam is wired. One real-binary test covering the same recovery path lives in [`plan-recovery-staged-lint.sandbox-unrunnable.test.ts`](../src/execution/plan-recovery-staged-lint.sandbox-unrunnable.test.ts) for genuine end-to-end coverage of `recoverPlanStage` against the pinned `markdownlint-cli2` binary.

`resumePopulatedIntentPublication` (`IntentFinalizationResumeDeps.runner`) and `executeWriteLoop` (`WriteLoopInput.stagedMarkdownLintRunner`) forward an optional runner to the reviewed staged-lint gate; `executeWriteLoop` also forwards it to the intent-split landing autofix (`evaluateIntentSplitLandingGate`). `lintStagedMarkdown` skips the harness-root binary/config existence checks when a runner is injected, so stubbed calls need no installed binary. The daemon resume/recover tests, `write-loop-staged-markdown-lint.test.ts`, `write-loop-intent-landing.test.ts`, the `write-loop.test.ts` `runLoop` helpers, and the two resume calls in `workflow-runner-review.test.ts` stub it. Real-binary coverage lives in [`write-loop-staged-markdown-lint.sandbox-unrunnable.test.ts`](../src/execution/write-loop-staged-markdown-lint.sandbox-unrunnable.test.ts) (MD012 reprompt), [`intent-resume-staged-lint.sandbox-unrunnable.test.ts`](../src/execution/intent-resume-staged-lint.sandbox-unrunnable.test.ts) (violation refuses publication), and `intent-output.sandbox-unrunnable.test.ts` (MD038 autofix applied). Measured on operator hardware (2026-09-16, another lane running): `write-loop-staged-markdown-lint.test.ts` (8 tests) 11.2s → 10.0s; `intent-output.test.ts` (15 tests) 15.95s → 1.31s; `write-loop-intent-landing.test.ts` (6 tests) 16.4s → 2.8s; `write-loop.test.ts` (322 tests) 44.4s → 41.6s; `workflow-runner-review.test.ts` (20 tests) 4.1s → 3.9s (its `executeWorkflow` review-actuator lint tests still spawn the real binary: `executeWorkflow` has no runner seam and is not a guarded entry point).

## Real-lint structural guard

`scripts/guard-real-lint-in-unit-tests.ts` (wired into `bun run check`) scans unit-slice test-file sources (`.test.ts`, excluding `.sandbox-unrunnable.test.ts`) for calls into a fixed list of markdown-lint entry points, each of which defaults to `realAsyncSubprocessRunner` (a real `bun markdownlint-cli2` spawn), and requires that entry point's specific injection: a `runner` property for `runMarkdownlintAutofix`, `lintStagedMarkdown`, `landIntentWorkflowOutput`, `recoverPlanStage`, and `resumePopulatedIntentPublication`; a `stagedMarkdownLintRunner` property for `executeWriteLoop`; the positional runner argument (5th for `validateIntentStage`, 4th for `repairIntentStageContent`). An explicit `undefined` value does not count. The scan is lexical (import-binding + call-site match), not a runtime spawn trap: it catches an unexercised or newly-added call site. Limits: the property may sit in any object literal inside the call's parentheses; a call passing a prebuilt variable (`executeWriteLoop(input)`) is flagged even if the variable carries the runner, so spread it at the call (`{ ...input, stagedMarkdownLintRunner }`); helpers and indirect callers (e.g. `executeWorkflow`) are not checked; allowlisted files are not checked at all.

Allowlisted files: `shared/intent-stage.test.ts` and `v2/src/execution/staged-markdown-lint.test.ts` (direct real-binary assertions), and `v2/src/daemon/daemon-start-list.test.ts`, `v2/src/execution/write-loop.test.ts`, `write-loop-idle-watchdog.test.ts`, `write-loop-session-log.test.ts` (verified with a spawn trap — default runners made to throw — to never reach the real binary: their write-loop stages contain no `.md` files, or the `runLoop` helper injects a stub). `workflow-runner-resume-review-dispatch.test.ts`, `workflow-runner-resume-recover-plan-stage.test.ts`, `workflow-runner-resume-recover-plan-stage-review-failed-admission.test.ts`, `workflow-runner-resume-intent-finalization-seed-consumption.test.ts`, `workflow-runner-review.test.ts`, `write-loop-intent-landing.test.ts`, `daemon-resume.test.ts`, `daemon-pipeline-recover.test.ts`, and `pipeline-stage-recovery.test.ts` inject stub runners and are not allowlisted; the guard test pins that they are flagged. A single call site that provably never reaches the spawn (an error branch that returns before validation runs) can carry a `// guard-real-lint-in-unit-tests: <reason>` marker on its own line or the line above instead of an injected runner — see the three rogue-path rejection tests in [`intent-output.test.ts`](../src/execution/intent-output.test.ts).

**Test temp dirs.** Test code (`*.test.ts`, `*.test-support.ts`, `v2/src/testing/**`, the preload) allocates temp dirs through `trackedMkdtempSync`/`trackedMkdtemp` from [`shared/tracked-temp-dir.test-support.ts`](../../shared/tracked-temp-dir.test-support.ts), never `mkdtemp`/`mkdtempSync` directly; tracked dirs are removed recursively by the preload's `afterAll` once the process's tests finish (`bun test` never emits process `exit`), or by a catchable SIGTERM/SIGINT; a SIGKILL (the runners' timeout kill) leaks that process's dirs. `scripts/guard-test-temp-dir-cleanup.ts` (wired into `bun run check`) flags direct calls. Leaked dirs once grew the operator tmpdir past a million entries, slowing every bun startup there enough to red-gate daemon readiness tests.
...
- **Private startup deadline on a subprocess handshake** (forbidden): a test that spawns a real child and waits for a stdout marker must not put its own short wall clock on that wait (`state-store-wal-concurrency.test.ts` had a 10s one). The suite runs files concurrently, so the deadline measures machine load, and when it fires inside the ready gate the base-ref probe reproduces it on `main` and settles `ready_gate_out_of_scope`. Use `waitForStdoutMarker` from [`v2/src/testing/subprocess-marker.ts`](../src/testing/subprocess-marker.ts): it resolves on the marker and rejects after the child closes with its exit code, signal, and a bounded stderr tail. A child that holds a pipe open without exiting waits until the file's test timeout; the helper has no private deadline.
- **`// @mutate` directives** (inert, do not write): the checkpoint verifier that read them was retired and the corpus was swept on 2026-09-09. Mutation evidence comes from the diff-derived verifier over the resolved killing set; the only directive it reads is the `@mutate-equivalent` escape hatch on a source line. A copied `// @mutate` comment pins nothing.
...
The `DescendantTracker` class (historical example, frozen at `v1/src/modes/patch/reap.ts`) demonstrates the agent-runnable pattern:
...
The behavior under test (capture descendants, kill survivors by PID+identity, prune gone/reused PIDs) is pure and does not depend on real OS scheduling. Its full test suite is frozen at `v1/test/modes/patch/reap.test.ts`.
...
### Checkpoint test-file references
...
Forbidden production test seams (enforced by `bun run check` via `scripts/guard-production-test-flags.ts`, an AST walk over every non-`.test.` file under `v2/src` and `shared`): type members, function parameters, module variables, and exported functions or variables named `*ForTest`/`*ForTests`, plus `invert*` parameters. The guard is verified against real source — fixtures lifted from production shapes and a run-time meta-test that enumerates seams under the scan roots and proves each is reported — so a green guard means the tree carries none. Tests that need to disable a production check inject a neutral dependency instead: ready-gate repair fence tests pass `persistedRepairFenceEnforcer: async () => undefined` on `WriteLoopInput` / `ReviewMutationResumeDeps` rather than a production bypass flag, and publication-time mutation-repair tests inject `mutationRepairBindingFactory`. Contract details: [`coding-standards.md` § Production test seams](./coding-standards.md#production-test-seams).
