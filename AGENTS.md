...
- root — shared glue, config, public docs
- `v1/` — current shipping implementation (src, test, spec, docs, scripts)
...
- Flip draft→ready with `bun run ready` (`install → check:fix → typecheck → test → check`, 10-min deadline; override via `JARVIS_READY_TIMEOUT_MS`).
