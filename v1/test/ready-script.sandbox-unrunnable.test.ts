...
describe("ready tier parsing and step lists", () => {
  test("package biome scripts use bun's resolved biome binary", () => {
    ...
    expect(check).toContain("bun biome check .");
    expect(check).toContain("bun test scripts/ci-test-scope.test.ts");
    expect(check).toContain("bun run scripts/guard-sync-child-processes.ts");
    expect(check).toContain("bun run scripts/guard-deterministic-daemon-tests.ts");
    expect(check).toContain("bun run scripts/guard-test-double-production-calls.ts");
