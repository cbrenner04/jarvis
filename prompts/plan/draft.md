...
- **One artifact per bullet.** A bullet under `## Acceptance criteria`, `## Decisions`, or `## Documentation updates` may name at most one backticked repo-relative artifact path. Split work on multiple files into one bullet per file; prose bullets naming no artifact are allowed.
