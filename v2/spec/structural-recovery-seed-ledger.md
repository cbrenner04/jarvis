...
- **persist-events (`persist-accepted-equivalent-mutation-events`, P0 escape-hatch audit half) DEFERRED.** Failed 3×: biome cognitive-complexity in 3 finalization functions (`buildFinalizationErrorResponse`, the ready-finalizer closure, `publishWithReadyRepair`) + a hard-to-cover `ready_flip_failed` acceptedSites guard (possibly an equivalent mutation — `result.acceptedSites` reads undefined even on correct code). The audit-events design fights both the complexity gate and coverage; needs a simpler implementation or a dedicated clean pass. Escape-hatch directive itself already landed (#3188), so this is a nice-to-have. Workspace abandoned.
