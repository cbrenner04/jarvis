...
**Landing a pipeline's stacked stage PRs (operator practice, 2026-08-31).** `approve` advances the gate but does not merge the approved stage's PR, so a pipeline finishes with 3–4 stacked intermediate PRs. Note a sharp gotcha: the plan workflow is *designed* to *move* (`git mv`) its ready-intent into the spec's `intent.md`, but that move only happens when the ready-intent is on the plan stage's base — and **the pipeline plan stage bases off the main pinned at pipeline admission, not current main, so merging the intent PR at `approve-intent` does NOT get the plan to move it** (verified 2026-08-31, pipeline `45f78585`: plan still recreated `intent.md`, leaving the merged intent PR's ready-intent orphaned on main). So until the base-rebase fix ships, **do not rely on merge-at-gate to trigger the move.** Practical landing: let the pipeline run to the implement stage, then merge the terminal implement PR (retarget → `main`; it subsumes the spec), close the intent and plan stage PRs, and delete any orphan `ready-intents/*.md` the intent stage left on main. Seeded: `merge-pipeline-stage-pr-at-its-approval-gate` (needs both approve-time intent merge AND plan-stage base rebase). Cleanup: delete this note when that seed ships.
...
**Nothing you normally check tells you a workflow is finished (2026-07-21).** Three separate signals all read "done" while agents were still writing the worktree — attached `jarvis run workflow` exit after workflow entry-terminal rollup, `jarvis run list` showing every row `completed` / `not-live`, and an idle run table. Post-completion mutation verification keeps running in the worktree after all of them, and implement in-loop mutation verification can also run while the write step is still `in-progress` (after `done`/`no-work` but before that iteration checkpoints `complete`). While either verifier runs, the tree legitimately contains deliberately broken source (the verifier flips a guard, e.g. `===` → `!==`, then runs scoped tests) — reading the tree in that window shows uncommitted garbage that looks exactly like a harness defect. **Before concluding anything about a v2 run, require all three: no process at all under `lsof +D <worktree>`, a clean `git status`, and `local == remote`.** Seeds: `workflow-commands-block-the-operator-terminal`, ready-intent `workflow-command-reports-terminal-workflow-failure`.
...
- **A settled write+review pair with no PR does not mean publication was skipped — its row dispatches late.** The publication step (push, draft PR, ready gate) appears as its own run row, sometimes minutes after the write and review rows both read `completed`. Hand-finishing in that window races the late row: it resets the worktree to a squashed completion commit and its push collides with yours (observed 2026-08-30: hand-push → daemon publication row dispatched → non-fast-forward rejection → `completion_commit_failed`, force-push to reconcile). Check `jarvis run list --branch <spec-dir>` for the third row and give it several minutes; hand-finish only after that row has genuinely failed terminally. An automated waiter must require **both** no live/queued/in-progress row **and** some row carrying a PR number — liveness alone exits in the gap before the publication row exists: `while true; do rows=$(jarvis run list --branch <b>); [ -z "$(echo "$rows" | awk -F'\t' '$4=="in-progress"||$4=="queued"||$5=="live"')" ] && [ -n "$(echo "$rows" | awk -F'\t' '$14!="-"&&$14!=""')" ] && break; sleep 60; done`
- **`jarvis run log` records lifecycle events, not the agent's stream.** A long claude/cursor iteration always looks silent there, so run-log silence is structurally uninformative — never evidence of a stall. The idle/wall watchdogs exist to judge stalls; don't kill a quiet run without a terminal signal or hard evidence (a killed working run forfeits its whole iteration; observed 2026-08-07 on a healthy 30-min run killed over a load hunch).
...
**Agent-written cognitive complexity: three enforcement boundaries (2026-08-31).** (1) **Durability checkpoints** (`commitSettledIteration` / controlled-loss quiescence): best-effort scoped `biome format --write` on enumerated changed paths; commit on file changes without lint/format gating — complexity lint at the checkpoint seam no longer strands work or settles `iteration_commit_failed`. (2) **Terminal completion and ready-gate repair re-commits** (`forceDistinctCommit` publication boundary): fail-closed scoped `biome check --write` on enumerated changed paths before staging — uncommitted lint-dirty paths (including `noExcessiveCognitiveComplexity`, max 24) still settle `completion_commit_failed` here. (3) **Ready gate and CI**: complexity enforcement remains; `bun run fix` / `check:fix:unsafe` cannot repair cognitive complexity. Recovery when terminal completion or the gate fails on complexity: add a `// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: <reason>` above the function (or extract helpers preserving guard text so `@mutate` pins still match), then finalize or resume. Recurs across sessions (#2591, #2595, and later).
