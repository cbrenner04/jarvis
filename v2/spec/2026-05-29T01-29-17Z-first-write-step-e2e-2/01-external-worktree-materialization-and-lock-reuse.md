...
- Add or update a durable `v2/docs/` operator-facing home for where the worktree appears and what `.jarvis.lock` means during a Phase 1 run if no existing doc fits cleanly.
...
- [ ] No Phase 1 code introduces durable SQLite state or any persistence beyond the external worktree and git checkout, as shown by the landed code paths and tests.
