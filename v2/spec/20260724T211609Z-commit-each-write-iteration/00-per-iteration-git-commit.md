...
- [x] A new `write-loop.test.ts` case drives multiple `progress` iterations that each
...
- [x] A new `write-loop.test.ts` case runs a `progress` iteration that materializes no
...
- [x] A new `write-loop.test.ts` case completes one changed `progress` iteration, then
...
- [x] A new `write-loop.test.ts` case aborts immediately after a settled `progress` step
      ...
      fails the test.
- [x] A new `write-loop.test.ts` case forces the iteration committer to throw on `progress`
...
- [x] `write-loop-dirty-completion.sandbox-unrunnable.test.ts` stays green (terminal
