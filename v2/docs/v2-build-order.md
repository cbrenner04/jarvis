...
CLI entry at `v2/src/cli.ts`, and a flat-root co-located test shape under
`v2/src/*.test.ts` (what shipped). Target shape — role-based domain directories
with tests beside modules — is pinned in
[`v2-architecture.md` → Source layout](v2-architecture.md#source-layout).
